...
def set_version(version: str):
    ...
    if new_content == content:
        # Fallback: replace first version = "x.y.z" after [workspace.package]
...
**Linux / macOS:**
```bash
tar xzf CoraCore-{tag}-<target>.tar.gz
chmod +x coracore
sudo mv coracore /usr/local/bin/
```

**Windows:**
```powershell
Expand-Archive CoraCore-{tag}-<target>.zip -DestinationPath .
```
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="CoraCore Release Automation",
...
Exemplos:
  ...
  python scripts/release.py --upload ./dist   # upload artefatos para ultima release
    ...
    print("=" * 60)
    print("  CoraCore Release Automation")
    print("=" * 60)
    ...
    n += 1
    steps.append(f"{n}. Actions CI compila para 6 plataformas:")
    ...
    print(f"    - Linux x64 (x86_64-unknown-linux-gnu)")
    print(f"    - Linux ARM64 (aarch64-unknown-linux-gnu)")
    print(f"    - macOS x64 (x86_64-apple-darwin)")
    print(f"    - macOS ARM64 (aarch64-apple-darwin)")
    print(f"    - Windows x64 (x86_64-pc-windows-msvc)")
    print(f"    - Windows ARM64 (aarch64-pc-windows-msvc)")
    print(f"\n  Artefatos gerados:")
    print(f"    - CoraCore-{tag}-<target>.tar.gz (Linux/macOS)")
    print(f"    - CoraCore-{tag}-<target>.zip (Windows)")
    print(f"    - CoraCore-{tag}-<target>.sha256 (checksums)")
