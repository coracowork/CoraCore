...
- All request/response types defined in `cora-cowork-api-types`
- `cora-cowork-api-types` must NOT depend on axum/tower or any HTTP framework
- Use `cora-cowork_common::ApiError` only at API/HTTP boundaries such as routes and middleware. Service/domain code must prefer crate-owned errors (`ConversationError`, `TeamError`, etc.) and map them to `ApiError` in route modules. Do not introduce new `AppError` usages; it exists only as a temporary compatibility alias.
...
- Repository traits in `cora-cowork-db`, prefixed with `I`
- Concrete implementations prefixed with `Sqlite`
- Row models in `cora-cowork-db/src/models/`
...
- All assembly happens in `cora-cowork-app`'s `build_*_state()` functions
...
New subprocess spawn sites must use `cora-cowork_runtime::Builder::agent(program)` or `cora-cowork_runtime::Builder::clean_cli(program)`. Do NOT use raw `tokio::process::Command`. See [ARCHITECTURE.md § Runtime Infrastructure](./ARCHITECTURE.md#runtime-infrastructure) for details.
...
1. Request/response types → `cora-cowork-api-types/src/{domain}.rs`
2. Handler function → `crates/cora-cowork-{domain}/src/routes.rs`
3. Business logic → `crates/cora-cowork-{domain}/src/service.rs`
4. Register route in `domain_routes()` function
5. Add test → `crates/cora-cowork-{domain}/tests/` or `crates/cora-cowork-app/tests/`

### Add Migration

1. Next number → `ls crates/cora-cowork-db/migrations/`
2. Create `NNN_descriptive_name.sql` with `IF NOT EXISTS`

### Add WebSocket Event

1. Event type → `cora-cowork-api-types`
...
> ⚠️ **When to run what:**
> - During development: only test the crate you're working on → `cargo test -p cora-cowork-<crate>`
...
> - `cargo clippy -p cora-cowork-<crate>` and `cargo test -p cora-cowork-<crate>` typically complete in under 1 minute.

### During Development (fast feedback loop)

```bash
cargo test -p cora-cowork-<crate>                          # Test the crate you changed
cargo clippy -p cora-cowork-<crate> -- -D warnings         # Lint the crate you changed
```
...
cargo clippy -p cora-cowork-<crate1> -p cora-cowork-<crate2> -- -D warnings              # Lint affected crates
cargo test -p cora-cowork-<crate1> -p cora-cowork-<crate2>                               # Test affected crates
```
