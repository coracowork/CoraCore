//! Integration tests for file-based MCP Agent adapters (Opencode, Corars, cora-cowork).
//!
...
//!
//! For cora-cowork, we use a mock repository since it reads from the DB.

use std::collections::HashMap;
use std::sync::Arc;

use cora_cowork_common::McpSource;
use cora_cowork_mcp::{CoraCoworkAdapter, McpAgentAdapter, McpServerTransport};

// ===========================================================================
// cora-cowork adapter (DB-backed)
// ===========================================================================

mod cora_cowork {
    use super::*;
    use cora_cowork_db::models::McpServerRow;
    use cora_cowork_db::{CreateMcpServerParams, DbError, IMcpServerRepository, UpdateMcpServerParams};
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn update_status(
            ...
            _id: &str,
            _s: &str,
            _lc: Option<cora_cowork_common::TimestampMs>,
        ) -> Result<(), DbError> {
            unimplemented!()
        }
    ...
    #[tokio::test]
    async fn source_is_cora_cowork() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = CoraCoworkAdapter::new(repo);
        assert_eq!(adapter.source(), McpSource::CoraCowork);
    }
    ...
    async fn install_is_noop() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = CoraCoworkAdapter::new(repo.clone());
    ...
    async fn trait_object_safety() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter: Arc<dyn McpAgentAdapter> = Arc::new(CoraCoworkAdapter::new(repo));
        assert_eq!(adapter.source(), McpSource::CoraCowork);
        assert!(adapter.is_installed().await.unwrap());
    }
}
...
mod opencode {
    use super::*;
    use cora_cowork_mcp::OpencodeAdapter;
...
// ===========================================================================
// Corars adapter (CLI + TOML-backed)
// ===========================================================================

// Note: Full lifecycle tests for Corars require the `corars` CLI to be
// installed (for `--config-path`). The unit tests in corars.rs thoroughly
// cover TOML parsing, serialization, and roundtrip behavior. Here we
// verify the public API surface.

mod corars {
    use super::*;
    use cora_cowork_mcp::CorarsAdapter;

    #[test]
    fn source_is_corars() {
        assert_eq!(CorarsAdapter.source(), McpSource::Corars);
    }

    #[test]
    fn trait_object_safety() {
        let adapter: Box<dyn McpAgentAdapter> = Box::new(CorarsAdapter);
        assert_eq!(adapter.source(), McpSource::Corars);
    }
}
