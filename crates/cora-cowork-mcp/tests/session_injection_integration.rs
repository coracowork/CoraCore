...
use cora_cowork_common::McpServerStatus;
use cora_cowork_mcp::{
...
fn si_7_builtin_image_gen_injection() {
    ...
    if let Some(builtin) = build_builtin_image_gen_server(&caps, "/usr/local/bin/cora-cowork-img-gen", &img_config) {
    ...
    match builtin {
        AcpSessionMcpServer::Stdio { name, command, env, .. } => {
            assert_eq!(name, "cora-cowork-image-generation");
            assert_eq!(command, "/usr/local/bin/cora-cowork-img-gen");

            // Verify all 6 env vars are present
            assert_eq!(env.len(), 6);

            let env_map: HashMap<&str, &str> = env.iter().map(|p| (p.name.as_str(), p.value.as_str())).collect();
            assert_eq!(env_map["cora_cowork_IMG_MODEL"], "dall-e-3");
            assert_eq!(env_map["cora_cowork_IMG_API_URL"], "https://api.openai.com/v1");
            assert_eq!(env_map["cora_cowork_IMG_API_KEY"], "sk-test-key");
            assert_eq!(env_map["cora_cowork_IMG_SIZE"], "1024x1024");
            assert_eq!(env_map["cora_cowork_IMG_QUALITY"], "hd");
            assert_eq!(env_map["cora_cowork_IMG_STYLE"], "natural");
        }
