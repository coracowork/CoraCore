...
pub fn build_builtin_image_gen_server(
    ...
    Some(AcpSessionMcpServer::Stdio {
        name: "cora-cowork-image-generation".into(),
...
fn build_image_gen_env(config: &ImageGenConfig) -> Vec<NameValuePair> {
    let entries: [(&str, &Option<String>); 6] = [
        ("cora_cowork_IMG_API_KEY", &config.api_key),
        ("cora_cowork_IMG_API_URL", &config.api_url),
        ("cora_cowork_IMG_MODEL", &config.model),
        ("cora_cowork_IMG_QUALITY", &config.quality),
        ("cora_cowork_IMG_SIZE", &config.size),
        ("cora_cowork_IMG_STYLE", &config.style),
    ];
    ...
    fn builtin_image_gen_with_full_config() {
        ...
        match result.unwrap() {
            AcpSessionMcpServer::Stdio {
                ...
                assert_eq!(env.len(), 6);
                assert_eq!(env[0].name, "cora_cowork_IMG_API_KEY");
    ...
    fn builtin_image_gen_with_partial_config() {
        ...
        match result.unwrap() {
            AcpSessionMcpServer::Stdio { env, .. } => {
                ...
                assert_eq!(env[0].name, "cora_cowork_IMG_API_URL");
                assert_eq!(env[0].value, "https://api.openai.com");
                assert_eq!(env[1].name, "cora_cowork_IMG_MODEL");
