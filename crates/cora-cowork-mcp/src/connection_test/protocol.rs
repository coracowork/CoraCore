...
use cora_cowork_api_types::{McpAuthMethod, McpConnectionTestErrorCode, McpConnectionTestResult, McpToolResponse};
...
const CLIENT_NAME: &str = "cora-cowork-mcp-test";
...
fn command_not_found_message(command: &str) -> String {
    match missing_command_runtime(command) {
        "node" => format!(
            "Command not found: {command}. CoraCore usually provisions its managed Node runtime automatically. Retry the connection test, check backend logs if it still fails, or configure this MCP server to use an absolute command path."
        ),
        "bun" => format!(
            "Command not found: {command}. Install Bun (which includes bun/bunx), then restart cora-cowork or configure this MCP server to use an absolute command path."
        ),
        "uv" => format!(
            "Command not found: {command}. Install uv, then restart cora-cowork or configure this MCP server to use an absolute command path."
        ),
        "python" => format!(
            "Command not found: {command}. Install Python, then restart cora-cowork or configure this MCP server to use an absolute command path."
        ),
        "deno" => format!(
            "Command not found: {command}. Install Deno, then restart cora-cowork or configure this MCP server to use an absolute command path."
        ),
        _ => format!(
