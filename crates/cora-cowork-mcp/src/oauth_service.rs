...
use cora_cowork_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use cora_cowork_common::{TimestampMs, now_ms};
use cora_cowork_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
const DEFAULT_CLIENT_ID: &str = "cora-cowork";
...
impl McpOAuthService {
    ...
    pub async fn logout(&self, server_url: &str) -> Result<(), McpError> {
        match self.token_repo.delete(server_url).await {
            ...
            Err(cora_cowork_db::DbError::NotFound(_)) => {
    ...
    async fn handle_callback_connection(
        ...
        let response = "HTTP/1.1 200 OK\r\n\
            ...
            <p>You can close this window and return to cora-cowork.</p>\
            </body></html>";
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        ...
        ) -> Result<cora_cowork_db::models::OAuthTokenRow, cora_cowork_db::DbError> {
            unimplemented!()
        }

        async fn delete(&self, _: &str) -> Result<(), cora_cowork_db::DbError> {
            Ok(())
        }

        async fn list_authenticated_urls(&self) -> Result<Vec<String>, cora_cowork_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, url: &str) -> Result<(), cora_cowork_db::DbError> {
            Err(cora_cowork_db::DbError::NotFound(format!(
