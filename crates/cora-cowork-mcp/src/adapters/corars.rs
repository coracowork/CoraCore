...
const CLI_NAME: &str = "corars";

/// MCP Agent adapter for Corars.
///
/// Corars stores MCP configuration in a TOML config file. The config path
/// is obtained via `corars --config-path`.
///
/// # Config Format (TOML)
///
/// ```toml
...
/// ```
pub struct CorarsAdapter;

#[async_trait::async_trait]
impl McpAgentAdapter for CorarsAdapter {
...
/// Resolve the TOML config file path from the `corars` CLI.
///
/// Newer `corars` exposes this via the `config path` subcommand; older
...
async fn get_config_path() -> Result<String, McpError> {
    ...
    // Legacy fallback for `corars` versions predating the `config` subcommand.
    ...
    if path.is_empty() {
        return Err(McpError::AgentOperationFailed(
            "corars config path returned empty output".into(),
        ));
    }
    Ok(path)
}
