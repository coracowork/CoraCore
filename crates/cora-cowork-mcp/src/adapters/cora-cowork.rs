...
/// MCP Agent adapter for Coracowork itself.
///
/// Unlike CLI-based adapters, this adapter reads/writes directly to the
/// local database. It is always "installed" since CoraCowork is the host
/// application.
///
/// # Behavior
///
...
pub struct CoracoworkAdapter {
    repo: Arc<dyn IMcpServerRepository>,
}

impl CoracoworkAdapter {
...
impl McpAgentAdapter for CoracoworkAdapter {
    fn source(&self) -> McpSource {
        McpSource::Coracowork
    }
    ...
    async fn install_server(&self, _name: &str, _transport: &McpServerTransport) -> Result<(), McpError> {
        ...
        // Coracowork the server is already in the DB.
        Ok(())
    }
