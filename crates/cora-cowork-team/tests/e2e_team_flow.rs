...
use cora_cowork_ai_agent::protocol::events::{AgentStreamEvent, FinishEventData};
use cora_cowork_ai_agent::shared_kernel::approval_key;
use cora_cowork_ai_agent::types::{BuildTaskOptions, SendMessageData};
use cora_cowork_api_types::AgentModeResponse;
...
use cora_cowork_team::event_loop::AgentLoopContext;
...
use cora_cowork_team::service::TeamSessionService;
use cora_cowork_team::{TeamAgent, TeamProjectionMessageStore, TeamSession, TeammateRole};
...
impl TeamProjectionMessageStore for NoopProjectionStore {
    ...
    async fn find_projected_message(
        ...
        _msg_id: &str,
        _msg_type: &str,
    ) -> Result<Option<cora_cowork_db::models::MessageRow>, cora_cowork_team::TeamError> {
        Ok(None)
    }

    async fn insert_projected_message(
        &self,
        _row: &cora_cowork_db::models::MessageRow,
...
impl IAgentTask for RecordingAgent {
    ...
    async fn send_message(&self, data: SendMessageData) -> Result<(), cora_cowork_ai_agent::AgentSendError> {
        ...
        match &self.fail_with {
            Some(msg) => Err(cora_cowork_ai_agent::AgentSendError::from_agent_error(AgentError::internal(
                msg.clone(),
            ))),
            None => Ok(()),
        }
...
#[async_trait]
impl cora_cowork_ai_agent::IWorkerTaskManager for StubTaskManager {
...
fn backend_path() -> Arc<PathBuf> {
    Arc::new(PathBuf::from("/tmp/coracore-e2e-test"))
}
...
async fn setup_session_with_turn_recorder_inner(
    ...
    let task_manager_dyn: Arc<dyn cora_cowork_ai_agent::IWorkerTaskManager> = task_manager.clone();
    ...
    let team = cora_cowork_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
...
async fn setup_session_with_runtime_ports(
    ...
    let task_manager_dyn: Arc<dyn cora_cowork_ai_agent::IWorkerTaskManager> = task_manager;
    let team = cora_cowork_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
        workspace: "/tmp/e2e-team".into(),
...
async fn s3a_on_agent_finish_writes_idle_notification_to_lead() {
    ...
    session
        .scheduler()
        .set_status("worker-1", cora_cowork_team::TeammateStatus::Working)
        .await
        .unwrap();
    ...
    assert_eq!(
        worker_status,
        cora_cowork_team::TeammateStatus::Idle,
...
async fn s3b_lead_finish_does_not_write_idle_notification() {
    ...
    session
        .scheduler()
        .set_status("lead-1", cora_cowork_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3c_finish_triggers_lead_wake_with_idle_notification() {
    ...
    session
        ...
        .write(
            ...
            "worker-1",
            "lead-1",
            cora_cowork_team::MailboxMessageType::Message,
            "do the work",
            None,
        )
        .await
...
async fn one_notify_drains_five_intents_after_busy_turn() {
    ...
    for index in 1..=5 {
        ...
        assert_eq!(ack.enqueue_status, cora_cowork_api_types::TeamMessageEnqueueStatus::Queued);
...
async fn s10_error_finish_sets_agent_status_to_error() {
    ...
    assert_eq!(
        status,
        cora_cowork_team::TeammateStatus::Error,
