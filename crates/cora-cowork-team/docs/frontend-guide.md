...
| 模块 | 状态 | 说明 |
|------|:---:|------|
| D28a `is_team_capable_backend` 白名单 | ✅ | `capability.rs`，白名单 `claude / codex / gemini / corars` |
...
Payload 类型定义：`cora-cowork-api-types::TeamMcpStatusPayload`、`TeamMcpPhase`；另有 `TeammateMessagePayload`（为 teammate 之间消息的左气泡展示预留）。
...
Payload 类型定义在 `crates/cora-cowork-api-types/src/team.rs`（含 `TeamMcpPhase`、`TeamMcpStatusPayload`、`TeammateMessagePayload`）。**HTTP 没有状态轮询端点**，想知道 agent 现在在干啥只能靠 WS。
