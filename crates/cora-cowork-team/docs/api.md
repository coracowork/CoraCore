...
所有端点前缀 `/api/teams`，全部需要 JWT。响应统一 `ApiResponse<T>` / `ErrorResponse`。DTO 定义在 `crates/cora-cowork-api-types/src/team.rs`。
...
- `backend`：`acp / claude / gemini / qwen / nanobot / corars / remote / openclaw-gateway`
...
走 `cora-cowork-conversation` 模块的单聊端点，跟普通单聊**完全一致**：
