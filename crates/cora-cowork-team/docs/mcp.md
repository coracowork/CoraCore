...
- 参见 `crates/cora-cowork-team/src/mcp/protocol.rs` 的 `read_frame` / `write_frame`
...
```json
{
  ...
  "result": {
    ...
    "serverInfo": { "name": "cora-cowork-team-mcp", "version": "1.0.0" },
    "capabilities": { "tools": {} }
  }
}
```
...
后端（cora-cowork-backend）负责 agent 进程的完整生命周期：启动 agent、注入 MCP 连接配置、管理 stdio bridge。这是前后端分离架构下后端的职责。
...
| 管理模块 | `cora-cowork-mcp` crate（`build_session_mcp_servers()`） | `cora-cowork-team` crate（`TeamMcpStdioConfig`） |
| 生命周期 | 跟用户配置走，持久化 | 跟 team session 走，session 停止即失效 |
| 类型 | stdio / http / sse（取决于 ACP backend 能力） | 仅 stdio（TCP bridge） |

CoraCowork 参考实现中，两套在 agent 建 session 时合并注入（`userServers + presetServers + teamServer`）。后端需要在 `ConversationService.send_message` → agent factory → `session/new` 路径上做同样的合并。
...
| 操作 | 现有 API | 位置 |
|------|----------|------|
| 杀旧进程 | `IWorkerTaskManager::kill(conv_id, reason)` | `crates/cora-cowork-ai-agent/src/task_manager.rs` |
...
session_new() 构造 payload                    ← 需要改
    ...
    │      "type": "stdio",
    │      "name": "cora-cowork-team",
    │      "command": "cora-cowork-backend",
    │      "args": ["mcp-bridge"],
    │      "env": [
...
ACP CLI (claude / codex / ...) 收到 session/new
    ...
    ▼
cora-cowork-backend mcp-bridge (子进程)
...
| 文件 | 改动 | 影响范围 |
|------|------|----------|
| `cora-cowork-ai-agent/src/types.rs` | `AcpBuildExtra` 加 `#[serde(default)] team_mcp_stdio_config: Option<TeamMcpStdioConfig>` | 旧 extra 无此字段 → `None`，零影响 |
| `cora-cowork-ai-agent/src/acp_agent.rs :: session_new()` | 如果 `team_mcp_stdio_config.is_some()`，构造 `mcpServers` 数组追加到 payload | 单聊 config 为 `None` → 不追加，零影响 |
| `cora-cowork-mcp/src/session_injection.rs` | team MCP 作为额外一项 append 到 `build_session_mcp_servers()` 结果里（或在 `session_new` 里直接 merge） | 视实现选择 |
...
`crates/cora-cowork-team/src/mcp/bridge.rs`：
...
stdio bridge 作为 `cora-cowork-backend` 的 subcommand 实现，不单独出二进制：

```
cora-cowork-backend mcp-bridge
```
...
| 内容 | 位置 |
|------|------|
| TCP server 启停 | `crates/cora-cowork-team/src/mcp/server.rs :: TeamMcpServer` |
| 握手 + 鉴权 | `crates/cora-cowork-team/src/mcp/server.rs :: handle_initialize` |
| Tool dispatch | `crates/cora-cowork-team/src/mcp/server.rs :: dispatch_tool` |
| Tool descriptor（tools/list 返回） | `crates/cora-cowork-team/src/mcp/tools.rs :: all_tool_descriptors` |
| Backend 白名单 | `crates/cora-cowork-team/src/mcp/tools.rs :: SPAWN_BACKEND_WHITELIST` |
| 帧 + JSON-RPC 类型 | `crates/cora-cowork-team/src/mcp/protocol.rs` |
| Stdio config 结构 | `crates/cora-cowork-team/src/mcp/bridge.rs :: TeamMcpStdioConfig` |
| Session 与 server 生命周期绑定 | `crates/cora-cowork-team/src/session.rs :: TeamSession::start` |
| Scheduler 对 MCP action 的执行 | `crates/cora-cowork-team/src/scheduler.rs :: execute_action` |
| `SpawnAgent` 空壳位置 | `crates/cora-cowork-team/src/scheduler.rs`（搜 `"spawn_agent action — requires TeamSession to complete"`）|
| Mailbox 写入 | `crates/cora-cowork-team/src/scheduler.rs :: handle_send_message` |
