# cora-cowork-backend Phase1 — 完整调研 + 全量开发计划

> **阶段目标**：phase1 = **完整调研 + backend 实现 CoraCowork 参考 team 能力的全量开发计划**。
>
...
> - [cora-cowork-audit.md](./cora-cowork-audit.md) — CoraCowork 能力清单（`ed8a6bcd3`）
>
> **开发文档**：
...
1. **只描述后端要做什么**（cora-cowork-backend 仓内），不涉及 CoraCowork 参考实现的渲染/交互层
2. **只考虑 ACP agent type**（claude / codex 走 ACP；Gemini 走 ACP；corars 是 stub）
3. **一人一模块 ≤ 200 行**（[modules.md §2/§3/§7/§8/§9](./modules.md)；例外见下方"例外模块"清单）
4. **Wave 先拆纯净非业务，再拆业务串接**；底层不允许超级模块
5. **文档必须带交叉引用链接**（全局 CLAUDE.md 规则）
6. **所有结论基于审计报告的事实，禁止推测**（cora-cowork-audit / backend-audit 都锚定到源码行号）
...
- `D5b-1`（188 行 CoraCowork 原文 .txt + 48 行 Rust `include_str!`）——模板文本是原料不是逻辑
...
### Wave 1（纯净原料，10 人并行）
> **开工准则**：彼此无依赖，单文件可交付。
- D1  `cora-cowork-api-types::team_mcp` 子模块（类型种子）
...
- **D4b `TEAM_SPAWN_AGENT_DESCRIPTION` 原文常量**（补漏 P0#48，替换现有极简描述为 CoraCowork `toolDescriptions.ts` 原文） — [backend-audit §3.5 #48](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现)
...
- D6  `cora-cowork-backend mcp-bridge` 子命令
...
- D7b **send 路径接 wake + `files` 附件（P0#45）+ log-not-throw（P0#46）** — [backend-audit §3.5 #45/#46](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现)
...
- **D11.5 `remove_team` 级联 kill agent 进程**（补漏 P0#47，避免 agent 孤儿进程） — [backend-audit §3.5 #47](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现)
...
- W3-D15a `CreateAgentRequest.conversation_id` 字段 / W3-D15b `create_team` 复用 conversation 分支 — [cora-cowork-audit §1.1](./cora-cowork-audit.md#11-能力清单) "单聊→team 的 conversation 复用"
...
> **目标**：把 CoraCowork §8 列的 17 条硬约束里 phase1 Wave 2 跳过的 8 条全部落地。

- W4-D25a `AgentStreamChunk` enum / W4-D25b `subscribe_stream()` trait / W4-D25c-1 **broadcast channel 字段 + subscribe_stream impl** / W4-D25c-2 **5 个 chunk emit 点注入** — [backend-audit P1#53](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现)
- W4-D18a `active_wakes` 重入锁 / W4-D18b-1 **wake_timeouts 存储字段 + clear** / W4-D18b-2 **arm_wake_timeout spawn task（select! 主体）** / W4-D18c session 接入 wake lock — [cora-cowork-audit §8 #1/#2](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点)
- W4-D19a `finalized_turns` 存储 + API / W4-D19b session 接入 dedup — [cora-cowork-audit §8 #3](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点)
...
- W4-D22 Inactivity watchdog handler — [cora-cowork-audit §2.1 inactivity watchdog](./cora-cowork-audit.md#21-能力清单)
...
> **目标**：让"单聊→建团→真 spawn→真 kill"四条完整链路全部闭环，等价 CoraCowork 参考实现。

- W5-D26a `GuideMcpServer` 启停 / W5-D26b-1 **`cora_create_team` args 解析 + 默认值** / W5-D26b-2 **`handle_cora_create_team` 调 service + 返回结构化** / W5-D26c `handle_cora_list_models` / W5-D26d 建团后 3 个 WS 事件 — [backend-audit P1#28/#33](./backend-audit.md#32-p1能跑但体验差--关键硬约束)
...
**验收标准**（Wave 5 完工门禁）：
- ❏ solo agent 能调 `cora_create_team` 建出 team 且 leader 被正确复用（W5-D26a/b/c/d e2e）
...
- 编译 `cargo build --release`，跑 `./target/release/cora-cowork-backend`
...
- [ ] `sqlite3 cora-cowork.db "SELECT id, extra FROM conversations WHERE extra LIKE '%team_mcp%'"` 输出
- [ ] 后端 `RUST_LOG=cora_cowork_team=debug,cora_cowork_ai_agent::manager::acp=debug` 运行日志（含 MCP tools/call 行）
...
| 决策 | 内容 | 作用域 | 来源 |
|------|------|:-:|------|
| stdio bridge 打包 | 打进主二进制 `cora-cowork-backend mcp-bridge` subcommand；Wave 5 Guide bridge 复用同一 subcommand 加 `--guide` 分支 | W1 / W5 | team-lead 确认 · [mcp.md §4.6](../mcp.md#46-stdio-bridge-方案打进主二进制) |
...
| Prompt 注入路径 | **wake 时作为首个 send_message content**（不走 preset_context） | W2 / W5 | team-lead 确认 · [cora-cowork-audit §2.1](./cora-cowork-audit.md#21-能力清单) wake 时机 |
| AcpBuildExtra 字段 | `team_mcp_stdio_config: Option<TeamMcpStdioConfig>`（snake_case） | W1 | [interface-contracts §2](./interface-contracts.md#2-cora-cowork-ai-agenttypesacpbuildextra-扩展wave-1--模块-d2) |
| 后端 Prompt 文本 | 必须**原样**复用 CoraCowork `leadPrompt.ts` / `teammatePrompt.ts` / `teamGuidePrompt.ts`，禁翻译禁改写 | 所有 Wave | [cora-cowork-audit §8 #5](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点) |
...
| Conversation 复用范围 | phase1 只实现 REST 复用（Wave 3 W3-D15）；`cora_create_team` MCP 路径等 Wave 5 W5-D26 | W3 / W5 | [cora-cowork-audit §1.1](./cora-cowork-audit.md#11-能力清单) |
...
| Guide MCP 与 team 内部 MCP 互斥 | `!extra.team_mcp_stdio_config` 才注入 Guide；写死在注入链路里，Wave 5 实现 | W5 | [cora-cowork-audit §8 #17](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点) |
| `team.mcp_status` 10 phase | Wave 5 W5-D31 完整落地；Wave 2 只保留占位不 emit | W5 | [cora-cowork-audit §7.6](./cora-cowork-audit.md#76-事件--ipc后端等价需提供-websocket-或-sse) |
| `TeammateStatus::Pending` | Wave 2 D8 新增 variant 区分"首次 wake"；`failed` **不**会回退到 Pending（状态机：`failed` 保持 `failed`；下次 wake 时 `status in {pending, failed}` 直接触发 role prompt 注入；inactivity / crash / 429 三条路径都只 `set_status(Failed)` 不回退） | W2 | [cora-cowork-audit §2.1 状态机](./cora-cowork-audit.md#22-agent-生命周期状态机) |
...
- CoraCowork 源码 commit：`ed8a6bcd3`（cora-cowork-audit 锚定）
- cora-cowork-backend 分支：`docs/api-for-frontend`，HEAD `21abc46`（backend-audit 锚定）
