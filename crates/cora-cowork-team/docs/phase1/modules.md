...
>
> **事实来源**：[backend-audit.md](./backend-audit.md) §1–§5 · [cora-cowork-audit.md](./cora-cowork-audit.md) §1–§4 · [mcp.md](../mcp.md) §4 · [team-prompts.md](../team-prompts.md) §2–§4
...
### D1 — `cora-cowork-api-types::team_mcp` 子模块

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-api-types/src/team_mcp.rs`（新增） + `lib.rs` 导出（修改） |
...
| 预估 LoC | 40 行 |
| 预估人天 | 0.5 |
| 接口契约 | [interface-contracts.md §1](./interface-contracts.md#1-cora-cowork-api-types-新增类型wave-1--模块-d1) |

---

### D2 — `cora-cowork-ai-agent::AcpBuildExtra` 字段扩展

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/types.rs`（修改，只加 1 个字段 + import） |
| 职责 | 加 `#[serde(default)] team_mcp_stdio_config: Option<TeamMcpStdioConfig>` |
| 输入 | D1 导出的类型 |
| 输出 | 新字段可反序列化 |
| 依赖 | D1 必须先 merge（Cargo 依赖层面）**— phase1 特例：D2 里先用 `pub use cora_cowork_api_types::TeamMcpStdioConfig;`，单元测试里用 stub struct 跑通 JSON；待 D1 merge 后只需删 stub** |
| 测试策略 | 2 条：旧 JSON（无字段）反序列化为 None；新 JSON 正确解出 config |
| 预估 LoC | 15 行 |
| 预估人天 | 0.3 |
| 接口契约 | [§2](./interface-contracts.md#2-cora-cowork-ai-agenttypesacpbuildextra-扩展wave-1--模块-d2) |

---

### D3 — `cora-cowork-team::mcp::bridge::TeamMcpStdioServerSpec`

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/bridge.rs`（修改：替换原 `TeamMcpStdioConfig` 用 `pub use` + 新增 ServerSpec） |
...
| 接口契约 | [§3](./interface-contracts.md#3-cora-cowork-teammcpbridge-新增serverspecwave-1--模块-d3) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/tools.rs`（修改）+ `server.rs` dispatch 分支（修改） |
| 职责 | 加 descriptor（文本原样复用 [team-prompts.md §5.2](../team-prompts.md#52-team-内部-mcp10-个工具)）+ phase1 最小 handler |
| phase1 最小实现 | `team_list_models`：返回固定 JSON `{agent_types:[{type:"claude",models:["claude-sonnet-4","claude-opus-4"]},{type:"codex",models:["gpt-5"]},...]}`（不读真实 registry）；`team_describe_assistant`：统一返回 `"Preset assistant not found"` 文本（backend 尚无 assistants 配置，cora-cowork-audit §7.1 "workspace" 一致未实现） |
...
| 预估人天 | 1.0 |
| 接口契约 | [§4](./interface-contracts.md#4-cora-cowork-teammcptools-新增两个工具-descriptorwave-1--模块-d4) |
| ⚠️ 硬约束 | descriptor 文本**原样**来自 CoraCowork（cora-cowork-audit §8 #5）；Wave 2 才补真实数据源 |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/tools.rs`（新增 `pub const TEAM_SPAWN_AGENT_DESCRIPTION: &str = r#"..."#;`） + 替换现有 `team_spawn_agent` descriptor 中的 description 字段引用常量 |
| 职责 | 只做一件事：把 CoraCowork `toolDescriptions.ts:1-18` 原文（"3 PRECONDITIONS + STRICT 流程"）**逐字节**复制到 Rust 常量替换后端原有的极简自造描述（"Dynamically create a new teammate agent (Lead only)."）；现有 D4 模块只管新加的 2 个工具，这条是改已有工具 |
| 依赖 | 无（纯文本常量） |
| 测试 | 2 条：常量与 team-prompts.md §5.2 `team_spawn_agent` 原文 `diff -w` 零差异；`tools/list` 返回的该工具 description 等于常量 |
| 预估 LoC | 40（常量 + 替换 + 测试） |
| 预估人天 | 0.3 |
| 事实来源 | [backend-audit §3.5 #48](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现) 标 **P0** · [cora-cowork-audit §8 #5](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点)（原文复用硬约束） · [team-prompts.md §5.2 team_spawn_agent](../team-prompts.md#team_spawn_agent) |

---

### D5 — `cora-cowork-team::prompts` 三层模板 + builder

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/prompts.rs`（**重写**：保留 `build_wake_payload` 概念，替换三个 builder） |
| 职责 | 定义四份常量（Guide / Lead / Teammate 模板 + spawn 工具描述），实现新签名 builder |
| 模板来源 | [team-prompts.md §2/§3/§4/§5](../team-prompts.md) —— **原样复用 CoraCowork 英文，禁翻译、禁改写** |
| builder 任务 | 按 [interface-contracts.md §5](./interface-contracts.md#5-cora-cowork-teamprompts-大幅扩写wave-1--模块-d5) 的 params 产出字符串；`## Your Teammates` / `## Available Agent Types` / `## Available Preset Assistants` / `## Team Workspace` 四个动态 section 按条件开关 |
| 输入 | TeamAgent / MailboxMessage / TeamTask / HashMap<slot_id,name> |
| 输出 | 5 个 pub fn（3 个 role builder + team_guide + wake_payload） |
| 依赖 | `cora-cowork-team::types`（已存在） |
| 测试策略 | 6 条快照测试：lead 最小参数；lead 带 preset assistants；teammate 最小；teammate 带 renamed；wake_payload 空邮件箱；wake_payload 有任务和邮件 |
| 预估 LoC | 需要承载 500+ 行 CoraCowork 原文文本 → **虽超 200 行约束，但都是 `r#"..."#` 常量，实际"代码"逻辑 < 150 行。此处申请例外**：模板原文是"原料"不是"逻辑"，leader 已默许（phase1 README 会重申） |
| 预估人天 | 1.5（含模板逐行从 CoraCowork 源码拷贝校对的 0.5 天） |
| 接口契约 | [§5](./interface-contracts.md#5-cora-cowork-teamprompts-大幅扩写wave-1--模块-d5) |

**例外说明**：因 CoraCowork 三份 prompt 加起来 410 行原文必须原样搬运，模板文本视作"原料"而非"逻辑"（cora-cowork-audit §8 #5 硬约束）。默认方案已按 team lead 要求把 D5 拆成 **4 个子模块 D5a / D5b-1 / D5b-2 / D5c**（即 8 人 Wave 1 里的 D5 系列），每人代码 < 200 行：

- **D5a**：Team Guide 模板常量 + `build_team_guide_prompt()`（~120 行）
- **D5b-1**：Lead prompt 常量，用 `include_str!("prompt_templates/lead.txt")` 引用 CoraCowork 原文件。目标文件 `crates/cora-cowork-team/src/prompts/lead.rs`（代码 < 50 行）+ `crates/cora-cowork-team/src/prompts/prompt_templates/lead.txt`（逐字节复制 CoraCowork `leadPrompt.ts` 模板原文）
- **D5b-2**：`build_lead_prompt()` builder 实现（依赖 D5b-1 的常量）。目标文件 `crates/cora-cowork-team/src/prompts/lead.rs` 的 builder 部分（~30 行 Rust）
...
### D6 — `cora-cowork-app mcp-bridge` 子命令

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-app/src/bridge.rs`（新增） + `main.rs`（新增 argv 分支） |
| 职责 | 实现 stdio↔TCP 透传（mcp.md §4.6 的 4 步） |
| 依赖 | `rmcp` 或手写最小 JSON-RPC 2.0 over stdio；`cora-cowork-team::mcp::protocol::{read_frame, write_frame}`；D1 的 env key |
...
| 预估人天 | 1.5 |
| 接口契约 | [§8](./interface-contracts.md#8-cora-cowork-app-新增子命令-mcp-bridgewave-1--模块-d6) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（只新增三个 pub 方法，不改 send 路径） |
...
| 预估 LoC | 150 |
| 预估人天 | 1.5 |
| 接口契约 | [§6](./interface-contracts.md#6-cora-cowork-teamsessionteamsession-新方法wave-2--模块-d7) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（只改两个 send 方法）+ `crates/cora-cowork-api-types/src/team.rs`（请求 DTO 加 `files: Option<Vec<String>>` 字段）+ `crates/cora-cowork-team/src/routes.rs`（handler 透传 files） |
...
| 预估 LoC | 120 |
| 预估人天 | 1.2 |
| 接口契约 | [§6.2](./interface-contracts.md#6-cora-cowork-teamsessionteamsession-新方法wave-2--模块-d7) |
| 事实来源 | [backend-audit §3.5 #45/#46](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现) · [cora-cowork-audit §4.1 表格备注](./cora-cowork-audit.md#41-wake-触发源) "log-not-throw" |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`send_message_to_agent` 加 `silent: bool` 参数 + 占位分支） |
| 职责 | 只做占位：`silent=true` 走和 `silent=false` 几乎一样的流程，**但不写 user bubble** 到目标 conversation（phase1 因为 Wave 2 还没接通 conversation 的 user bubble 写入路径，此模块仅让签名支持参数，实际 silent 行为的**完整测试**在 Wave 5 W5-D26b 的 `cora_create_team` 场景里真跑（那时 leader 复用 conversation 会用到 silent=true） |
| 依赖 | D7b |
| 测试 | 2 条：silent=true 不 panic；Wave 5 e2e 验证实际效果 |
| 预估 LoC | 40 |
| 预估人天 | 0.3 |
| 接口契约 | [§6.3](./interface-contracts.md#6-cora-cowork-teamsessionteamsession-新方法wave-2--模块-d7) |
| 事实来源 | [cora-cowork-audit §4.1](./cora-cowork-audit.md#41-wake-触发源) "silent=true 时 **不** 写 user bubble" |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（修改） |
| 职责 | 1) 在 `TeammateStatus` 加 `Pending` variant（默认值，取代 `None` 状态的"首次"语义，cora-cowork-audit §2.1）<br>2) `try_wake` 判断 `status in {Pending, Failed}` 时返回 `WakePayload::WithRolePrompt`，否则 `WakePayload::MailboxOnly`<br>3) `maybe_wake_leader_when_all_idle` 扩大 settled 集合到 `{Idle, Completed, Failed, Pending}`（cora-cowork-audit §8 #4） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（修改 `new` + `ensure_session`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/acp_agent.rs`（修改 `session_new_and_prompt` + 构造函数加 `backend_binary_path` 字段） |
| 职责 | 按 [interface-contracts.md §7](./interface-contracts.md#7-cora-cowork-ai-agentacp_agentsession_new_and_prompt-注入wave-2--模块-d10) 改造 `NewSessionRequest` 构造 |
...
### D11 — `cora-cowork-app` 装配 + e2e smoke test

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-app/src/state_builders.rs`（修改 `build_team_state` 签名）+ `lib.rs`（传 `backend_binary_path`）+ `crates/cora-cowork-app/tests/team_phase1_smoke.rs`（新增） |
| 职责 | 1) `build_team_state` 按 [interface-contracts.md §10](./interface-contracts.md#10-cora-cowork-appstate_buildersbuild_team_state-扩展wave-2--模块-d11) 加参数<br>2) `lib.rs::build_router` 一次性 `current_exe()` 缓存到 `Arc<PathBuf>`<br>3) e2e smoke：按 [README.md §3](./README.md) 的 8 步脚本实跑，校验 agent 能调 `team_send_message` |
| 输入 | D7 / D9 / D10 |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（仅 `remove_team` 方法内在 `stop_session` 前循环遍历 `team.agents` 调 `task_manager.kill(conv_id, Some(AgentKillReason::TeamDeleted))`） |
...
| 事实来源 | [backend-audit §3.5 #47](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现) 标 **P0**（agent 进程会变成孤儿） · [cora-cowork-audit §1.4 删除时序图](./cora-cowork-audit.md#14-删除时序图) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（仅 `list_teams`）+ `crates/cora-cowork-db/src/repository/team.rs`（加 `list_by_user`）+ `crates/cora-cowork-team/src/routes.rs`（仅 list handler） |
...
| 事实来源 | [backend-audit §1.2](./backend-audit.md#12-cratescora-cowork-teamsrcservicers--teamsessionservice) · [cora-cowork-audit §1.1 listTeams(userId)](./cora-cowork-audit.md#11-能力清单) |

### W3-D12b — `get_team(user_id, id)` 归属校验

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（仅 `get_team` 签名改造）+ `crates/cora-cowork-team/src/routes.rs`（仅 get handler） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（仅 `remove_team`）+ `crates/cora-cowork-team/src/routes.rs`（仅 delete handler） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-db/src/repository/conversation.rs`（trait 加方法）+ `crates/cora-cowork-db/src/repository/sqlite_conversation.rs`（实现） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（新增私有 fn，不触碰 `get_team`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（仅在 `get_team` 末尾加 if 分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs` 新子模块（或 `utils.rs`），暴露 `pub fn normalize_name(&str) -> String` |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`TeammateManager::rename_agent` 改造 + 内存字段 `renamed_agents: Mutex<HashMap<String, String>>`） |
...
| 事实来源 | [cora-cowork-audit §2.1 renameAgent](./cora-cowork-audit.md#21-能力清单) |

### W3-D14c — Prompt builder 读 renamed_agents 渲染 `[formerly: X]`

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/prompts/lead.rs` + `teammate.rs`（只改 `## Your Teammates` 段的渲染逻辑） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-api-types/src/team.rs`（`CreateAgentRequest` 加字段） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（`create_team` 内 for each agent 循环里加一个 if 分支） |
...
| 事实来源 | [cora-cowork-audit §1.1 "单聊→team 的 conversation 复用"](./cora-cowork-audit.md#11-能力清单) |
| ⚠️ 注意 | phase1 只实现 REST 路径复用；MCP 路径 `cora_create_team` 的复用由 Wave 5 W5-D26b 完成 |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-conversation/src/traits.rs`（新增 trait）+ `crates/cora-cowork-conversation/src/state.rs`（`ConversationService` 加 `team_router: Option<Arc<dyn ITeamMessageRouter>>`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-conversation/src/service.rs`（仅 `send_message` 入口增加一段 if） |
...
| 事实来源 | [cora-cowork-audit §7.1 "对 agent 发话"](./cora-cowork-audit.md#71-rest--ipc-等价入口backend-需要暴露的-api) |

### W3-D16c — `TeamSessionService impl ITeamMessageRouter` + 装配

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（`impl ITeamMessageRouter for TeamSessionService`）+ `crates/cora-cowork-app/src/state_builders.rs`（装配时把 team_session_service clone 作为 router 传进 conversation state） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-common/src/lib.rs`（新增常量 `TEAM_MCP_MAX_FRAME_BYTES`）+ `crates/cora-cowork-team/src/mcp/protocol.rs`（`MAX_MCP_MESSAGE_SIZE` 改为引用） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-common/src/lib.rs`（`TEAM_MCP_REQUEST_TIMEOUT_MS`）+ `crates/cora-cowork-team/src/mcp/server.rs`（dispatch_tool 外层 `tokio::time::timeout`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/types.rs`（新 enum） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/task_manager.rs`（trait 加方法） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/acp_agent.rs`（只加字段 `stream_tx: broadcast::Sender<AgentStreamChunk>` + 构造函数初始化 capacity 256 + impl `AgentManagerHandle::subscribe_stream`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/acp_agent.rs`（现有 5 个 chunk 处理点各插一行 `let _ = self.stream_tx.send(...)`） |
...
| 事实来源 | [backend-audit §3.5 #53](./backend-audit.md#35-交叉审阅补漏二轮对照-cora-cowork-audit-7-8-后新发现) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`TeammateManager` 加字段 `active_wakes: DashSet<String>` + 两个方法 `try_acquire_wake_lock` / `release_wake_lock`） |
...
| 事实来源 | [cora-cowork-audit §8 #1/#2](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点) |

### W4-D18b-1 — `wake_timeouts` 存储字段 + `clear_wake_timeout`

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`TeammateManager` 只加字段 `wake_timeouts: DashMap<String, JoinHandle<()>>` + `clear_wake_timeout(slot_id)` 实现） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`arm_wake_timeout(slot_id, stream_rx)` 方法 spawn 后台 task） |
...
| 事实来源 | [cora-cowork-audit §2.1 inactivity watchdog](./cora-cowork-audit.md#21-能力清单) |

### W4-D18c — `compute_wake_input` / `send_message` 接入 wake lock

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`compute_wake_input` / `send_message` 开头结尾加 acquire / release） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`TeammateManager` 加字段 `finalized_turns: DashMap<String, Instant>` + `begin_finalize(conv_id) -> bool` + `clear_finalized_turn(conv_id)`） |
...
| 事实来源 | [cora-cowork-audit §4.3 + §8 #3](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点) |

### W4-D19b — `on_agent_finish` / re-wake 路径接入 dedup

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`on_agent_finish` 第一行插入 `if !scheduler.begin_finalize(conv_id) { return; }`；wake 成功后调 `clear_finalized_turn`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（新 fn + `CrashReason` enum） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`handle_agent_crash` 内部"写 testament"私有 helper） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`handle_agent_crash` 主体：串接 D20b-1 的 testament + 下列"进程卫生"步骤） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（在 D20b 的方法里加 `if role == Lead` 分支） |
| 职责 | 只做 leader crash 的特殊分支：只 `set_status(Failed)`，不 remove、不 wake 其他（cora-cowork-audit §2.1）|
| 依赖 | W4-D20b |
...
| 事实来源 | [cora-cowork-audit §2.1 crash recovery](./cora-cowork-audit.md#21-能力清单) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-common/src/lib.rs`（`RATE_LIMIT_REGEX` once_cell Lazy）+ `crates/cora-cowork-team/src/session.rs`（`on_agent_finish` 里 Error chunk 分支：若 regex 命中 → `set_status(Failed)` 不走 crash） |
...
| 事实来源 | [cora-cowork-audit §2.1 "429 / 限流识别"](./cora-cowork-audit.md#21-能力清单) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`handle_inactivity_timeout(slot_id)` 被 D18b 的 timer 调用） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（`TeamSessionService` 加 `add_agent_locks: DashMap<String, Arc<tokio::sync::Mutex<()>>>` + `add_agent` 入口取锁 + `remove_team` 清 lock entry） |
...
| 事实来源 | [cora-cowork-audit §8 #14](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/protocol.rs`（新枚举 `McpNotification::McpReady { slot_id, auth_token }`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/server.rs`（`TeamMcpServer` 加字段 `ready_latch: DashSet<String>` + `ready_notify: DashMap<String, Arc<Notify>>`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/server.rs`（方法实现：收到 D24a 的通知帧时调） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/server.rs`（方法实现：外部 service 调用此方法等待 ready） |
| 职责 | 只做一件事：若 `ready_latch.contains(slot_id)` 直接 Ok；否则取 / 创建 `Notify` 放入 `ready_notify`；`tokio::select!` 等 `notify.notified()` 或 `sleep(30s)`；**timeout 分支也 `Ok(())`**（cora-cowork-audit §8 #11 graceful） |
...
| 事实来源 | [cora-cowork-audit §3.1 "MCP ready 握手"](./cora-cowork-audit.md#31-能力清单) · [cora-cowork-audit §8 #11](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点)（graceful timeout） |

### W4-D24c — Bridge 端发 `mcp_ready` 通知

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-app/src/bridge.rs`（D6 主 bridge 逻辑内 initialize 成功后追加一行 send） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/guide/server.rs`（新 struct + `start_singleton` + `stop`） |
...
| 事实来源 | [cora-cowork-audit §3.1 Team Guide MCP 生命周期](./cora-cowork-audit.md#31-能力清单) |

### W5-D26b-1 — `cora_create_team` args 解析 + 默认值补全（纯函数）

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/guide/handlers.rs`（新 `parse_create_team_args(args, caller_conversation) -> CreateTeamParams` 纯函数） |
...
| 预估 LoC | 70 |
| 预估人天 | 0.6 |
| 接口契约 | [§27.2.1](./interface-contracts.md#272-handle_cora_create_team) |

### W5-D26b-2 — `handle_cora_create_team` 调 service + 返回结构化

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/guide/handlers.rs`（handler 主体） |
...
| 预估 LoC | 70 |
| 预估人天 | 0.7 |
| 接口契约 | [§27.2.2](./interface-contracts.md#272-handle_cora_create_team) |
| 事实来源 | [cora-cowork-audit §1.2 建团流程](./cora-cowork-audit.md#12-建团流程时序图mcp-spawn) |

### W5-D26c — `handle_cora_list_models` handler

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/guide/handlers.rs`（新 fn，仅 list_models） |
| 职责 | 只做 tool 的 handler：直接复用 D4 的 `team_list_models` handler（硬编码 backend × model 表） |
| 依赖 | W5-D26a + D4 |
| 测试 | 1 条：返回 JSON schema 和 D4 一致 |
| 预估 LoC | 40 · 预估人天 0.3 |
| 接口契约 | [§27.3](./interface-contracts.md#273-handle_cora_list_models) |

### W5-D26d — 建团成功后 emit 3 个 WS 事件

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/guide/handlers.rs`（D26b 的末尾插入事件广播） |
| 职责 | 只在 `handle_cora_create_team` 成功路径尾部 emit 3 个 WS 事件：`team.listChanged` + `conversation.listChanged` + `deepLink.received { route:/team/<id> }` |
| 依赖 | W5-D26b（handler 已出结果）+ W5-D31a（事件类型定义） |
| 测试 | 1 条集成：建团成功 → WS 订阅者收到三个事件 |
| 预估 LoC | 50 · 预估人天 0.4 |
| 接口契约 | [§27.4](./interface-contracts.md#274-cora_create_team-成功后的-ws-事件) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-app/src/bridge.rs`（D6 主 bridge 里加 `if env::var(CORA_MCP_BACKEND).is_ok()` 分支） |
| 职责 | 只做 bridge 端分叉：env 里有 `CORA_MCP_BACKEND` → 走 guide bridge 模式，每条 tools/call payload 额外带 `backend` + `conversation_id`；否则走 team bridge（不动） |
...
| 事实来源 | [cora-cowork-audit §3.3 stdio↔TCP 桥架构](./cora-cowork-audit.md#33-stdio--tcp-桥架构) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/guide/capability.rs`（新增） |
| 职责 | 只做纯函数：`pub fn is_team_capable_backend(backend: &str, mcp_stdio_capable: bool) -> bool` + 硬白名单常量 `TEAM_CAPABLE_BACKENDS = &["claude", "codex", "gemini", "corars"]` |
| 依赖 | 无 |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/acp_agent.rs`（构造 instructions 的地方加分支） |
...
| 事实来源 | [cora-cowork-audit §8 #17 Guide 互斥](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点) |

### W5-D28c — `session/new.mcp_servers` 追加 Guide config + 互斥 guard

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-ai-agent/src/acp_agent.rs`（`session_new_and_prompt` 里 mcp_servers 构造的地方加分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（新 struct `SpawnAgentRequest` + `TeamSession::spawn_agent` 空壳 fn 签名，body `todo!()`） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 第一段校验） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 第二段校验） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 第三段校验） |
...
| 事实来源 | [cora-cowork-audit §2.1 MCP spawn](./cora-cowork-audit.md#21-能力清单) · backend-audit §1.5.2 SPAWN_BACKEND_WHITELIST |

### W5-D29b — `TeamSessionService::add_agent` 扩展用于 spawn

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（`add_agent` 扩展：原签名保留，内部持 W4-D23 lock + build conversation + 分配 slot_id + update repo） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 中段第 1 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 中段第 2/3 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 末段第 1 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 末段第 2 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`spawn_agent` 末段第 3 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/server.rs`（`handle_send_message` 顶部加 if 分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/server.rs`（在 D30a-1 识别后调用的 async helper） |
...
| 事实来源 | [cora-cowork-audit §2.1 shutdown 协议](./cora-cowork-audit.md#21-能力清单) |

### W5-D30b — `team_send_message` 识别 `shutdown_rejected: <reason>`

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/server.rs`（同 D30a 所在位置的第二个拦截分支） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`shutdown_agent` 方法顶部） |
...
| 事实来源 | [cora-cowork-audit §2.1 "Leader 不可 shutdown"](./cora-cowork-audit.md#21-能力清单) |

### W5-D30d-1 — `remove_agent` 改造：`task_manager.kill`

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`remove_agent` 第 1 步） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`remove_agent` 第 2 步；使用 W4-D18a/b-1/W4-D19a 已有 API） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/scheduler.rs`（`remove_agent` 第 3 步） |
...
| 事实来源 | [cora-cowork-audit §2.1 TeammateManager.removeAgent](./cora-cowork-audit.md#21-能力清单) |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-api-types/src/team.rs`（新 enum + 两个 payload struct） |
...
| 事实来源 | [cora-cowork-audit §7.6 事件](./cora-cowork-audit.md#76-事件--ipc后端等价需提供-websocket-或-sse) |

### W5-D31b-1 — `team.mcpStatus` tcp 层 2 点广播（mcp/server.rs）

| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/mcp/server.rs`（在 TCP `bind` 成功 / 失败处各插一行 broadcast） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/service.rs`（`ensure_session` 的 6 个分支处各插 broadcast） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-app/src/bridge.rs`（bridge 的 tools/list 前 / 后两个时机） |
...
| 项 | 内容 |
|---|---|
| 目标文件 | `crates/cora-cowork-team/src/session.rs`（`compute_wake_input` 内 teammate 分支末尾遍历 unread_messages 逐条 emit） |
...
| 事实来源 | [cora-cowork-audit §2.1 "active 期间输入字节流"](./cora-cowork-audit.md#21-能力清单) |
...
|:---:|:---:|---|---|:-:|:---:|
| D1 | 1 | team_mcp types | `cora-cowork-api-types/src/team_mcp.rs` | 40 | 0.5 |
| D2 | 1 | AcpBuildExtra 字段 | `cora-cowork-ai-agent/src/types.rs` | 15 | 0.3 |
| D3 | 1 | Stdio ServerSpec | `cora-cowork-team/src/mcp/bridge.rs` | 80 | 1.0 |
| D4 | 1 | 两个新 MCP 工具（list_models / describe_assistant） | `cora-cowork-team/src/mcp/{tools,server}.rs` | 150 | 1.0 |
| **D4b** | 1 | **`TEAM_SPAWN_AGENT_DESCRIPTION` 原文常量**（P0#48 补漏） | `cora-cowork-team/src/mcp/tools.rs` | 40 | 0.3 |
| D5a | 1 | Team Guide Prompt | `cora-cowork-team/src/prompts/team_guide.rs` | 120 | 0.5 |
| D5b-1 | 1 | Lead Prompt 常量 (include_str! txt) | `lead.rs` + `prompt_templates/lead.txt` | ⚠️ 48 rust+188 txt | 0.3 |
| D5b-2 | 1 | Lead Prompt builder | `cora-cowork-team/src/prompts/lead.rs` | 30 | 0.5 |
| D5c | 1 | Teammate Prompt + wake payload | `cora-cowork-team/src/prompts/teammate.rs` | ⚠️ 150 | 1.0 |
| D6 | 1 | mcp-bridge subcommand | `cora-cowork-app/src/bridge.rs` | 180 | 1.5 |
| **D7a** | 2 | **TeamSession 三个新方法（compute/spec/finish）** | `cora-cowork-team/src/session.rs` | 150 | 1.5 |
| **D7b** | 2 | **send 路径接 wake + `files` 附件 + log-not-throw**（P0#45/#46） | `cora-cowork-team/src/session.rs` + `cora-cowork-api-types/src/team.rs` + `routes.rs` | 120 | 1.2 |
| **D7c** | 2 | **`send_message_to_agent(silent=true)` 占位** | `cora-cowork-team/src/session.rs` | 40 | 0.3 |
| D8 | 2 | Scheduler 首次 wake | `cora-cowork-team/src/scheduler.rs` | 120 | 1.5 |
| D9 | 2 | ensure_session 闭环 | `cora-cowork-team/src/service.rs` | ⚠️ 200 | 2.0 |
| D10 | 2 | acp_agent 注入 | `cora-cowork-ai-agent/src/acp_agent.rs` | 60 | 1.0 |
| D11 | 2 | app 装配 + smoke test | `cora-cowork-app/*` | 180 | 2.0 |
| **D11.5** | 2 | **`remove_team` 级联 kill agent 进程**（P0#47 补漏） | `cora-cowork-team/src/service.rs` | 40 | 0.3 |
...
| **W5-D26b-1** | 5 | **`cora_create_team` args 解析 + 默认值（纯函数）** | team/guide/handlers.rs | 70 | 0.6 |
| **W5-D26b-2** | 5 | **`handle_cora_create_team` 调 service + 返回结构化** | team/guide/handlers.rs | 70 | 0.7 |
| W5-D26c | 5 | `handle_cora_list_models` handler | team/guide/handlers.rs | 40 | 0.3 |
...
2. **禁 mock 逃课**：D4 的 descriptor 文本要对比 team-prompts.md 原文；D5 的 builder 要对比 CoraCowork 源码拷贝；D9/D11 的集成测试用 `MockWorkerTaskManager`（**合法 mock**，只为隔离真 ACP 进程）但必须 assert 调用顺序和参数
