...
>
> **事实来源**：[backend-audit.md §3.4](./backend-audit.md#34-p0-gap-连锁关系修正版) 的 P0 连锁图 · [cora-cowork-audit.md §2.1](./cora-cowork-audit.md#21-能力清单)
...
2. D3 在 `crates/cora-cowork-team/src/mcp/bridge.rs` PR 里附一段注释：`// SDK McpServer variant shape (as of agent-client-protocol-schema 0.12.0): ...`
...
**产出物清单**（10 个模块）：
- ⬜ D1 `cora-cowork-api-types::team_mcp` 合并 + 单元测试绿
...
- ⬜ D5a/b-1/b-2/c 四个 prompt 子模块合并；快照测试绿；模板文本来自 CoraCowork 源码（非大模型生成）
- ⬜ D6 `cora-cowork-backend mcp-bridge` 子命令合并；独立集成测试通过（spawn 子进程 + mock TCP 收到 `auth_token`）
...
**不通过的典型信号**：
- 任何 descriptor 或 prompt 常量"改写成了更清晰的版本" → 驳回，按 CoraCowork 原文重改（[cora-cowork-audit §8 #5](./cora-cowork-audit.md#8-源码中发现的硬约束agent-行为易坏点)）
...
- ⬜ D11 `build_team_state` 签名扩展合并 + `cora-cowork-backend mcp-bridge` 在缺 env 时 1s 内退出且 exit code 非零
- ⬜ **D11.5 `remove_team` 级联 kill agent 进程合并 + 2 条集成测试绿**（P0#47 补漏，MockWorkerTaskManager kill 被调 N 次）

**验收证据**：
1. `cargo test --workspace` 全绿
2. `cargo clippy --workspace -- -D warnings` 无新增 warning
3. `cargo build --release` 成功产出 `cora-cowork-backend` 二进制
4. 手工执行：`./target/release/cora-cowork-backend mcp-bridge` 在无 env 时报错退出（证明 bridge 入口连通）
...
**产出物清单**：
- ⬜ D11 smoke test `crates/cora-cowork-app/tests/team_phase1_smoke.rs` 合并
...
2. `sqlite3 cora-cowork.db "SELECT extra FROM conversations WHERE id IN (...)"` 能看到 `team_mcp_stdio_config` 字段
3. WS 事件序列抓取成文本日志（`wscat` 或后端自己的 `/api/ws-token` + client）
4. MCP server TCP log（开 `RUST_LOG=cora-cowork_team::mcp=debug` 跑）能看到至少一次 `tools/call team_task_create`
...
4. `conversation.send_message` 发给 team 成员 conversation → 能看到 `RUST_LOG=cora-cowork_conversation=debug` 里 `routing to team path` 的日志

**关键对齐点**：
- W3-D16 的 trait `ITeamMessageRouter` 放 `cora-cowork-conversation` 或中间层，**禁止** `cora-cowork-conversation` 反向依赖 `cora-cowork-team`
- W3-D14 的 `renamed_agents` map 要能被 W2 D5 的 prompt builder 读到；若 D5 在 W2 阶段未预留参数，M5 里回头补 builder 参数（按 [interface-contracts.md §5](./interface-contracts.md#5-cora-cowork-teamprompts-大幅扩写wave-1--模块-d5) 本来就有）

**不通过的典型信号**：
- W3-D12 的实现把越权访问暴露成 `Forbidden` 而不是 `NotFound` → 驳回（信息泄漏）
- W3-D16 的 trait 误放 `cora-cowork-team` 导致 `cora-cowork-conversation → cora-cowork-team` 循环依赖 → 驳回
...
- ⬜ **可靠性 smoke test** `crates/cora-cowork-app/tests/team_reliability_smoke.rs` 通过
...
2. `RUST_LOG=cora-cowork_team=debug` 运行日志显示所有 8 个 phase 的触发点
3. `cargo test --workspace` 全绿

**关键对齐点**：
- **W4-D25 必须最先合并**（D18/D20/D21/D22 都订阅它）
- W4-D18 的 `release_wake_lock` 必须在"消息发出成功"时立即调用，不等 finish（cora-cowork-audit §8 #2）
- W4-D19 的 `clear_finalized_turn` 必须在 re-wake 前调用（cora-cowork-audit §8 #3）
- W4-D24 的 timeout 必须 graceful resolve，不能 reject（cora-cowork-audit §8 #11）

**不通过的典型信号**：
- 任何模块绕过 W4-D25 自己订阅 ACP stream → 驳回（违反 DRY）
- W4-D18 的 wake lock 在 finish 事件之后才释放 → 死锁风险，驳回
- W4-D20 的 leader crash 逻辑走了 remove 路径而不是只 failed → 驳回（cora-cowork-audit §2.1）
...
- ⬜ **W5-D26b-1 `cora_create_team` args 解析 + 默认值（纯函数）合并 + 4 条测试绿**
- ⬜ **W5-D26b-2 `handle_cora_create_team` 调 service + 返回结构化合并 + 3 条测试绿**
- ⬜ W5-D26c `handle_cora_list_models` handler 合并 + 1 条测试绿
...
- ⬜ **全量 e2e smoke test** `crates/cora-cowork-app/tests/team_full_e2e_smoke.rs` 通过
...
2. agent 基于 Guide prompt 调 cora_list_models → cora_create_team
...
场景 D：互斥验证
16. 进 team 的 agent 构造新 session 时 → mcp_servers 中 ✕ 没有 cora_* 工具
17. 断言：agent instructions 中 ✕ 没有 Team Guide prompt
```

**验收证据**：
1. 上述 3 + 互斥 4 个场景全部实跑日志（含 WS 事件流 + MCP server log）
2. `sqlite3 cora-cowork.db` 查场景 A 后 conversations 表确认复用：单聊 conversation.extra.team_id 被写入且 conversation 未删除
...
- 场景 D 是硬约束（cora-cowork-audit §8 #17），失败直接影响产品语义
...
- 场景 D 进 team 的 agent 能调 cora_create_team → W5-D28 互斥 guard 失效
...
| D5 的 CoraCowork prompt 文本在移植时有 UTF-8/换行问题 | M1 阶段 | 改用 `include_str!("prompt_templates/lead.txt")` 把 CoraCowork 原文件逐字节拷进来，再 diff |
...
| Team Guide MCP 单例（cora_create_team / cora_list_models） | Wave 5 | W5-D26 / W5-D27 / W5-D28 |
