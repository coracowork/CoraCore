...
> - [cora-cowork-audit.md](./cora-cowork-audit.md) §2.1 / §3.1 / §4 / §7
...
| 类型 | 目标 |
|------|------|
| 新增 | `cora-cowork-api-types::team_mcp` 子模块（提升 `TeamMcpStdioConfig`） |
| 新增 | `cora-cowork-team::mcp::bridge` 新 struct `TeamMcpStdioServerSpec`（session/new 注入体） |
| 新增 | `cora-cowork-team::prompts` 4 份常量字符串（leader/teammate/guide/spawn tool desc）+ builder 新签名 |
| 新增 | `cora-cowork-team::mcp::tools` 两个工具 descriptor + handler：`team_list_models` / `team_describe_assistant` |
| 新增 | `cora-cowork-team::session::TeamSession::stdio_spec(slot_id)` 返回 `TeamMcpStdioServerSpec` |
| 新增 | `cora-cowork-team::session::TeamSession::on_agent_finish(conversation_id, is_error)` 供 Wave 2 转发 Finish |
| 新增 | `cora-cowork-app` 子命令 `mcp-bridge`（stdio bridge 入口，无新 trait） |
| 修改 | `cora-cowork-ai-agent::AcpBuildExtra` 加字段 `team_mcp_stdio_config: Option<TeamMcpStdioConfig>` |
| 修改 | `cora-cowork-ai-agent::acp_agent::session_new_and_prompt` 按 config 注入 mcp_servers |
| 修改 | `cora-cowork-team::service::TeamSessionService::new` 加 `task_manager: Arc<dyn IWorkerTaskManager>` 入参 |
| 修改 | `cora-cowork-team::service::ensure_session` 实现 kill+rebuild 闭环（写回 extra → kill → get_or_build_task） |
| 修改 | `cora-cowork-team::session::TeamSession::send_message / send_message_to_agent` 接 wake 路径 |
| 修改 | `cora-cowork-conversation::service::ConversationService::update_extra(conv_id, patch)` 【新增接口】供 `ensure_session` 写 `team_mcp_stdio_config`（extra 是 JSON 字符串列，不需 schema 迁移） |
| 修改 | `cora-cowork-app::state_builders::build_team_state` 传 worker_task_manager |

以下按模块给出签名。**字段命名对齐 cora-cowork-backend Rust 规则（snake_case）；对外 JSON 字段按 backend-audit §1.10 的事实保持 snake_case（rebase 后已经全面去 `rename_all=camelCase`，见 commit `dae96f8`）。**

---

## 1. `cora-cowork-api-types` 新增类型（Wave 1 · 模块 D1）

**文件**：新增 `crates/cora-cowork-api-types/src/team_mcp.rs`；在 `lib.rs` `pub mod team_mcp; pub use team_mcp::*;`

```rust
use serde::{Deserialize, Serialize};

/// team session MCP server 的 stdio 连接三元组
/// (提升自 cora-cowork-team::mcp::bridge，供 cora-cowork-ai-agent 反序列化 AcpBuildExtra 用)
...
impl TeamMcpStdioConfig {
    /// stdio bridge 读到的 env 名（固定常量，不可改；cora-cowork-audit §3.1 列出）
...
**废弃**：`cora-cowork-team::mcp::bridge::TeamMcpStdioConfig` 原定义改成 `pub use cora_cowork_api_types::TeamMcpStdioConfig;`（保持 import path 不断）。

**不引入依赖**：`cora-cowork-api-types` 继续禁 axum/tower（AGENTS.md 硬规则）。

---

## 2. `cora-cowork-ai-agent::types::AcpBuildExtra` 扩展（Wave 1 · 模块 D2）

**文件**：`crates/cora-cowork-ai-agent/src/types.rs`
...
- **字段命名规则**：新增字段统一 **snake_case**（本字段 `team_mcp_stdio_config` 即如此）；既有 `teamId` 字段（[`service.rs:73`](../../../crates/cora-cowork-team/src/service.rs) 早期写入）**留作历史兼容**不动，但不再新增驼峰字段。conversation.extra 的 JSON 由后端自己写自己读，snake_case 自洽即可（commit `dae96f8` 方向）

---

## 3. `cora-cowork-team::mcp::bridge` 新增 ServerSpec（Wave 1 · 模块 D3）

**文件**：`crates/cora-cowork-team/src/mcp/bridge.rs`
...
pub struct TeamMcpStdioServerSpec {
    pub name: String,          // 固定 "cora-cowork-team-<team_id>"
    pub command: String,       // cora-cowork-backend 的绝对路径，由调用方传入（见 §7）
...
## 4. `cora-cowork-team::mcp::tools` 新增两个工具 descriptor（Wave 1 · 模块 D4）

**文件**：`crates/cora-cowork-team/src/mcp/tools.rs`
...
// （cora-cowork-audit §8 硬约束：prompt/tool description 原样复用，禁改写）
...
```rust
// phase1 最小实现：返回固定 backend 列表（claude / codex / gemini / corars）
...
**`all_tool_descriptors()`** 扩展为返回 **10 条**，顺序与 cora-cowork-audit §3.2 表格一致。

---

## 5. `cora-cowork-team::prompts` 大幅扩写（Wave 1 · 模块 D5）

**文件**：`crates/cora-cowork-team/src/prompts.rs`（现有 3 个 builder 重写）

**新增常量**（原样复用 CoraCowork 英文，**不翻译不改写**，见 team-prompts.md §5 / §3 / §4）：
...
pub const TEAM_GUIDE_PROMPT_TEMPLATE: &str = r#"..."#; // 108 行 CoraCowork 原文
pub const LEAD_PROMPT_TEMPLATE: &str = r#"..."#;        // 188 行 CoraCowork 原文
pub const TEAMMATE_PROMPT_TEMPLATE: &str = r#"..."#;    // 114 行 CoraCowork 原文
...
**硬约束**（cora-cowork-audit §8 #5、team-prompts.md §5）：模板常量一旦定义，Wave 2 只能填 param，**不得改模板文本**。

---

## 6. `cora-cowork-team::session::TeamSession` 新方法（Wave 2 · 模块 D7）

**文件**：`crates/cora-cowork-team/src/session.rs`
...
## 6.5 `cora-cowork-team::scheduler::TeammateManager` 签名扩展（Wave 2 · 模块 D8）

**文件**：`crates/cora-cowork-team/src/scheduler.rs`

D8 负责把 scheduler 的已有方法**接上生产路径**。以下签名 CoraCowork 参考实现已全部实现（`TeammateManager.ts`），后端代码里方法存在但行为不完整。
...
impl TeammateManager {
    ...
    /// （CoraCowork TeammateManager.ts:440-452 的判定逻辑）
    ...
    /// （CoraCowork TeammateManager.ts:94-100 的 activeWakes Map）
...
fn is_settled(status: &TeammateStatus) -> bool {
    matches!(status,
        ...
        | TeammateStatus::Error      // 对应 CoraCowork 的 Failed
...
## 7. `cora-cowork-ai-agent::acp_agent::session_new_and_prompt` 注入（Wave 2 · 模块 D10）

**文件**：`crates/cora-cowork-ai-agent/src/acp_agent.rs:448-458`
...
if let Some(cfg) = &self.config.team_mcp_stdio_config {
    let spec = TeamMcpStdioServerSpec::from_config(
        /* team_id 从 cfg 不直接含，改从 env 或 cfg.slot_id 的前缀取；phase1 简化：
           name 直接写 "cora-cowork-team" 足以区分 */
        "",
...
## 8. `cora-cowork-app` 新增子命令 `mcp-bridge`（Wave 1 · 模块 D6）

**文件**：`crates/cora-cowork-app/src/lib.rs`（或新子模块 `bridge.rs`）

**CLI 入口**：
```rust
// main.rs
if args.get(1).map(|s| s.as_str()) == Some("mcp-bridge") {
    cora_cowork_app::bridge::run_mcp_bridge().await;
    return;
}
```
...
/// 3. TCP 连 127.0.0.1:<port>：cora_cowork_team::mcp::protocol 的 4 字节长度帧 + JSON-RPC
...
**phase1 范围**：mcp_ready 握手"简化"为"tcp 连接建立成功即认为 ready"，不强制 phase1 做完整握手 —— CoraCowork 侧 waitForMcpReady 超时 graceful resolve（cora-cowork-audit §8 #11），所以后端 phase1 先不接 server 端等待，后续 P1 再补。

---

## 9. `cora-cowork-team::service::TeamSessionService` 签名扩展（Wave 2 · 模块 D9）

**文件**：`crates/cora-cowork-team/src/service.rs`
...
## 10. `cora-cowork-app::state_builders::build_team_state` 扩展（Wave 2 · 模块 D11）

**文件**：`crates/cora-cowork-app/src/state_builders.rs:304`
...
`backend_binary_path` 在 `cora_cowork_app::lib::build_router` 一次性 `Arc::new(std::env::current_exe()?)` 后 clone 到各子 builder。

---

## 11. ConversationService 不改（关键决策）

**phase1 决定不改 `cora-cowork-conversation::service::build_task_options`**。
...
```rust
// crates/cora-cowork-db/src/repository/team.rs
...
**Routes 改动**：`crates/cora-cowork-team/src/routes.rs` 所有 handler 从 `Extension<AuthUser>` 取 `user_id` 传给 service。
...
**文件**：`crates/cora-cowork-api-types/src/team.rs`
...
**新 trait**：放 `cora-cowork-conversation` 里（避免 `cora-cowork-conversation` 反向依赖 `cora-cowork-team`）：

```rust
// crates/cora-cowork-conversation/src/service.rs（或新 traits.rs）
#[async_trait]
...
**文件**：`crates/cora-cowork-common/src/lib.rs`
...
**文件**：`crates/cora-cowork-team/src/mcp/protocol.rs`

```rust
// 原：const MAX_MCP_MESSAGE_SIZE: usize = 10 * 1024 * 1024;
// 改：
pub const MAX_MCP_MESSAGE_SIZE: usize = cora_cowork_common::TEAM_MCP_MAX_FRAME_BYTES;
```

**文件**：`crates/cora-cowork-team/src/mcp/server.rs`

```rust
// dispatch_tool 外层：
let result = tokio::time::timeout(
    std::time::Duration::from_millis(cora_cowork_common::TEAM_MCP_REQUEST_TIMEOUT_MS),
    dispatch_tool(...),
).await;
...
**文件**：`crates/cora-cowork-ai-agent/src/task_manager.rs`
...
**文件**：`crates/cora-cowork-ai-agent/src/acp_agent.rs`
...
**broadcast channel 大小**：256 足够 agent stream 高峰（CoraCowork 参考按 in-process 事件 bus 无上限，后端用 broadcast 限定避免 OOM）。
...
impl TeammateManager {
    ...
    /// 消息发出成功后立即调用（不等 finish）—— cora-cowork-audit §8 #2
...
impl TeammateManager {
    ...
    /// cora-cowork-audit §8 #3：不清这个 dedup 会吞掉新 turn 的 finish
...
impl TeamSession {
    pub async fn on_agent_finish(
        ...
        if let AgentStreamChunk::Error { message } = &chunk {
            if cora_cowork_common::RATE_LIMIT_REGEX.is_match(message) {
...
**文件**：`crates/cora-cowork-app/src/bridge.rs`（D6 已实现 bridge 主逻辑；W4-D24 在 TCP connect 成功 + initialize ok 后追加）
...
**文件**：`crates/cora-cowork-team/src/guide/server.rs`（新增）
...
impl GuideStdioConfig {
    pub const ENV_PORT: &'static str = "CORA_MCP_PORT";
    pub const ENV_TOKEN: &'static str = "CORA_MCP_TOKEN";
    pub const ENV_BACKEND: &'static str = "CORA_MCP_BACKEND";
    pub const ENV_CONVERSATION_ID: &'static str = "CORA_MCP_CONVERSATION_ID";
}
```

**文件**：`crates/cora-cowork-team/src/guide/handlers.rs`

```rust
pub async fn handle_cora_create_team(
...
pub async fn handle_cora_list_models(
    args: &serde_json::Value,
) -> Result<ToolResult, AppError> {
...
**用户常量**：`const MCP_SPAWN_USER_ID: &str = "system_default_user";`（cora-cowork-audit §8 #15，后端 multi-tenant 时替换）。
**MCP spawn 默认值**：`workspace_mode = "shared"`, `session_mode = "yolo"`（cora-cowork-audit §8 #16）。

---

## 28. Guide stdio bridge 分支（Wave 5 · 模块 W5-D27）

**文件**：`crates/cora-cowork-app/src/bridge.rs`（在 D6 已有的 `run_mcp_bridge()` 内部分叉）

```rust
pub async fn run_mcp_bridge() -> ! {
    if std::env::var(cora_cowork_team::guide::GuideStdioConfig::ENV_BACKEND).is_ok() {
...
async fn run_guide_bridge() -> ! {
    ...
    // （Guide server 用这两个字段做业务判断，比如 cora_create_team 的 caller 复用）
}
```
...
**文件**：`crates/cora-cowork-team/src/guide/capability.rs`（新增）

```rust
const TEAM_CAPABLE_BACKENDS: &[&str] = &["claude", "codex", "gemini", "corars"];
...
**文件**：`crates/cora-cowork-ai-agent/src/acp_agent.rs`（instructions 构造点）
...
```rust
impl TeamSession {
    /// MCP spawn 闭环（CoraCowork TeamSessionService.ts:763-787 等价）
...
**错误回滚**：任一步失败需回滚前面的副作用（agent remove + conversation delete + task kill）；phase1 最小实现：若步 9/10 失败只 log + set_status(Failed) 不回滚 agents 数组（CoraCowork 参考实现也不回滚）。
...
fn handle_send_message(
    ...
    // shutdown 协议拦截（cora-cowork-audit §2.1 shutdown）
...
impl TeammateManager {
    ...
    /// Wave 5 改造：target role 校验（cora-cowork-audit §2.1 "Leader 不可 shutdown"）
