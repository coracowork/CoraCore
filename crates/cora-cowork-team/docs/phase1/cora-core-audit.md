# CoraCowork Team 模块后端能力审计（Phase 1）

> **目的**：对照 CoraCowork 前端 main 分支（`/Volumes/Macintosh HD/Users/zhuqingyu/project/CoraCowork/`，commit `ed8a6bcd3`），列出 cora-cowork-backend 后端需要复刻的所有 team 能力。
>
> **范围约束**：
> - 只关注后端需要实现的能力，不关注前端渲染（例如 React 页面、SWR 缓存、ipcBridge 事件消费）。
> - 只考虑 ACP agent type（范围内：claude / codex / corars / 其他 ACP MCP stdio-capable backend；Gemini 非 ACP 但也出现在 team-capable 白名单里，本文档按原样记录不剔除）。
...
>
> **CoraCowork 源码路径索引**（下文中所有路径均相对前端根目录 `/Volumes/Macintosh HD/Users/zhuqingyu/project/CoraCowork/`）：
>
> | 模块 | 路径 | 行数 |
> |------|------|------|
...
| 创建 team（MCP spawn） | `TeamGuideMcpServer.handleCreateTeam(args, backend, callerConversationId)`：1) 从 `CORA_COWORK_MCP_BACKEND` env 拿 agent type 2) 校验 team-capable 3) 复用 callerConversationId 作为 leader 4) 调 `teamSessionService.createTeam` 5) emit `team.listChanged` + `conversation.listChanged` + `deepLink.received` 6) 异步 `getOrStartSession` 并向 leader 发 summary message | `TeamGuideMcpServer.ts:165-261` |
...
solo agent (claude/codex)
   │ cora_create_team(summary, [name], [workspace])
   ▼
[team-guide-mcp-stdio.js]
   │ TCP → { tool:"cora_create_team", args, backend, conversation_id, auth_token }
   ▼
...
| Team Guide 工具集 | 2 个：`cora_create_team`、`cora_list_models`（共享 `handleListModels`） | `TeamGuideMcpServer.ts:155-163` |
| Team Guide 注入时机 | ACP agent `loadBuiltinSessionMcpServers()` 里：**不在 team 里** 且 `shouldInjectTeamGuideMcp(backend)` 通过时，取 `getTeamGuideStdioConfig()`，追加 env `CORA_COWORK_MCP_BACKEND` + `CORA_COWORK_MCP_CONVERSATION_ID` 然后 `buildTeamMcpServer` 包装注入 | `acp/index.ts:1623-1640` |
| Gemini / Corars 路径 | Gemini 有独立 `GeminiAgentManager.getMcpServers`；Corars 有 `CorarsManager` 额外字段 `awaitReady:true` + `notifyMcpReady(slotId)` | `GeminiAgentManager.ts:341-391`, `CorarsManager.ts:149-201` |
| Team Guide 能力判断 | `shouldInjectTeamGuideMcp(backend)` → `isTeamCapableBackend(backend, cachedInitResults)`：硬编码白名单 `{gemini, claude, codex, corars}`，其他 backend 靠 `cachedInitResults.capabilities.mcpCapabilities.stdio===true` | `teamGuideCapability.ts:18-21`, `teamTypes.ts:16-30` |
| Team Guide Prompt 注入 | ACP / Gemini / Corars 的 `AgentManager` 在构建 system instructions 时检查 `!isInTeam && shouldInjectTeamGuideMcp(backend)` → `getTeamGuidePrompt({ backend, leaderLabel })` 拼到 `instructions` 数组 | `AcpAgentManager.ts:1024-1052`, `GeminiAgentManager.ts:248-254`, `task/agentUtils.ts:61/165/219` |
...
| `StdioMcpConfig` 形状 | `{ name: 'cora-cowork-team-<teamId>', command: 'node', args: [scriptPath], env: [{name:'TEAM_MCP_PORT',value},{name:'TEAM_MCP_TOKEN',value},{name:'TEAM_AGENT_SLOT_ID',value}] }` | `TeamMcpServer.ts:44-49, 101-118` |
...
│  env: CORA_COWORK_MCP_PORT/TOKEN       │  env: TEAM_MCP_PORT/TOKEN
│       CORA_COWORK_MCP_BACKEND          │       TEAM_AGENT_SLOT_ID
│       CORA_COWORK_MCP_CONVERSATION_ID  │
└───────▲────────┘                └──────▲────────┘
...
注意：同一个 agent 进入 team 后，**Team Guide MCP 不再注入**（`!this.extra.teamMcpStdioConfig` 才注入），避免 team 成员把 `cora_create_team` 当工具再次建团。
...
- `cora_create_team` description：**动态**由 `getCreateTeamToolDescription()` 返回（`teamGuidePrompt.ts:88-108`），被 stdio bridge 在 import 时引用。
- `cora_list_models` description：**静态**写在 `teamGuideMcpStdio.ts:109-112` inline。
...
> CoraCowork 走 ipcBridge；后端要包掉这些 team 逻辑意味着提供等价 HTTP/WebSocket 端点。具体 endpoint 命名/路径由后端决定，此处只列能力。
...
| 创建 team（agent 调 MCP） | `cora_create_team(summary, name?, workspace?)` + system-injected `backend` + `conversation_id` | 复用 caller conversation 作 leader；`sessionMode='yolo'`, `workspaceMode='shared'`；返回 `{teamId, route, leadAgent, next_step}` |
| 列 team | `userId` | `TTeam[]` |
...
1. `cora_create_team(summary, name?, workspace?)` — `system.backend`、`system.conversation_id` 由 bridge 注入
2. `cora_list_models(agent_type?)`
...
- `getCreateTeamToolDescription()` — `cora_create_team` 工具描述（3 PRECONDITIONS + STRICT 流程）
...
- `shouldInjectTeamGuideMcp(backend)` — 复用 `isTeamCapableBackend` 白名单 `{gemini, claude, codex, corars}` + `cachedInitResults.capabilities.mcpCapabilities.stdio`
...
CoraCowork `ipcBridge.team.*` 集合（前端消费）：
...
这些是 CoraCowork 代码里写死的、前后端分离必须保留的行为契约：
...
- CoraCowork 没有显式 REST 端点做 "cora_create_team"——它完全依赖 MCP + ipcBridge。后端要暴露 REST 需要后端自己设计。
...
- CoraCowork 源码 commit：`ed8a6bcd3 fix(bundled-bun): add baseline variant for linux-x64 to support non-AVX2 CPUs (#2654)`（main 分支，2026-04-29 拉取）
- cora-cowork-backend 所在 worktree：`/Users/zhuqingyu/.superset/worktrees/cora-cowork-backend/repeated-algebra`，分支 `docs/api-for-frontend`
