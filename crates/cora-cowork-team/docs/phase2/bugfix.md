...
**双保险：**
1. `crates/cora-cowork-team/src/guide/server.rs:156` — send_message 追加 `[SYSTEM NOTE]` 明确指示直接 spawn
2. `crates/cora-cowork-team/src/prompts/prompt_templates/lead.txt` — 规则 9/10/88 增加 SYSTEM NOTE 例外
...
- `crates/cora-cowork-team/src/service.rs` — create_team 成功后 broadcast `team.created` 事件
- 同时在 adopt 旧 conversation 时广播 `conversation.listChanged(updated)` 让前端移除旧对话

### 前端待做

前端需监听 `team.created` + `conversation.listChanged` 实现自动跳转。已提 issue: https://github.com/coracowork/cora-cowork-desktop/issues/2734
...
- `crates/cora-cowork-team/src/service.rs` — `rebuild_agent_processes` 的 patch 加 `session_mode`
- `crates/cora-cowork-team/src/session.rs` — `attach_spawned_agent_process_bg` 的 patch 加 `session_mode`
...
`crates/cora-cowork-team/src/mcp/server.rs` — `exec_shutdown_agent` 写入 mailbox 后调用 `svc.wake_agent_in_session(team_id, &target_slot_id)` 唤醒目标。
...
`crates/cora-cowork-team/src/service.rs` — 新增 `ensure_session_locks: DashMap<String, Mutex>` 字段，`ensure_session` 加 per-team mutex + double-check locking。
...
`crates/cora-cowork-app/src/guide_stdio.rs` — `forward_tool` 加最多 3 次重试（间隔 500ms/1s/1.5s）。
...
```
crates/cora-cowork-team/src/service.rs          — wake_lock 修复、StreamRelay、ensure_session 锁、team.created 事件、conversation.listChanged、session_mode
crates/cora-cowork-team/src/session.rs          — try_wake 加 StreamRelay、移除 leader mirror 跳过、session_mode、msg_id 去重
crates/cora-cowork-team/src/guide/server.rs     — send_message 加 [SYSTEM NOTE]
crates/cora-cowork-team/src/prompts/prompt_templates/lead.txt — SYSTEM NOTE 例外规则
crates/cora-cowork-team/src/mcp/server.rs       — shutdown_agent 加 wake
crates/cora-cowork-team/docs/phase2/bugfix.md   — 本文档
crates/cora-cowork-conversation/src/service.rs  — pub fn repo() accessor
crates/cora-cowork-app/src/guide_stdio.rs       — forward_tool 重试逻辑
```
...
1. build release 后必须执行 `pkill -f cora-cowork-backend` 杀掉旧进程，再重启前端
2. 当前 release binary 已包含所有修复：`/Users/zhuqingyu/project/cora-cowork-backend/target/release/cora-cowork-backend`
...
- Backend binary: `/Users/zhuqingyu/project/cora-cowork-backend/target/release/cora-cowork-backend`
- Symlink: `~/.local/bin/cora-cowork-backend` → 上述 binary
- Frontend: `/Users/zhuqingyu/project/CoraCowork` branch `feat/backend-migration`
- Backend fix branch: `fix/team-communication-bugs`
- Log file: `/Users/zhuqingyu/Library/Logs/Cora-Cowork-Dev/2026-05-02.backend.log`
- Frontend issue: https://github.com/coracowork/cora-cowork-desktop/issues/2734
