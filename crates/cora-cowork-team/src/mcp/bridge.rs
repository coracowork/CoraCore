...
pub use cora_cowork_api_types::{TEAM_MCP_SERVER_NAME, TeamMcpStdioConfig};

/// Stdio MCP server description ready to be handed to `session/new`.
///
/// Field shapes:
/// - `name` = `"cora-cowork-team"` (fixed; team routing is done via `port` + `token`,
...
impl TeamMcpStdioServerSpec {
    ...
    ///
    /// `backend_binary_path` is the absolute path to the `coracore`
...
mod tests {
    ...
    fn sample_cfg() -> TeamMcpStdioConfig {
        TeamMcpStdioConfig {
            ...
            token: "tok-abc".into(),
            slot_id: "slot-1".into(),
            binary_path: "/usr/bin/coracore".into(),
        }
    }

    #[test]
    fn from_config_fills_all_fields() {
        let spec = TeamMcpStdioServerSpec::from_config("/usr/bin/coracore", &sample_cfg());

        assert_eq!(spec.name, TEAM_MCP_SERVER_NAME);
        assert_eq!(spec.command, "/usr/bin/coracore");
    ...
    fn into_sdk_serializes_as_stdio_variant() {
        let spec = TeamMcpStdioServerSpec::from_config("/bin/coracore", &sample_cfg());
        ...
        assert_eq!(json["command"], "/bin/coracore");
