...
///
/// Injected by `cora-cowork-app` — wraps `cora_cowork_auth::extract_token_from_ws_headers`
/// so that `cora-cowork-realtime` does not depend on `cora-cowork-auth` directly.
