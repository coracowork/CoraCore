...
use cora_cowork_file::{FileService, IFileService};
...
async fn list_workspace_files_skips_directory_symlinks() {
    ...
    std::os::unix::fs::symlink(&skill_dir, workspace_skills_dir.join("cora-cowork-skills")).unwrap();
    ...
    assert!(
        files.iter().all(|file| file.name != "cora-cowork-skills"),
