...
use cora_cowork_common::constants::UPLOAD_MAX_SIZE;
...
pub fn file_routes(state: FileRouterState) -> Router {
    ...
    // We first disable the global `DefaultBodyLimit` that `cora-cowork-app`
...
fn to_zip_entry(e: cora_cowork_api_types::ZipFileEntry) -> ZipEntry {
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let mode = match info.mode {
        SnapshotMode::GitRepo => cora_cowork_api_types::SnapshotMode::GitRepo,
        SnapshotMode::Snapshot => cora_cowork_api_types::SnapshotMode::Snapshot,
    };
...
mod tests {
    ...
    fn zip_entry_conversion_text() {
        let e = cora_cowork_api_types::ZipFileEntry {
            name: "a.txt".into(),
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, cora_cowork_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, cora_cowork_api_types::SnapshotMode::Snapshot);
        assert!(r.branch.is_none());
    }

    #[test]
    fn compare_response_conversion() {
        use cora_cowork_common::FileChangeOperation;
