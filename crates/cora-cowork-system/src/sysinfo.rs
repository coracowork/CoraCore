use cora_cowork_api_types::SystemInfoResponse;
...
/// Resolve the cache directory for CoraCowork.
///
/// Priority: `CORA_COWORK_CACHE_DIR` env → `dirs::cache_dir()/cora-cowork`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("CORA_COWORK_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("cora-cowork").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for CoraCowork.
///
/// Priority: `CORA_COWORK_WORK_DIR` env → `dirs::data_dir()/cora-cowork`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("CORA_COWORK_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
/// Resolve the log directory for CoraCowork.
///
/// Priority: `CORA_COWORK_LOG_DIR` env →
///   macOS: `~/Library/Logs/cora-cowork`
///   Linux: `dirs::state_dir()/cora-cowork/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/cora-cowork/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("CORA_COWORK_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/cora-cowork").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("cora-cowork/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/cora-cowork/logs
    dirs::data_dir()
        .map(|p| p.join("cora-cowork/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
