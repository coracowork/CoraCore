...
//! backends. It lives in `cora-cowork-common` (below every consumer: `cora-cowork-session`,
//! `cora-cowork-conversation`, the agent-* crates all depend on common) so the
//! consuming backends gain no upward crate dependency. Ported verbatim from the
//! deleted `cora-cowork-ai-agent::manager::acp::stderr_error_extractor` (git
