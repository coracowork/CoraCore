...
impl AgentType {
    ...
    pub fn supports_new_conversation(&self) -> bool {
        matches!(self, AgentType::Acp | AgentType::Corars)
    }
    ...
    pub fn native_skills_dirs(&self) -> Option<&'static [&'static str]> {
        match self {
            AgentType::Corars => Some(&[".corars/skills"]),
            AgentType::Acp
...
mod tests {
    ...
    fn test_agent_type_display_names() {
        ...
        assert_eq!(AgentType::Corars.display_name(), "Cora CLI");
    ...
    fn test_agent_type_id_stability() {
        let id = AgentType::Corars.id();
        assert_eq!(id.len(), 8);
        assert!(id.chars().all(|c| c.is_ascii_hexdigit()));
        assert_eq!(AgentType::Corars.id(), AgentType::Corars.id());
    }
    ...
    fn agent_type_new_conversation_support_policy_is_explicit() {
        ...
        assert!(AgentType::Corars.supports_new_conversation());
    ...
    fn agent_type_deprecated_runtime_policy_matches_new_conversation_support() {
        ...
        assert!(!AgentType::Corars.is_deprecated_runtime());
    ...
    fn agent_type_full_auto_mode_id_supports_non_acp_agents() {
        ...
        assert_eq!(AgentType::Corars.full_auto_mode_id(None), "yolo");
