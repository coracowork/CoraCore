...
use cora_cowork_ai_agent::{AgentError, AgentSendError, AgentStreamEvent, IMockAgent, IWorkerTaskManager};
use cora_cowork_api_types::WebSocketMessage;
use cora_cowork_channel::channel_settings::ChannelSettingsService;
use cora_cowork_channel::error::ChannelError;
use cora_cowork_channel::message_service::ChannelMessageService;
...
use cora_cowork_conversation::skill_resolver::{ResolvedAgentSkill, SkillResolver};
use cora_cowork_db::models::AssistantSessionRow;
use cora_cowork_db::models::UpsertAssistantDefinitionParams;
use cora_cowork_db::{
...
#[tokio::test]
async fn send_to_agent_without_saved_binding_defaults_to_bare_corars_assistant() {
    let db = init_database_memory().await.unwrap();
    let pool = db.pool().clone();
    ...
    let session = AssistantSessionRow {
        id: "session-assisted-default-corars".to_owned(),
