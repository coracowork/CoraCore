...
use cora_cowork_db::models::AssistantUserRow;
use cora_cowork_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
...
use cora_cowork_channel::pairing::PairingService;
use cora_cowork_channel::session::SessionManager;
use cora_cowork_channel::types::{
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn cora_cowork_db::IClientPreferenceRepository> =
        Arc::new(cora_cowork_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
async fn action_session_new() {
    ...
    match result {
        MessageResult::Action(resp) => {
            ...
            // With no client_preferences, defaults to "corars"
            assert!(text.contains("corars"));
        }
        _ => panic!("Expected Action result"),
    }
}
...
// cora-cowork-db sqlite_channel.rs::update_session_conversation_persists.
