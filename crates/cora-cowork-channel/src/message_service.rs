...
use cora_cowork_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
use cora_cowork_api_types::{AssistantConversationRequest, CreateConversationRequest, SendMessageRequest};
use cora_cowork_common::{AgentType, ConversationSource};
use cora_cowork_conversation::ConversationService;
use cora_cowork_db::models::AssistantSessionRow;
...
impl ChannelMessageService {
    ...
    async fn create_conversation_for_session(
        ...
        // Top-level `model` is only accepted for corars; other types pass via `extra`.
        let top_level_model = if agent_type == AgentType::Corars {
            Some(model)
        } else {
    ...
    pub fn process_stream_event(event: &AgentStreamEvent) -> Option<StreamAction> {
        match event {
            ...
            | AgentStreamEvent::BackendTurnBound(_) => None,
        }
    }
...
fn platform_to_source(platform: PluginType) -> ConversationSource {
    match platform {
        ...
        // Reserved variants default to CoraCowork
        PluginType::Slack | PluginType::Discord => ConversationSource::CoraCowork,
    }
}
...
mod tests {
    use super::*;
    use cora_cowork_ai_agent::protocol::events::{
    ...
    #[test]
    fn platform_to_source_reserved_defaults_to_coracore() {
        assert_eq!(platform_to_source(PluginType::Slack), ConversationSource::CoraCowork);
        assert_eq!(platform_to_source(PluginType::Discord), ConversationSource::CoraCowork);
    }
    ...
    // ── model placement by agent_type (regression: non-corars must not
    //    use top-level model) ──────────────────────────────────────────
    ...
    #[test]
    fn corars_model_stays_at_top_level() {
        let agent_type = AgentType::Corars;
        let model = ProviderWithModel {
        ...
        assert!(top_level_model.is_some(), "corars must use top-level model");
    ...
    #[test]
    fn conv_name_telegram_corars() {
        let name = channel_conversation_name(PluginType::Telegram, "corars", None, Some("70880480"));
        assert_eq!(name, "tg-corars-70880480");
    }
    ...
    fn conv_name_non_acp_ignores_backend() {
        let name = channel_conversation_name(PluginType::Telegram, "corars", Some("claude"), Some("70880480"));
        assert_eq!(name, "tg-corars-70880480");
    }
}
