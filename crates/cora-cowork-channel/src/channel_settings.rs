...
const DEFAULT_AGENT_TYPE: &str = "corars";
...
/// Non-ACP agent types (corars, nanobot, remote, …) have `backend = None`.
#[derive(Debug, Clone)]
...
impl ChannelSettingsService {
    ...
    ///
    /// Falls back to `agent_type=corars, backend=None` when no config exists.
    ...
    async fn resolve_default_assistant_identity(&self) -> Result<Option<String>, ChannelError> {
        ...
        let mut any_corars = None;
        for definition in &definitions {
            if self.effective_assistant_backend(definition, &overlays).await? == DEFAULT_AGENT_TYPE {
                any_corars = Some(definition);
                break;
            }
        }
        if let Some(definition) = any_corars {
            return Ok(Some(definition.assistant_id.clone()));
        }
    ...
    async fn effective_assistant_backend(
        &self,
        definition: &cora_cowork_db::models::AssistantDefinitionRow,
        overlays: &[cora_cowork_db::models::AssistantOverlayRow],
...
fn backend_to_agent_type(backend: &str) -> String {
    match backend {
        "corars" | "cora-cli" => "corars".to_owned(),
...
mod tests {
    ...
    use cora_cowork_db::{IAssistantDefinitionRepository, IAssistantOverlayRepository};
    use std::sync::Mutex;
    ...
    #[test]
    fn corars_backends_map_to_corars() {
        assert_eq!(backend_to_agent_type("corars"), "corars");
        assert_eq!(backend_to_agent_type("cora-cli"), "corars");
    }
    ...
    async fn agent_config_returns_default_when_no_pref() {
        ...
        assert_eq!(config.agent_type, "corars");
    ...
    #[tokio::test]
    async fn agent_config_corars_has_no_backend() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "assistant.lark.agent",
            r#"{"backend":"corars","name":"Cora CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
    ...
    #[tokio::test]
    async fn agent_config_reads_new_format_corars() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "assistant.lark.agent",
            r#"{"agent_type":"corars","name":"Cora CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
    ...
    #[tokio::test]
    async fn get_assistant_setting_defaults_to_generated_corars_assistant() {
        let repo = Arc::new(MockPrefRepo::new());
        let definition_repo = Arc::new(MockAssistantDefinitionRepo {
            rows: vec![
                make_definition("bare-claude", "claude"),
                make_definition("bare-corars", "corars"),
            ],
        });
        ...
        assert_eq!(setting.assistant_id.as_deref(), Some("bare-corars"));
        assert!(setting.custom_agent_id.is_none());
        assert!(setting.backend.is_none());
        assert!(setting.agent_type.is_none());
    ...
    #[tokio::test]
    async fn get_platform_settings_defaults_to_generated_corars_assistant() {
        let repo = Arc::new(MockPrefRepo::new());
        let definition_repo = Arc::new(MockAssistantDefinitionRepo {
            rows: vec![
                make_definition("bare-claude", "claude"),
        ...
        assert_eq!(assistant.assistant_id.as_deref(), Some("bare-corars"));
        assert!(assistant.custom_agent_id.is_none());
        assert!(assistant.backend.is_none());
        assert!(assistant.agent_type.is_none());
