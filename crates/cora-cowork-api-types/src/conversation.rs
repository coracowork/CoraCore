...
///
/// `model` is the canonical top-level field **only for `AgentType::Corars`**.
...
mod tests {
    ...
    fn deserialize_create_request_full() {
        ...
        assert_eq!(req.source, Some(ConversationSource::CoraCowork));
    ...
    fn serialize_conversation_response_snake_case() {
        let resp = ConversationResponse {
            ...
            runtime: None,
            source: Some(ConversationSource::CoraCowork),
            pinned: false,
            pinned_at: None,
        ...
        assert_eq!(json["source"], "cora-cowork");
