...
mod tests {
    ...
    fn test_system_info_response_serialization() {
        let resp = SystemInfoResponse {
            cache_dir: "/home/user/.cache/cora-cowork".into(),
            work_dir: "/home/user/.local/share/cora-cowork".into(),
            log_dir: "/home/user/.local/state/cora-cowork/logs".into(),
            platform: "linux".into(),
            arch: "x64".into(),
        };
        let json = serde_json::to_value(&resp).unwrap();
        assert_eq!(json["cache_dir"], "/home/user/.cache/cora-cowork");
        assert_eq!(json["work_dir"], "/home/user/.local/share/cora-cowork");
        assert_eq!(json["log_dir"], "/home/user/.local/state/cora-cowork/logs");
    ...
    fn test_update_check_request_with_options() {
        let raw = json!({
            ...
            "repo": "coracowork/cora-cowork-desktop"
        });
        let req: UpdateCheckRequest = serde_json::from_value(raw).unwrap();
        assert!(req.include_prerelease);
        assert_eq!(req.repo.as_deref(), Some("coracowork/cora-cowork-desktop"));
    }
    ...
    fn test_update_check_result_with_update() {
        let result = UpdateCheckResult {
            ...
            latest: Some(UpdateReleaseInfo {
                ...
                html_url: "https://github.com/coracowork/cora-cowork-desktop/releases/tag/v2.0.0".into(),
                published_at: Some("2026-04-01T00:00:00Z".into()),
                prerelease: false,
                draft: false,
                assets: vec![GitHubReleaseAsset {
                    name: "cora-cowork-2.0.0-darwin-arm64.dmg".into(),
                    url: "https://github.com/download/cora-cowork.dmg".into(),
                    size: 85_000_000,
