...
use cora_cowork_common::{AgentType, TimestampMs};
...
pub enum AgentSource {
    /// Ships with the backend binary (no CLI install required — e.g. `corars`).
    Internal,
...
/// the frontend verbatim, and typed-decoded inside `cora-cowork-ai-agent`
...
pub struct AgentMetadata {
    ...
    /// Native mode id that CoraCowork's legacy `yolo` / `yoloNoSandbox`
