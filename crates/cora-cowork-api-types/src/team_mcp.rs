...
//!
//! These are promoted from `cora-cowork-team::mcp::bridge` so that downstream
//! crates (`cora-cowork-ai-agent` deserializing `AcpBuildExtra`, etc.) can reference
//! the same shape without depending on `cora-cowork-team`.
...
pub const TEAM_MCP_SERVER_NAME: &str = "cora-cowork-team";
...
mod tests {
    ...
    fn deserialization_tolerates_unknown_fields() {
        ...
        let json = r#"{"team_id":"t-1","port":1,"token":"t","slot_id":"s","binary_path":"/usr/bin/coracore","future_field":42}"#;
        let parsed: TeamMcpStdioConfig = serde_json::from_str(json).unwrap();
