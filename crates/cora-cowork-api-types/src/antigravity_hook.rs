...
//! hook can only TIGHTEN a decision, never loosen one (verified). So CoraCowork
...
//! Every decision is made at runtime, by the user, in CoraCowork.
//!
//! Flow: agy → hook process (stdin JSON) → CoraCowork endpoint → user's permission
...
pub struct AntigravityHookConfig {
    /// Scheme + host + port of the running CoraCowork backend.
    pub base_url: String,
    pub token: String,
...
impl AntigravityHookConfig {
    /// env key the hook reads to learn CoraCowork's base URL (scheme + host +
    /// port, e.g. `http://127.0.0.1:25808`).
    pub const ENV_BASE_URL: &'static str = "CORA_COWORK_ANTIGRAVITY_HOOK_BASE_URL";
    /// env key the hook reads to learn its auth token.
    pub const ENV_TOKEN: &'static str = "CORA_COWORK_ANTIGRAVITY_HOOK_TOKEN";
    /// env key the hook reads to learn which conversation it belongs to.
    pub const ENV_CONVERSATION_ID: &'static str = "CORA_COWORK_ANTIGRAVITY_HOOK_CONVERSATION_ID";
    /// Header carrying [`Self::ENV_TOKEN`] on the callback request.
    pub const TOKEN_HEADER: &'static str = "x-cora-cowork-hook-token";
    /// Name under which the bridge registers itself in `hooks.json`.
    pub const HOOK_NAME: &'static str = "cora-cowork-permission-bridge";
}
...
mod tests {
    ...
    fn an_mcp_call_is_shown_as_server_slash_tool() {
        let raw = r#"{"conversationId":"c1","toolCall":{"name":"call_mcp_tool","args":{"ServerName":"cora-cowork-team","ToolName":"cora_cowork_team_ping"}}}"#;
        let input: AntigravityHookInput = serde_json::from_str(raw).unwrap();
        assert_eq!(input.tool_call.display_name(), "cora-cowork-team/cora_cowork_team_ping");
    }
