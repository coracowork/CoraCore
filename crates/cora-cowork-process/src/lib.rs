//! `cora-cowork-process` — self-contained subprocess mechanism (feature 001).
//!
...
//! existing `CliAgentProcess` / process registry in `cora-cowork-ai-agent`.
//!
//! "Bytes not semantics": it never parses agent output, holds no session
//! state, and never mutates `std::env`. It depends only on `cora-cowork-common`
//! and `cora-cowork-runtime`.
//!
//! ## Isolation contract (why two mechanisms coexist without conflict)
//! All shared resources are namespaced under `{data_dir}/runtime/cora-cowork-process/`
