...
use cora_cowork_process::{Containment, ContainmentKillOutcome, ProcessGroupContainment};
...
fn a_tool_child_that_left_the_group_is_still_reaped() {
    let marker = std::env::temp_dir().join(format!("cora-cowork-reap-{}.pid", std::process::id()));
...
fn an_unrelated_process_is_left_running() {
    ...
    let marker = std::env::temp_dir().join(format!("cora-cowork-bystander-{}.pid", std::process::id()));
    let marker_s = marker.to_string_lossy().to_string();
    let _ = std::fs::remove_file(&marker);
...
async fn managed_process_kill_reaps_a_child_that_left_the_group() {
    ...
    use cora_cowork_process::ManagedProcess;

    let marker = std::env::temp_dir().join(format!("cora-cowork-mpkill-{}.pid", std::process::id()));
    let marker_s = marker.to_string_lossy().to_string();
    let _ = std::fs::remove_file(&marker);
