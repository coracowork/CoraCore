...
async fn setup() -> (Router, String, String, TempDir, Database) {
    ...
    let router =
        project_routes(ProjectRouterState { project: service }).layer(axum::Extension(cora_cowork_auth::CurrentUser {
            id: "system_default_user".to_owned(),
            username: "admin".to_owned(),
            user_type: cora_cowork_db::UserType::Local,
            status: cora_cowork_db::UserStatus::Active,
        }));
