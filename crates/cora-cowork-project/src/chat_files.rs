...
//! reproduces the legacy `[[CORA_FILES]]` inlined-attachment content form so
//! every downstream consumer — corars `build_content_blocks`, the ACP prompt
...
use cora_cowork_api_types::ChatFileRef;
...
/// re-inlined into [`content`](Self::content) via the `[[CORA_FILES]]` marker.
#[derive(Debug)]
pub struct ResolvedChatMessage {
    ...
    /// corars's `build_content_blocks` strips the marker (files match) and
    /// re-adds its own; clearing this would leak the raw marker to corars.
...
impl ProjectService {
    ...
    ///
    /// `upload_root` is the managed upload directory (`temp_dir()/coracowork`);
    ...
    /// (send boundary) and the preview content endpoint (`cora-cowork-file`, cross-crate — hence `pub`).
