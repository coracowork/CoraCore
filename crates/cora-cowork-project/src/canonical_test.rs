...
fn casing_folds_per_platform() {
    let mixed = canonicalize("file:///Users/Me/Cora").unwrap();
    let lower = canonicalize("file:///users/me/cora").unwrap();
    if IGNORE_PATH_CASING {
        // macOS / Windows: same folder.
        assert_eq!(mixed, lower);
        assert_eq!(mixed.as_str(), "file:///users/me/cora");
    } else {
...
fn basename_is_final_segment() {
    let c = canonicalize("file:///Users/me/cora").unwrap();
    assert_eq!(basename(&c), "cora");
}
...
fn to_file_uri_does_not_fold_casing() {
    ...
    let uri = to_file_uri(std::path::Path::new("/Users/Me/Cora")).unwrap();
    assert_eq!(uri, "file:///Users/Me/Cora");
}
