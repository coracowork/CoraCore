...
use cora_cowork_db::{Database, IProjectStore, SqliteProjectStore, init_database_memory};
use cora_cowork_project::ProjectService;
use cora_cowork_project::canonical::{canonicalize, to_file_uri};
use cora_cowork_project::types::{AttachInput, FileOp, ReferenceInput};
...
async fn create_standard_missing_dir_is_folder_not_found() {
    let (svc, _db) = service().await;
    let missing = uri_of(std::path::Path::new("/nonexistent-cora-cowork-xyz-8f3a2b1c"));
...
async fn seed_user(db: &Database, username: &str) -> String {
    ...
    let repo = cora_cowork_db::SqliteUserRepository::new(db.pool().clone());
...
async fn a_case_variant_folder_resolves_to_one_repo_identity() {
    use cora_cowork_project::canonical::IGNORE_PATH_CASING;
