...
use cora_cowork_ai_agent::task_manager::IWorkerTaskManager;
use cora_cowork_api_types::{AssistantConversationRequest, CreateConversationRequest};
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<cora_cowork_db::models::ConversationRow>, CronError> {
    ...
    pub async fn get_assistant_snapshot(
        &self,
        conversation_id: &str,
    ) -> Result<Option<cora_cowork_db::models::ConversationAssistantSnapshotRow>, CronError> {
        self.conversation_repo
...
/// Only corars conversations carry meaningful model info in `conversations.model`;
...
mod tests {
    ...
    use cora_cowork_ai_agent::AgentStreamEvent;
    ...
    fn sample_job() -> CronJob {
        CronJob {
            ...
            agent_config: Some(CronAgentConfig {
                ...
                model: None,
                config_options: None,
                workspace: Some(ensure_named_workspace_path("cora-cowork-cron-sample-job-workspace")),
            }),
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        ...
        let repo = Arc::new(cora_cowork_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    async fn parse_agent_type_known_types() {
        ...
        assert_eq!(parse_agent_type(&registry, "corars").await.unwrap(), AgentType::Corars);
    }
    ...
    fn resolve_model_returns_none_for_acp() {
        // Model info only applies to corars; ACP ignores it.
        let job = sample_job();
    ...
    #[test]
    fn resolve_model_returns_none_for_non_corars_type() {
        let job = CronJob {
    ...
    #[test]
    fn resolve_model_corars_with_full_config() {
        let job = CronJob {
            agent_type: "corars".into(),
            agent_config: Some(CronAgentConfig {
                name: "OpenAI".into(),
                cli_path: None,
                is_preset: None,
        ...
        let model = resolve_model(&job).expect("corars + full config returns Some");
        assert_eq!(model.provider_id, "4056cdea");
        assert_eq!(model.model, "gpt-5");
    }

    #[test]
    fn resolve_model_corars_uses_model_payload() {
        let job = CronJob {
            agent_type: "corars".into(),
            agent_config: Some(CronAgentConfig {
                name: "OpenAI".into(),
        ...
        let model = resolve_model(&job).expect("corars model payload returns Some");
        assert_eq!(model.provider_id, "4056cdea");
        assert_eq!(model.model, "gpt-5");
    }

    #[test]
    fn resolve_model_corars_without_config_returns_none() {
    ...
    #[test]
    fn resolve_model_corars_without_model_returns_none() {
        let job = CronJob {
            agent_type: "corars".into(),
            agent_config: Some(CronAgentConfig {
                name: "Bogus".into(),
    ...
    async fn build_conversation_extra_preserves_agent_workspace() {
        ...
        assert_eq!(
            extra["workspace"],
            ensure_named_workspace_path("cora-cowork-cron-sample-job-workspace")
        );
    }
    ...
    async fn execute_inner_uses_conversation_workspace_when_job_workspace_missing() {
        ...
        assert_eq!(
            ...
            ensure_named_workspace_path("cora-cowork-cron-existing-conversation-workspace")
        );
    ...
    async fn execute_inner_persists_agent_workspace_when_conversation_workspace_missing() {
        ...
        assert_eq!(
            value["workspace"],
            ensure_named_workspace_path("cora-cowork-cron-agent-workspace")
        );
    }
    ...
    async fn execute_inner_inserts_right_side_user_message_for_cron_prompt() {
        ...
        let repo = Arc::new(MissingWorkspaceConversationRepo::new(
            "conv_1",
            serde_json::json!({
                "workspace": ensure_named_workspace_path("cora-cowork-cron-existing-conversation-workspace")
            }),
        ));
    ...
    fn make_executor_for_busy_tests() -> JobExecutor {
        ...
        impl IWorkerTaskManager for StubTaskManager {
            ...
            fn collect_idle(&self, _: cora_cowork_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl cora_cowork_realtime::EventBroadcaster for StubBroadcaster {
        ...
        let stub_broadcaster: Arc<dyn cora_cowork_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
        let stub_repo: Arc<dyn IConversationRepository> = Arc::new(StubConvRepo);
        let agent_metadata_repo: Arc<dyn cora_cowork_db::IAgentMetadataRepository> = Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn cora_cowork_db::IAcpSessionRepository> = Arc::new(StubAcpSessionRepo);
    ...
    impl RecordingAgent {
        fn new(conversation_id: &str, mode: &str, initialized: bool) -> Self {
            ...
            Self {
                conversation_id: conversation_id.to_owned(),
                workspace: ensure_named_workspace_path("cora-cowork-cron-agent-workspace"),
                event_tx,
    ...
    impl IAgentTask for RecordingAgent {
        ...
        async fn send_message(&self, data: SendMessageData) -> Result<(), cora_cowork_ai_agent::AgentSendError> {
    ...
    impl IMockAgent for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, cora_cowork_ai_agent::AgentError> {
        ...
        async fn set_config_option(
            ...
            option_id: &str,
            value: &str,
        ) -> Result<SetConfigOptionResponse, cora_cowork_ai_agent::AgentError> {
            if option_id != "mode" {
                return Err(cora_cowork_ai_agent::AgentError::bad_request(format!(
    ...
    impl IWorkerTaskManager for FailingTaskManager {
        ...
        async fn get_or_build_task(
            ...
            Err(cora_cowork_ai_agent::AgentError::bad_gateway(self.message.clone()))
        }
    ...
    struct MissingWorkspaceConversationRepo {
        ...
        inserted_messages: Mutex<Vec<cora_cowork_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<cora_cowork_db::models::MessageRow> {
    ...
    impl cora_cowork_realtime::EventBroadcaster for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        async fn update(&self, _id: &str, updates: &ConversationRowUpdate) -> Result<(), cora_cowork_db::DbError> {
    ...
    fn make_executor_with_task_manager_and_repo(
        ...
        let broadcaster: Arc<dyn cora_cowork_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
    ...
    #[async_trait::async_trait]
    impl cora_cowork_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<cora_cowork_db::models::AcpSessionRow>, cora_cowork_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &cora_cowork_db::CreateAcpSessionParams<'_>,
        ) -> Result<cora_cowork_db::models::AcpSessionRow, cora_cowork_db::DbError> {
            Err(cora_cowork_db::DbError::Init("stub".into()))
        }
        async fn update_session_id(
            &self,
            _conversation_id: &str,
            _session_id: &str,
        ) -> Result<bool, cora_cowork_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _conversation_id: &str) -> Result<bool, cora_cowork_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<cora_cowork_db::PersistedSessionState>, cora_cowork_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state(
            &self,
            _conversation_id: &str,
            _params: &cora_cowork_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, cora_cowork_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl cora_cowork_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        ...
        async fn upsert(
            &self,
            _params: &cora_cowork_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<cora_cowork_db::models::AgentMetadataRow, cora_cowork_db::DbError> {
            Err(cora_cowork_db::DbError::Init("stub".into()))
        }
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &cora_cowork_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<cora_cowork_db::models::AgentMetadataRow>, cora_cowork_db::DbError> {
            Ok(None)
        }
        ...
        async fn set_enabled(&self, _id: &str, _enabled: bool) -> Result<bool, cora_cowork_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _id: &str) -> Result<bool, cora_cowork_db::DbError> {
            Ok(false)
        }
    }
}
