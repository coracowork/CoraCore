...
async fn create_conversation_cron(
    ...
    let user_id = header_value(&headers, "x-cora-cowork-user-id")?;
    let conversation_id = header_value(&headers, "x-cora-cowork-conversation-id")?;
    let Json(req) = body.map_err(ApiError::from)?;
    let resp = state
        .cron_service
...
mod tests {
    ...
    fn header_value_rejects_missing_or_blank_required_header() {
        ...
        assert!(matches!(
            header_value(&headers, "x-cora-cowork-user-id"),
            Err(ApiError::BadRequest(message)) if message.contains("x-cora-cowork-user-id")
        ));

        headers.insert("x-cora-cowork-user-id", "   ".parse().unwrap());
    ...
    fn header_value_trims_required_header() {
        let mut headers = HeaderMap::new();
        headers.insert("x-cora-cowork-user-id", " user_1 ".parse().unwrap());

        assert_eq!(header_value(&headers, "x-cora-cowork-user-id").unwrap(), "user_1");
    }
