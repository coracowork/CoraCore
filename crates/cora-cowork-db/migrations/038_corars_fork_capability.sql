-- Constructed fork capability for the builtin corars agent (Cora CLI).
--
-- Same pattern as migration 036 (claude/codex): corars is the in-process SDK
...
--
-- Capability shape, verified against the corars session store:
--   * corars sessions are whole-file JSON state keyed by conversation id
--     (verified: cora-cowork-ai-agent factory loads `SessionManager::load(<conversation_id>)`
--     from `<data_dir>/corars-sessions`); `SessionManager::fork_from` copies
...
--     corars manager emits `BackendTurnBound` with the conversation-layer
--     turn id at every turn start, the stream persistence layer stamps it
--     onto that turn's message rows, and the corars engine stamps the same id
