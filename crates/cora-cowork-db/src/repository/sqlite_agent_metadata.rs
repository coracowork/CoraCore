...
struct AgentMetadataSafeRow {
    ...
    last_check_at: Option<cora_cowork_common::TimestampMs>,
    last_success_at: Option<cora_cowork_common::TimestampMs>,
    last_failure_at: Option<cora_cowork_common::TimestampMs>,
    command_override: Option<String>,
    env_override: Option<String>,
    created_at: cora_cowork_common::TimestampMs,
    updated_at: cora_cowork_common::TimestampMs,
}
...
impl IAgentMetadataRepository for SqliteAgentMetadataRepository {
    ...
    async fn update_agent_overrides(
        ...
        .bind(env_override)
        .bind(cora_cowork_common::now_ms())
        .bind(id)
        .execute(&self.pool)
        .await
...
mod tests {
    ...
    async fn seed_rows_populated_after_migrations() {
        ...
        assert!(
            rows.iter()
                .any(|r| r.name == "Cora CLI" && r.agent_source == "internal")
        );
    ...
    async fn seed_rows_include_icon_backfill() {
        ...
        let corars = rows
            .iter()
            .find(|row| row.agent_type == "corars" && row.agent_source == "internal")
            .expect("seeded cora cli row");
        assert_eq!(corars.icon.as_deref(), Some("/api/assets/logos/brand/cora.svg"));
        let corars_modes: serde_json::Value =
            serde_json::from_str(corars.available_modes.as_deref().expect("corars modes catalog")).unwrap();
        assert_eq!(corars_modes["current_mode_id"].as_str(), Some("default"));
        assert_eq!(
            corars_modes["available_modes"]
                .as_array()
                .expect("corars available modes")
                .iter()
                .filter_map(|item| item.get("id").and_then(serde_json::Value::as_str))
                .collect::<Vec<_>>(),
            vec!["default", "auto_edit", "yolo"]
        );
        let corars_config_options: serde_json::Value =
            serde_json::from_str(corars.config_options.as_deref().expect("corars config options")).unwrap();
        assert_eq!(
            corars_config_options["config_options"][0]["options"][1]["value"].as_str(),
            Some("auto_edit")
        );
