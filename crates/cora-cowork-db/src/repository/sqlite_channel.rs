...
impl IChannelRepository for SqliteChannelRepository {
    ...
    async fn update_session_activity(
        &self,
        id: &str,
        last_activity: cora_cowork_common::TimestampMs,
    ) -> Result<(), DbError> {
    ...
    async fn cleanup_expired_pairings(&self, now: cora_cowork_common::TimestampMs) -> Result<u64, DbError> {
        let result = sqlx::query(
...
mod tests {
    ...
    async fn update_user_last_active_updates_timestamp() {
        ...
        let new_ts = cora_cowork_common::now_ms() + 5000;
