...
async fn setup() -> (SqliteConversationRepository, cora_cowork_db::Database) {
...
fn make_conversation(suffix: &str) -> ConversationRow {
    ...
    ConversationRow {
        id: cora_cowork_common::generate_prefixed_id("conv"),
        ...
        source: Some("cora-cowork".to_string()),
...
fn make_message(conv_id: &str, content: &str) -> MessageRow {
    let now = cora_cowork_common::now_ms();
    MessageRow {
        id: cora_cowork_common::generate_prefixed_id("msg"),
        conversation_id: conv_id.to_string(),
        msg_id: Some(cora_cowork_common::generate_prefixed_id("cmsg")),
...
fn make_artifact(conv_id: &str, artifact_id: &str) -> cora_cowork_db::ConversationArtifactRow {
    cora_cowork_db::ConversationArtifactRow {
...
async fn filter_by_source_and_pinned_combined() {
    ...
    let mut c1 = make_conversation("cora-cowork-pinned");
    c1.source = Some("cora-cowork".to_string());
    c1.pinned = true;
    c1.pinned_at = Some(cora_cowork_common::now_ms());
    repo.create(&c1).await.unwrap();

    let mut c2 = make_conversation("telegram-pinned");
    c2.source = Some("telegram".to_string());
    c2.pinned = true;
    c2.pinned_at = Some(cora_cowork_common::now_ms());
    repo.create(&c2).await.unwrap();

    let mut c3 = make_conversation("cora-cowork-unpinned");
    c3.source = Some("cora-cowork".to_string());
    c3.pinned = false;
    repo.create(&c3).await.unwrap();

    // Filter: source=cora-cowork AND pinned=true
    let result = repo
        .list_paginated(
            USER_ID,
            &ConversationFilters {
...
async fn concurrent_insert_message_does_not_require_sequence_allocation() {
    ...
    let db = cora_cowork_db::init_database(&dir.path().join("messages.db")).await.unwrap();
    let repo = SqliteConversationRepository::new(db.pool().clone());
...
async fn anchor_rejects_legacy_artifact_rows() {
    ...
    assert!(matches!(err, cora_cowork_db::DbError::NotFound(_)));
}
...
async fn pin_and_unpin_conversation() {
    ...
    // Pin
    let pin_time = cora_cowork_common::now_ms();
    repo.update(
        &conv.id,
        &ConversationRowUpdate {
            pinned: Some(true),
