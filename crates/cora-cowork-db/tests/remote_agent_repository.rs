...
async fn repo() -> (Arc<dyn IRemoteAgentRepository>, cora_cowork_db::Database) {
...
async fn update_status_to_connected_with_timestamp() {
    ...
    let ts = cora_cowork_common::now_ms();
...
async fn full_crud_lifecycle() {
    ...
    // Update status
    r.update_status(&created.id, "connected", Some(cora_cowork_common::now_ms()))
        .await
        .unwrap();
