...
/// until their first handshake, and corars has a NULL backend the capability
...
async fn known_good_team_whitelist_survives() {
    ...
    let corars = repo
        ...
        .unwrap()
        .into_iter()
        .find(|row| row.agent_type == "corars")
        .expect("corars row is seeded");
    let policy: serde_json::Value = serde_json::from_str(corars.behavior_policy.as_deref().unwrap()).unwrap();
