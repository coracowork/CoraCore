//! CoraPro adoption coverage gate.
//!
//! `adopt_system_default_data` re-owns the local default user's data to the
//! machine's first external (CoraPro) account by convention: any table with a
...
#[tokio::test]
async fn every_table_is_classified_for_corapro_adoption() {
    ...
    assert!(
        ...
        "NEW TABLE(S) WITHOUT AN CORAPRO ADOPTION DECISION: {unclassified:?}\n\
         ...
         See crates/cora-cowork-db/tests/adoption_coverage.rs."
    );
...
async fn seed_default_user_project(pool: &sqlx::SqlitePool) -> (String, String) {
    ...
    let (project, entry) = store
        .create_project_with_workspace_entry(
            ...
            &folder.folder_id,
            "adopt-ws",
            cora_cowork_db::ProjectKind::Standard,
        )
        .await
        .unwrap();
...
async fn adoption_fires_once_then_new_local_data_stays_local() {
    ...
    let user = repo
        .ensure_external_user(UserType::Corapro, "ext-once", ExternalUserProjection::default())
        .await
        .unwrap();
    ...
    // CoraCowork keeps working locally: NEW data lands on the default user.
    sqlx::query(
        "INSERT INTO conversations (id, user_id, name, type, extra, status, created_at, updated_at)
...
async fn empty_first_sweep_still_consumes_the_adoption_opportunity() {
    ...
    let user = repo
        .ensure_external_user(UserType::Corapro, "ext-fresh", ExternalUserProjection::default())
        .await
        .unwrap();
    assert_eq!(repo.adopt_system_default_data(&user.id).await.unwrap(), 0);
    assert!(
    ...
    let other = repo
        .ensure_external_user(UserType::Corapro, "ext-other", ExternalUserProjection::default())
        .await
        .unwrap();
