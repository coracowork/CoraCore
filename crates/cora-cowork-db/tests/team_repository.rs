...
async fn repo() -> (Arc<dyn ITeamRepository>, cora_cowork_db::Database) {
...
async fn update_team_can_patch_workspace() {
    ...
    repo.update_team(
        "t1",
        &UpdateTeamParams {
            workspace: Some("/tmp/cora-cowork-team-shared-workspace".into()),
            ..Default::default()
        },
    )
    .await
    ...
    assert_eq!(updated.workspace, "/tmp/cora-cowork-team-shared-workspace");
}
