use cora_cowork_db::{AgentBindingResolution, init_database_memory, resolve_agent_binding};
...
async fn resolves_internal_agent_type_when_backend_is_null() {
    ...
    let resolved = resolve_agent_binding(db.pool(), "corars")
        .await
        .unwrap()
        .expect("corars should resolve");

    assert_eq!(resolved.agent_id, "632f31d2");
    assert_eq!(resolved.agent_source, "internal");
    assert_eq!(resolved.agent_type, "corars");
    assert_eq!(resolved.runtime_backend, "corars");
}
