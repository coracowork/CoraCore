use cora_cowork_db::{init_database_memory, models::ConversationRow, models::MessageRow};
use sqlx::Row;
...
async fn conversation_row_from_row() {
    ...
    sqlx::query(
        "INSERT INTO conversations \
         ...
         VALUES ('conv_fr', $1, 'FromRow Test', 'gemini', '{\"workspace\":\"/home\"}', \
                 ...
                 'finished', 'cora-cowork', 'group:42', 1, 1700000000000, 1000, 2000)",
    )
    .bind(&user_id)
    .execute(db.pool())
    .await
    ...
    assert_eq!(row.source.as_deref(), Some("cora-cowork"));
