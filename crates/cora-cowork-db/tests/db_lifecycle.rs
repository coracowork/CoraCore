...
async fn system_user_has_valid_timestamps() {
    let before = cora_cowork_common::now_ms();
    let db = init_database_memory().await.unwrap();
    let after = cora_cowork_common::now_ms();
...
fn copy_legacy_noop_when_no_legacy_db() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("cora-cowork-backend.db");
...
fn copy_legacy_noop_when_target_exists() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("cora-cowork-backend.db");
    let legacy = dir.path().join("cora-cowork.db");
...
// `coracore doctor` racing the server) both decide to apply the same
...
fn concurrent_init_database_does_not_panic_on_unique_conflict() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("cora-cowork-backend.db");
    ...
    let lock = path.with_file_name("cora-cowork-backend.db.migrate.lock");
