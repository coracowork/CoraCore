...
///
/// Key mismatch fix: the CoreCowork frontend stores the config under
...
pub(crate) async fn load_stt_config(
...
) -> Result<SpeechToTextConfig, cora_cowork_system::SystemError> {
    ...
    Ok(prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: cora_cowork_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        }))
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(cora_cowork_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = cora_cowork_system::ClientPrefService::new(repo);
