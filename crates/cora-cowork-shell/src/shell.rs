...
use cora_cowork_api_types::ToolType;
...
mod tests {
    ...
    fn linux_show_item_uses_filemanager1_when_gdbus_available() {
        let path = Path::new("/home/user/Downloads/CoraCowork.deb");
        ...
        assert!(args.contains(&"['file:///home/user/Downloads/CoraCowork.deb']".to_owned()));
    ...
    fn linux_show_item_percent_encodes_spaces_in_uri() {
        let path = Path::new("/home/user/My Downloads/CoraCowork.deb");
        let (program, args) = linux_show_item_command(path, true);
        assert_eq!(program, "gdbus");
        assert!(
            args.contains(&"['file:///home/user/My%20Downloads/CoraCowork.deb']".to_owned()),
