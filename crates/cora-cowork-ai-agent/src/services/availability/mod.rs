...
use cora_cowork_common::{AgentType, CommandSpec, EnvVar};
use cora_cowork_db::{IProviderRepository, UpdateAgentAvailabilitySnapshotParams};
...
pub struct AgentAvailabilityService {
    ...
    // Used to decide corars (built-in, no external CLI) availability: it is
...
async fn run_probe(
        ...
        && let Some(tool) = ManagedAcpToolId::from_backend(backend)
    {
    ...
    } else if meta.agent_type == AgentType::Corars {
        // corars is the built-in Rust agent: there is no external CLI to probe,
        ...
        probe_corars_provider_readiness(provider_repo).await
    } else {
        (AgentSnapshotCheckStatus::Online, None, None)
    };
...
fn explicit_probe_args(meta: &AgentMetadata) -> Result<Vec<String>, String> {
    if meta.agent_source == AgentSource::Builtin && meta.agent_source_info.bridge_binary.as_deref() == Some("npx") {
        ...
        return cora_cowork_runtime::pin_registry_npx_args(backend, &meta.args).map_err(|error| error.to_string());
    }
    Ok(meta.args.clone())
}

/// Readiness check for the built-in corars agent.
///
/// corars has no external CLI; it runs models through configured providers.
/// Mirrors `AssistantService::resolve_default_agent_type`, which treats corars
...
async fn probe_corars_provider_readiness(
    provider_repo: &Arc<dyn IProviderRepository>,
...
mod tests {
    ...
    use super::{AgentAvailabilityService, explicit_probe_args, probe_corars_provider_readiness, run_probe};
    ...
    #[tokio::test]
    async fn corars_is_offline_without_an_enabled_provider() {
        let db = init_database_memory().await.unwrap();
        let provider_repo: Arc<dyn IProviderRepository> = Arc::new(SqliteProviderRepository::new(db.pool().clone()));

        let (status, code, _msg) = probe_corars_provider_readiness(&provider_repo).await;
    ...
    #[tokio::test]
    async fn corars_is_online_when_a_provider_is_enabled() {
        let db = init_database_memory().await.unwrap();
        let provider_repo: Arc<dyn IProviderRepository> = Arc::new(SqliteProviderRepository::new(db.pool().clone()));
