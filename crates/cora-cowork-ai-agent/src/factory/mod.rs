...
pub(crate) mod corars;
mod context;

use std::path::PathBuf;
use std::sync::Arc;

use cora_cowork_db::{IMcpServerRepository, IProviderRepository};
...
async fn build_agent(deps: Arc<AgentFactoryDeps>, options: BuildTaskOptions) -> Result<AgentInstance, AgentError> {
    ...
    match context.kind {
        ...
        AgentSessionKind::Corars(corars_context) => corars::build(deps, *corars_context, model, ctx).await,
    }
}
