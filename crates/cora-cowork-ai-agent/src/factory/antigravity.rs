...
pub(super) async fn build(
    ...
    // Register CoraCowork as agy's PreToolUse gate for THIS workspace, and mint the
    ...
    let hook_env = match deps.antigravity_hook_base_url.as_deref() {
        ...
        Some(base_url) => {
            ...
            vec![
                (
                    cora_cowork_api_types::AntigravityHookConfig::ENV_BASE_URL.to_owned(),
                    base_url.to_owned(),
                ),
                (cora_cowork_api_types::AntigravityHookConfig::ENV_TOKEN.to_owned(), token),
                (
                    cora_cowork_api_types::AntigravityHookConfig::ENV_CONVERSATION_ID.to_owned(),
