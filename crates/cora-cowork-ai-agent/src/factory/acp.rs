...
pub(super) async fn build(
    ...
    // reconcile pass have completed. Matches corars factory behaviour:
...
async fn resolve_agent_command_spec(
    meta: &cora_cowork_api_types::AgentMetadata,
    workspace: &str,
    conversation_id: &str,
    broadcaster: Arc<dyn cora_cowork_realtime::EventBroadcaster>,
) -> Result<CommandSpec, AgentError> {
    if meta.agent_source == cora_cowork_api_types::AgentSource::Builtin
    ...
    let mut args: Vec<String> = resolved
        ...
        .collect();
    let launch_args = if meta.agent_source == cora_cowork_api_types::AgentSource::Builtin
    ...
    let mut env: Vec<cora_cowork_common::EnvVar> = meta
        .env
        .iter()
        ...
        .collect();
    env.extend(resolved.env.iter().map(|(name, value)| cora_cowork_common::EnvVar {
...
mod tests {
    ...
    async fn resolve_agent_command_spec_flattens_bare_npx_command() {
        ...
        let mut meta = cora_cowork_api_types::AgentMetadata {
            id: "agent-1".into(),
            icon: None,
            name: "Test ACP".into(),
            name_i18n: None,
            ...
            agent_source: cora_cowork_api_types::AgentSource::Custom,
            ...
            env: vec![cora_cowork_api_types::AgentEnvEntry {
                name: "K".into(),
                value: "V".into(),
                description: None,
            }],
        ...
        meta.agent_source = cora_cowork_api_types::AgentSource::Builtin;
    ...
    impl IMcpServerRepository for MockRepo {
        async fn list(&self) -> Result<Vec<McpServerRow>, cora_cowork_db::DbError> {
            if self.fail {
                Err(cora_cowork_db::DbError::Init("simulated".into()))
            } else {
                Ok(self.rows.clone())
            }
        }
        async fn find_by_id(&self, _id: &str) -> Result<Option<McpServerRow>, cora_cowork_db::DbError> {
            unimplemented!()
        }
        async fn find_by_name(&self, _name: &str) -> Result<Option<McpServerRow>, cora_cowork_db::DbError> {
            unimplemented!()
        }
        async fn list_by_ids_any(&self, ids: &[String]) -> Result<Vec<McpServerRow>, cora_cowork_db::DbError> {
            if self.fail {
                return Err(cora_cowork_db::DbError::Init("simulated".into()));
            }
            Ok(ids
                .iter()
