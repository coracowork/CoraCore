...
impl OutputSink for BackendOutputSink {
    ...
    fn emit_tool_call(&self, tool_use_id: &str, name: &str, input: &str) {
        ...
        tracing::debug!(
            ...
            "Derived internal tool_call id from corars tool_use_id"
        );
        tracing::info!(
            tool_use_id = %tool_use_id,
            call_id = %call_id,
            tool = name,
            status = ?ToolCallStatus::Running,
            "Emitting corars tool_call event"
        );
    ...
    fn emit_tool_result(&self, tool_use_id: &str, name: &str, is_error: bool, content: &str) {
        ...
        tracing::info!(
            ...
            tool = name,
            status = ?status,
            "Emitting corars tool_result event"
        );
...
mod tests {
    ...
    fn duplicate_tool_names_use_distinct_internal_call_ids() {
        ...
        assert_eq!(call_ids[0].0, "corars-call_a");
        assert_eq!(call_ids[1].0, "corars-call_b");
        assert_eq!(call_ids[2].0, "corars-call_a");
        assert_eq!(call_ids[3].0, "corars-call_b");
