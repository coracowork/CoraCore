...
pub struct SkillDefinition {
    ...
    pub source: cora_cowork_extension::SkillSource,
...
pub struct AcpSkillManager {
    ...
    #[allow(dead_code)]
    paths: Arc<cora_cowork_extension::SkillPaths>,
...
impl AcpSkillManager {
    pub fn new(paths: Arc<cora_cowork_extension::SkillPaths>) -> Arc<Self> {
        Arc::new(Self {
    ...
    pub fn new_with_repo(paths: Arc<cora_cowork_extension::SkillPaths>, skill_repo: Arc<dyn ISkillRepository>) -> Arc<Self> {
        Arc::new(Self {
            cache: RwLock::new(HashMap::new()),
            discovered: RwLock::new(false),
            paths,
    ...
    /// Discover skills via `cora_cowork_extension::list_available_skills`.
    ///
    /// Filtering rules:
    ...
    pub async fn discover_skills(
        ...
        for item in items {
            ...
            let keep = match item.source {
                cora_cowork_extension::SkillSource::Builtin => {
                    if is_auto_inject {
                ...
                cora_cowork_extension::SkillSource::Custom
                | cora_cowork_extension::SkillSource::Cron
                | cora_cowork_extension::SkillSource::Extension => {
                    enabled_skills.is_some_and(|en| en.iter().any(|n| n == &item.name))
                }
            };
            if !keep {
    ...
    async fn list_available_skills(
        &self,
    ) -> Result<Vec<cora_cowork_extension::SkillListItem>, cora_cowork_extension::ExtensionError> {
        if let Some(repo) = &self.skill_repo {
            cora_cowork_extension::list_available_skills_with_repo(&self.paths, repo.as_ref()).await
        } else {
            cora_cowork_extension::list_available_skills(&self.paths).await
        }
    }
    ...
    /// - `Builtin` → `cora_cowork_extension::read_builtin_skill(&paths, relative)`
        ...
        let content = match def.source {
            cora_cowork_extension::SkillSource::Builtin => {
                if let Some(rel) = def.relative_location.as_deref() {
                    match cora_cowork_extension::read_builtin_skill(&self.paths, rel).await {
                        Ok(s) => s,
                        Err(e) => {
...
mod tests {
    ...
    async fn new_accepts_skill_paths() {
        ...
        let paths = std::sync::Arc::new(cora_cowork_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    ...
    fn skill_definition_has_source_and_relative_location() {
        let def = SkillDefinition {
            ...
            source: cora_cowork_extension::SkillSource::Builtin,
            relative_location: Some("auto-inject/x/SKILL.md".into()),
            body: None,
        };
        assert_eq!(def.source, cora_cowork_extension::SkillSource::Builtin);
        assert_eq!(def.relative_location.as_deref(), Some("auto-inject/x/SKILL.md"));
    }

    // Frontmatter parsing tests live in cora_cowork-extension (covers
    ...
    // because they now need `cora_cowork_extension::BUILTIN_SKILLS_ENV_VAR` to
    ...
    async fn get_skill_unknown_returns_none() {
        let tmp = TempDir::new().unwrap();
        let mgr = AcpSkillManager::new(std::sync::Arc::new(cora_cowork_extension::resolve_skill_paths(
            tmp.path(),
            tmp.path(),
        )));
