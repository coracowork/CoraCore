...
//! full `[[CORA_FILES]]` form (UI chips and history are untouched); only
//! the agent-bound copy is rewritten. Mirrors the corars precedent in
//! `manager/corars/content.rs`: strip the trailing marker block when it
...
pub struct MediaPartition {
    /// Agent-bound content: the user text with the `[[CORA_FILES]]` block
...
/// Strip the trailing `[[CORA_FILES]]` block iff its path list matches
/// `files` exactly (same validation as corars `strip_attachment_metadata`);
...
/// The agent-bound text with the `[[CORA_FILES]]` block listing EVERY
...
mod tests {
    ...
    fn temp_file(name: &str, bytes: &[u8]) -> String {
        let dir = std::env::temp_dir().join("cora-cowork-media-tests");
    ...
    fn svg_and_missing_and_oversized_stay_paths() {
        ...
        let missing = "/nonexistent/cora-cowork-media-test.png".to_owned();
