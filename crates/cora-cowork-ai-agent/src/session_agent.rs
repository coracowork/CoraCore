//! `SessionAgentTask` — adapts the clean-slate `cora_cowork_session::SessionBackend`
...
use cora_cowork_common::{AgentKillReason, ConversationStatus, TimestampMs, now_ms};
...
use cora_cowork_api_types::{AcpBuildExtra, TEAM_MCP_SERVER_NAME};
use cora_cowork_common::AgentType;
use cora_cowork_db::{IAcpSessionRepository, IMcpServerRepository, SaveRuntimeStateParams};
...
fn resolve_current_model_efforts(models: &[cora_cowork_session::ModelInfo], current_model: Option<&str>) -> Vec<String> {
...
/// handshake, in the SAME `cora_cowork_session` shape the getters read off live
...
struct CatalogPreload {
    available_models: Vec<cora_cowork_session::ModelInfo>,
    current_model: Option<String>,
    available_modes: Vec<cora_cowork_session::ModeInfo>,
...
impl CatalogPreload {
    ...
    fn from_handshake(handshake: &cora_cowork_api_types::AgentHandshake) -> Self {
        ...
        let (available_models, current_model) = handshake
            ...
            .map(|state| {
                let models = state
                    .available_models
                    .iter()
                    .map(|m| cora_cowork_session::ModelInfo {
        ...
        let (available_modes, current_mode) = handshake
            ...
            .map(|state| {
                let modes = state
                    .available_modes
                    .iter()
                    .map(|m| cora_cowork_session::ModeInfo {
                        id: m.id.to_string(),
                        name: m.name.clone(),
                        description: m.description.clone(),
                    })
...
impl SessionAgentTask {
    ...
    pub fn new_with_preload(
        ...
        backend: Arc<dyn SessionBackend>,
        session_repo: Option<Arc<dyn IAcpSessionRepository>>,
        handshake: &cora_cowork_api_types::AgentHandshake,
        prompt_dump: Option<SessionPromptDump>,
    ...
    /// pre-multimodal form (path in the [[CORA_FILES]] text + resource link).
    ///
    /// A native media block carries ONLY bytes: `ContentBlock::Image` is
    /// `{data, media_type}` with no path field, and `partition_media` has
    /// already stripped that path out of the [[CORA_FILES]] text. So each
    ...
    /// `PreToolUse` hook calls CoraCowork over HTTP instead, and that request lands
    ...
    ) -> cora_cowork_session::PermissionDecision {
        self.backend.request_external_permission(tool_name, input).await
    }

    pub fn get_confirmations(&self) -> Vec<cora_cowork_common::Confirmation> {
        self.backend
            ...
            .map(|p| {
                ...
                cora_cowork_common::Confirmation {
                    ...
                    options: options
                        .into_iter()
                        .map(|o| cora_cowork_common::ConfirmationOption {
                            label: o.name,
    ...
    pub fn answer_ask(
        &self,
        request_id: &str,
        answers: Option<Vec<cora_cowork_api_types::AskQuestionAnswer>>,
    ) -> Result<(), AgentError> {
        ...
        let answers = answers.map(|list| {
            list.into_iter()
                .map(|a| cora_cowork_session::QuestionAnswer {
                    question: a.question,
                    labels: a.labels,
                })
                .collect::<Vec<_>>()
        ...
        tokio::spawn(async move {
            let command = cora_cowork_session::Command::AnswerAsk {
    ...
    pub fn confirm(
        ...
        use cora_cowork_session::PermissionDecision;
    ...
    fn effective_catalog(
        ...
        Vec<cora_cowork_session::ModelInfo>,
        Option<String>,
        Vec<cora_cowork_session::ModeInfo>,
        Option<String>,
    ) {
    ...
    pub async fn mode(&self) -> Result<cora_cowork_api_types::AgentModeResponse, AgentError> {
        let caps = self.backend.capabilities();
        ...
        let current_mode = caps.current_mode.or_else(|| self.catalog_preload.current_mode.clone());
        Ok(cora_cowork_api_types::AgentModeResponse {
    ...
    pub async fn get_model(&self) -> Result<cora_cowork_api_types::GetModelInfoResponse, AgentError> {
        ...
        if models.is_empty() && current_model.is_none() && override_model.is_none() {
            return Ok(cora_cowork_api_types::GetModelInfoResponse { model_info: None });
        }
        let available_models: Vec<cora_cowork_api_types::ModelInfoEntry> = models
            .iter()
            .map(|m| cora_cowork_api_types::ModelInfoEntry {
                id: m.id.clone(),
                label: m.name.clone(),
            })
            .collect();
        ...
        Ok(cora_cowork_api_types::GetModelInfoResponse {
            model_info: Some(cora_cowork_api_types::ModelInfoPayload {
    ...
    pub async fn get_config_options(&self) -> Result<cora_cowork_api_types::GetConfigOptionsResponse, AgentError> {
        // Live catalog wins; cold-start resume falls back to the persisted-handshake
        ...
        if !modes.is_empty() {
            config_options.push(cora_cowork_api_types::AcpConfigOptionDto {
                id: "mode".into(),
                name: Some("Mode".into()),
                label: None,
                description: None,
                ...
                options: modes
                    .iter()
                    .map(|m| cora_cowork_api_types::AcpConfigSelectOptionDto {
                        value: m.id.clone(),
        ...
        // path's `thought_level` config option (category-keyed so CoraCowork's
        ...
        if !efforts.is_empty() {
            config_options.push(cora_cowork_api_types::AcpConfigOptionDto {
                ...
                options: efforts
                    .iter()
                    .map(|e| cora_cowork_api_types::AcpConfigSelectOptionDto {
                        value: e.clone(),
                        name: Some(e.clone()),
                        label: None,
                        description: None,
        ...
        Ok(cora_cowork_api_types::GetConfigOptionsResponse { config_options })
    }
    ...
    pub async fn set_config_option(
        ...
        option_id: &str,
        value: &str,
    ) -> Result<cora_cowork_api_types::SetConfigOptionResponse, AgentError> {
        ...
        let deferred = option_id == "mode"
            && self.backend.capabilities().mode_switch_effect == cora_cowork_session::ModeSwitchEffect::NextTurn;
        ...
        match option_id {
            ...
            _ => {
                return Ok(cora_cowork_api_types::SetConfigOptionResponse {
                    confirmation: cora_cowork_api_types::ConfigOptionConfirmation::CommandAck,
                    config_options: None,
                });
            }
        }
        ...
        let observed = snapshot
            ...
            == Some(value);
        Ok(cora_cowork_api_types::SetConfigOptionResponse {
            ...
            confirmation: if deferred {
                cora_cowork_api_types::ConfigOptionConfirmation::PendingNextTurn
            } else if observed {
                cora_cowork_api_types::ConfigOptionConfirmation::Observed
            } else {
                cora_cowork_api_types::ConfigOptionConfirmation::CommandAck
            },
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<cora_cowork_api_types::SlashCommandItem>, AgentError> {
        let caps = self.backend.capabilities();
        Ok(caps
            .slash_commands
            .iter()
            .map(|c| cora_cowork_api_types::SlashCommandItem {
    ...
    async fn cancel(&self) -> Result<(), AgentError> {
        ...
        self.backend
            .dispatch(Command::Cancel {
                target: cora_cowork_session::CancelTarget::Turn,
            })
            .await
            .map(|_| ())
...
pub struct SessionBuildInputs<'a> {
    ...
    pub metadata: &'a cora_cowork_api_types::AgentMetadata,
    ...
    /// The conversation runtime context env (`CORA_COWORK_USER_ID` /
    /// `CORA_COWORK_CONVERSATION_ID` / `CORA_COWORK_HELPER_BIN` / `CORA_COWORK_BASE_URL` /
    /// `CORA_COWORK_RUNTIME_TOKEN`, filled by `apply_conversation_runtime_context`).
...
pub(crate) fn resolved_effort(
    config: &cora_cowork_api_types::AcpBuildExtra,
...
pub(crate) fn resolved_session_mode(
    ...
    session_snapshot: Option<&PersistedSessionState>,
    metadata: &cora_cowork_api_types::AgentMetadata,
) -> Option<String> {
    session_snapshot
...
fn spec_mode_model(
    ...
    session_snapshot: Option<&PersistedSessionState>,
    metadata: &cora_cowork_api_types::AgentMetadata,
) -> (cora_cowork_session::SessionSpec, Option<String>, Option<String>) {
    use cora_cowork_session::SessionSpec;
    ...
    // transform the ACP path applies in `initial_mode_from_params`. CoraCowork persists
...
/// assembly (`crates/cora-cowork-app/src/session_runtime/mod.rs`): it resolves the
...
pub async fn build_antigravity_instance(
    ...
    use cora_cowork_session::{AntigravityConnection, BackendConnection, McpServerSpec, SessionConfig, SessionInit};
    ...
    let init = SessionInit {
        ...
        resume: matches!(spec, cora_cowork_session::SessionSpec::Resume { .. }),
    };
    ...
    let backend = AntigravityConnection::new(spawner)
        ...
        .await
        .map_err(|e| match e {
            cora_cowork_session::BackendError::WorkspaceUnavailable(path) => {
...
pub async fn build_session_instance(
    ...
    // reserved name `cora-cowork-team` is filtered from BOTH sources so the team
    ...
    if backend_label == "claude" {
        ...
        if !provider_env.is_empty() {
            ...
            session_config.spawn_env.extend(
                provider_env
                    .into_iter()
                    .map(|(name, value)| cora_cowork_common::EnvVar { name, value }),
            );
...
fn resolve_session_cli_program(
    ...
    // prompt behaved differently in CoraCowork and in the user's terminal, with
    ...
    cora_cowork_runtime::resolve_command_path(backend_label)
}
...
///     path injected these via `resolve_agent_command_spec`; `CORA_COWORK_*`/`PATH`/…
///     keys are already filtered at the registry (`is_blocked_override_env_key`),
///     so they cannot shadow the runtime context below.
///  2. the `CORA_COWORK_*` conversation runtime context (`CORA_COWORK_USER_ID` /
///     `CORA_COWORK_CONVERSATION_ID` / `CORA_COWORK_HELPER_BIN` / `CORA_COWORK_BASE_URL` /
///     `CORA_COWORK_RUNTIME_TOKEN`) — the legacy path appended these via
///     `apply_acp_launch_policy` for every agent spawn.
fn assemble_spawn_env(
    agent_env: &[cora_cowork_api_types::AgentEnvEntry],
    runtime_env: &[(String, String)],
) -> Vec<cora_cowork_common::EnvVar> {
    let mut env: Vec<cora_cowork_common::EnvVar> = agent_env
        .iter()
        .map(|entry| cora_cowork_common::EnvVar {
            name: entry.name.clone(),
            value: entry.value.clone(),
        })
        .collect();
    env.extend(runtime_env.iter().map(|(name, value)| cora_cowork_common::EnvVar {
        name: name.clone(),
        value: value.clone(),
    }));
    env
...
fn build_session_cli_config_dump_value(backend: &str, cfg: &cora_cowork_session::SessionConfig) -> serde_json::Value {
    use cora_cowork_session::McpTransport;
...
fn session_server_to_spec(server: &cora_cowork_api_types::SessionMcpServer) -> cora_cowork_session::McpServerSpec {
    use cora_cowork_api_types::SessionMcpTransport as T;
    use cora_cowork_session::{McpServerSpec, McpTransport};
...
fn team_mcp_server_spec(cfg: &cora_cowork_api_types::TeamMcpStdioConfig) -> cora_cowork_session::McpServerSpec {
    use cora_cowork_api_types::TeamMcpStdioConfig as C;
    cora_cowork_session::McpServerSpec {
        name: cora_cowork_api_types::TEAM_MCP_SERVER_NAME.to_owned(),
        transport: cora_cowork_session::McpTransport::Stdio {
...
pub fn spawn_catalog_writeback(
    agent_id: String,
    user_id: String,
    backend: Arc<dyn cora_cowork_session::SessionBackend>,
...
/// going empty).
fn catalog_partial_from_caps(caps: &cora_cowork_session::Capabilities) -> Option<cora_cowork_api_types::AgentHandshake> {
    let mut config_options = Vec::new();
    ...
    Some(cora_cowork_api_types::AgentHandshake {
        config_options,
        available_modes,
        available_models,
        available_commands,
...
fn emit_config_options_snapshot(modes: &[ModeInfo], models: &[ModelInfo], runtime: &SessionRuntime) {
    ...
    let mut config_options: Vec<cora_cowork_api_types::AcpConfigOptionDto> = Vec::new();
    if !modes.is_empty() {
        config_options.push(cora_cowork_api_types::AcpConfigOptionDto {
            id: "mode".into(),
            name: Some("Mode".into()),
...
fn spawn_event_pump(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            {
                use cora_cowork_session::{SubagentStatus, SubagentTaskKind};
                match status {
            ...
            let suppress_intermediate_finish = matches!(&env.event, SessionEvent::TurnResult { is_error, outcome, .. }
                if !workflow_inflight.is_empty()
                    && !*is_error
                    && !matches!(outcome, cora_cowork_session::TurnOutcome::Cancelled { .. }));
                ...
                SessionEvent::TurnResult { .. } | SessionEvent::Detached { .. } if !suppress_intermediate_finish => {
                    ...
                    let keep_background = matches!(
                        ...
                        SessionEvent::TurnResult { is_error: false, outcome, .. }
                            if !matches!(outcome, cora_cowork_session::TurnOutcome::Cancelled { .. })
                    );
                    for data in settle_workflow_cards(
                        &mut workflow_cards,
                        crate::workflow_progress::CardStatus::Cancelled,
...
/// `Orchestrator` uses (`cora_cowork_session::is_unrecoverable_resume_error`), so a
...
fn is_dead_resume_anchor(event: &SessionEvent) -> bool {
    use cora_cowork_session::{TurnOutcome, is_unrecoverable_resume_error};
    ...
    let reason = cora_cowork_session::ErrorReason::Backend {
...
fn broadcast_usage_frame(bus: &dyn EventBroadcaster, conversation_id: &str, user_id: &str, event: &SessionEvent) {
    ...
    cora_cowork_common::normalize_keys_to_snake_case(&mut event_data);
    ...
    bus.broadcast(cora_cowork_api_types::WebSocketMessage::new("message.stream", payload));
}
...
fn empty_turn_tip(outcome: &cora_cowork_session::TurnOutcome) -> Option<TipsEventData> {
    use cora_cowork_session::{StopReason, TruncationKind, TurnOutcome};
...
fn translate_event(event: SessionEvent, conversation_id: &str, terminal_result_seen: bool) -> Vec<AgentStreamEvent> {
    match event {
        ...
        SessionEvent::TurnResult {
            ...
            let is_cancel = matches!(outcome, cora_cowork_session::TurnOutcome::Cancelled { .. });
        ...
        SessionEvent::Detached { exit, redacted_summary } => {
            ...
            match cora_cowork_session::crash_outcome(terminal_result_seen, exit) {
                cora_cowork_session::Outcome::Crashed => {
                ...
                cora_cowork_session::Outcome::CleanNoResult | cora_cowork_session::Outcome::FollowResult => {
                    vec![AgentStreamEvent::Finish(FinishEventData::default())]
                }
            }
        }
        ...
        // what they are approving (CoraCowork issue #3779); the generic
        ...
        SessionEvent::UsageDelta {
            ...
            // The renderer reads these five keys out of `_meta` (CoraCowork
            ...
            let entries: Vec<serde_json::Value> = entries
                ...
                .map(|e| {
                    let status = match e.status {
                        cora_cowork_session::PlanStatus::Pending => "pending",
                        cora_cowork_session::PlanStatus::InProgress => "in_progress",
                        cora_cowork_session::PlanStatus::Completed => "completed",
                    };
        ...
        SessionEvent::Notice {
            ...
            let tip_type = match level {
                cora_cowork_session::NoticeLevel::Info => TipType::Info,
                cora_cowork_session::NoticeLevel::Warning => TipType::Warning,
            };
...
mod build_mapping_tests {
    ...
    fn extra_with_thought_level(level: Option<&str>) -> cora_cowork_api_types::AcpBuildExtra {
        cora_cowork_api_types::AcpBuildExtra {
    ...
    fn assemble_spawn_env_orders_agent_overrides_before_runtime_context() {
        let agent_env = vec![
            cora_cowork_api_types::AgentEnvEntry {
        ...
        let runtime_env = vec![
            ("CORA_COWORK_USER_ID".to_owned(), "user-1".to_owned()),
            ("CORA_COWORK_RUNTIME_TOKEN".to_owned(), "tok".to_owned()),
        ];
        ...
        assert_eq!(
            names,
            ["AWS_PROFILE", "HTTPS_PROXY", "CORA_COWORK_USER_ID", "CORA_COWORK_RUNTIME_TOKEN"]
        );
    ...
    fn session_cli_config_dump_value_captures_resolved_surface() {
        use cora_cowork_session::{McpServerSpec, McpTransport, SessionConfig, SessionInit};
        let cfg = SessionConfig {
            ...
            spawn_env: vec![cora_cowork_common::EnvVar {
    ...
    fn test_metadata(backend: Option<&str>, yolo_id: Option<&str>) -> cora_cowork_api_types::AgentMetadata {
        use cora_cowork_api_types::{AgentHandshake, AgentMetadata, AgentSource, AgentSourceInfo, BehaviorPolicy};
        use cora_cowork_common::AgentType;
        AgentMetadata {
            id: "test".into(),
            icon: None,
    ...
    struct DirectMcpRepo {
        rows: Vec<cora_cowork_db::models::McpServerRow>,
    }

    #[derive(Default)]
    struct RecordingFailSpawner {
        last_command: std::sync::Mutex<Option<cora_cowork_common::CommandSpec>>,
    }

    impl RecordingFailSpawner {
        fn last_command(&self) -> Option<cora_cowork_common::CommandSpec> {
            self.last_command.lock().unwrap().clone()
        }
    }

    #[async_trait::async_trait]
    impl cora_cowork_process::Spawner for RecordingFailSpawner {
        async fn spawn(
            &self,
            spec: cora_cowork_common::CommandSpec,
            _extra_env: &[(String, String)],
            _opaque_owner_tag: &str,
        ) -> Result<Arc<cora_cowork_process::ManagedProcess>, cora_cowork_process::ProcessError> {
            *self.last_command.lock().unwrap() = Some(spec);
            Err(cora_cowork_process::ProcessError::internal(
    ...
    impl IMcpServerRepository for DirectMcpRepo {
        async fn list(&self, user_id: &str) -> Result<Vec<cora_cowork_db::models::McpServerRow>, cora_cowork_db::DbError> {
        ...
        async fn find_by_id(
            ...
            user_id: &str,
            id: &str,
        ) -> Result<Option<cora_cowork_db::models::McpServerRow>, cora_cowork_db::DbError> {
            Ok(self
                .rows
                .iter()
        ...
        ) -> Result<cora_cowork_db::models::McpServerRow, cora_cowork_db::DbError> {
        ...
        async fn delete(&self, _user_id: &str, _id: &str) -> Result<(), cora_cowork_db::DbError> {
            unimplemented!("not needed for direct assembly test")
        }
        ...
        ) -> Result<Vec<cora_cowork_db::models::McpServerRow>, cora_cowork_db::DbError> {
            unimplemented!("not needed for direct assembly test")
        }
    ...
    fn direct_mcp_row(id: &str, name: &str) -> cora_cowork_db::models::McpServerRow {
        cora_cowork_db::models::McpServerRow {
            id: id.into(),
            user_id: "user-1".into(),
            name: name.into(),
            description: None,
    ...
    async fn direct_claude_spawn_contains_team_nonbuiltin_and_builtin_without_reserved_override() {
        use cora_cowork_api_types::{SessionMcpServer, SessionMcpTransport, TeamMcpStdioConfig};
        ...
        let broadcaster: Arc<dyn EventBroadcaster> = Arc::new(cora_cowork_realtime::BroadcastEventBus::new(16));
    ...
    fn spec_fork_quadrants() {
        let fork_cfg = AcpBuildExtra {
            fork: Some(cora_cowork_api_types::ForkSpec {
    ...
    fn catalog_partial_projects_discovered_modes_models_commands() {
        use cora_cowork_session::{ModeInfo, ModelInfo, SlashCommandInfo};
        let caps = cora_cowork_session::Capabilities {
        ...
        let empty = cora_cowork_session::Capabilities::default();
        assert!(
    ...
    fn session_server_to_spec_collapses_4_transports_to_3_and_sorts_kv() {
        use cora_cowork_api_types::{SessionMcpServer, SessionMcpTransport};
        use cora_cowork_session::McpTransport;
...
mod translate_tests {
    use super::*;
    use crate::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    use cora_cowork_session::PermissionKind;
    ...
    /// (CoraCowork `BREAKDOWN_KEYS`). A backend that reports nothing must emit NO
    ...
    fn breakdown_rides_meta_under_the_renderer_key_names() {
        let with = translate_event(
            SessionEvent::UsageDelta {
                ...
                breakdown: cora_cowork_session::UsageBreakdown {
    ...
    fn plan_surfaces_with_snake_case_status() {
        use cora_cowork_session::{PlanEntry, PlanStatus};
        let events = translate_event(
            SessionEvent::Plan {
                entries: vec![
                    PlanEntry {
    ...
    fn cancelled_turn_emits_finish_without_error() {
        use cora_cowork_session::{CancelReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
                api_error_status: None,
    ...
    fn errored_turn_emits_rich_error_terminal() {
        use cora_cowork_session::{StopReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
    ...
    fn detached_crash_surfaces_as_rich_disconnect_error() {
        use cora_cowork_session::ExitStatusLite;
        let events = translate_event(
    ...
    fn tip_code(outcome: cora_cowork_session::TurnOutcome) -> Option<(TipType, String)> {
...
mod persist_tests {
    ...
    use super::*;
    use cora_cowork_db::{CreateAcpSessionParams, IAcpSessionRepository, SqliteAcpSessionRepository, init_database_memory};
    ...
    async fn seeded_repo() -> (Arc<dyn IAcpSessionRepository>, cora_cowork_db::Database) {
    ...
    async fn usage_arriving_after_the_turn_ended_is_still_persisted() {
        ...
        persist_side_effects(
            ...
            &SessionEvent::TurnResult {
                ...
                epoch: 0,
                outcome: cora_cowork_session::TurnOutcome::EndTurn,
            },
        )
        .await;
    ...
    fn broadcast_frame_matches_the_renderer_contract() {
        #[derive(Default)]
        struct Recorder(std::sync::Mutex<Vec<cora_cowork_api_types::WebSocketMessage<serde_json::Value>>>);
        impl EventBroadcaster for Recorder {
            fn broadcast(&self, event: cora_cowork_api_types::WebSocketMessage<serde_json::Value>) {
    ...
    async fn send_dispatch_session_not_found_clears_anchor_and_classifies() {
        ...
        assert_eq!(
            err.code(),
            Some(cora_cowork_api_types::AgentErrorCode::UserAgentSessionNotFound),
    ...
    impl SessionBackend for EffortCapsBackend {
        ...
        fn capabilities(&self) -> Capabilities {
            use cora_cowork_session::ModelInfo;
            Capabilities {
    ...
    async fn set_config_option_effort_returns_observed_via_override() {
        ...
        assert!(
            matches!(resp.confirmation, cora_cowork_api_types::ConfigOptionConfirmation::Observed),
    ...
    fn is_dead_resume_anchor_matrix() {
        use cora_cowork_session::{CancelReason, StopReason, TurnOutcome};
...
mod pump_tests {
    ...
    struct RecordingBackend {
        ...
        blocks: cora_cowork_session::BlockSet,
    }
    ...
    // Image-capable backend: an image attachment leaves the [[CORA_FILES]]
    ...
    async fn send_message_partitions_image_into_native_block() {
        let dir = std::env::temp_dir().join("cora-cowork-session-media-tests");
        ...
        let marker = cora_cowork_common::constants::CORA_COWORK_FILES_MARKER;
    ...
    async fn send_message_pairs_image_block_with_resource_link() {
        let dir = std::env::temp_dir().join("cora-cowork-session-media-link-tests");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn clean_turn_end_keeps_detached_exec_card_but_cancels_others() {
        ...
        let clean_end = SessionEvent::TurnResult {
            ...
            epoch: 0,
            outcome: TurnOutcome::Completed {
                stop_reason: cora_cowork_session::StopReason::EndTurn,
            },
        };
    ...
    async fn cancelled_turn_closes_open_tool_calls_as_canceled_before_finish() {
        use cora_cowork_session::{CancelReason, SubagentKind, TurnOutcome};
        let script = vec![
    ...
    async fn completed_tool_call_is_not_recanceled_at_turn_end() {
        use cora_cowork_session::SubagentKind;
        let script = vec![
            env(SessionEvent::TurnStarted { epoch: 1 }),
            env(SessionEvent::ToolCall {
                tool_use_id: "call-1".into(),
    ...
    async fn catalog_updated_projects_config_option_with_model_and_mode() {
        use cora_cowork_session::{ModeInfo, ModelInfo};
    ...
    async fn catalog_updated_projects_available_commands_frame() {
        use cora_cowork_session::SlashCommandInfo;
        let script = vec![env(SessionEvent::CatalogUpdated {
            models: Vec::new(),
            modes: Vec::new(),
            slash_commands: vec![
    ...
    async fn workflow_launch_result_finish_is_suppressed_until_workflow_completes() {
        use cora_cowork_session::SubagentStatus;
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_wf".into(),
                name: "Task".into(),
                subagent: cora_cowork_session::SubagentKind::Workflow,
                input: serde_json::Value::Null,
                parent_tool_use_id: None,
            }),
            ...
            env(SessionEvent::SubagentUpdate {
                ...
                kind: Some(cora_cowork_session::SubagentTaskKind::WorkflowContainer),
            }),
            env(SessionEvent::MessageDelta {
                item_id: "m".into(),
    ...
    fn wf_detail(r#ref: &str, label: &str, state: cora_cowork_session::WorkflowLoopState) -> cora_cowork_session::SessionEvent {
        SessionEvent::SubagentDetail {
            r#ref: r#ref.into(),
    ...
    fn wf_container(
        status: cora_cowork_session::SubagentStatus,
        kind: Option<cora_cowork_session::SubagentTaskKind>,
    ) -> SessionEvent {
        SessionEvent::SubagentUpdate {
            r#ref: "task-wf".into(),
            label: Some("wf".into()),
    ...
    async fn workflow_progress_streams_while_the_workflow_runs() {
        use cora_cowork_session::{SubagentStatus, SubagentTaskKind, WorkflowLoopState};
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_wf".into(),
                name: "Workflow".into(),
    ...
    async fn background_bash_gets_a_live_card_and_settles_on_notification() {
        ...
        let script = vec![
            ...
            env(SessionEvent::ToolResult {
                tool_use_id: "toolu_bg".into(),
                is_error: false,
                content: vec![cora_cowork_session::ToolResultContent::Text(
    ...
    async fn cancelled_turn_settles_background_cards() {
        use cora_cowork_session::{CancelReason, SubagentStatus, SubagentTaskKind};
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_bg".into(),
                name: "Bash".into(),
            ...
            env(SessionEvent::TurnResult {
                ...
                result_text: String::new(),
                epoch: 0,
                outcome: cora_cowork_session::TurnOutcome::Cancelled {
                    reason: CancelReason::UserCancel,
                },
            }),
        ];
    ...
    struct PendingPermBackend(cora_cowork_session::PendingPermissionView);
    ...
    impl SessionBackend for PendingPermBackend {
        ...
        fn pending_permission_requests(&self) -> Vec<cora_cowork_session::PendingPermissionView> {
            vec![self.0.clone()]
        }
    }
    ...
    async fn get_confirmations_recovers_pending_ask_user_question() {
        let backend: Arc<dyn SessionBackend> = Arc::new(PendingPermBackend(cora_cowork_session::PendingPermissionView {
    ...
    impl SessionBackend for NextTurnEffectBackend {
        ...
        fn capabilities(&self) -> Capabilities {
            Capabilities {
                mode_switch_effect: cora_cowork_session::ModeSwitchEffect::NextTurn,
                ..StaticCapsBackend.capabilities()
            }
        }
    }
    ...
    fn handshake_with_catalog() -> cora_cowork_api_types::AgentHandshake {
        cora_cowork_api_types::AgentHandshake {
    ...
    async fn live_capabilities_overwrite_stale_preload() {
        ...
        let stale = cora_cowork_api_types::AgentHandshake {
            available_models: Some(serde_json::json!({
...
mod force_kill_tests {
    ...
    use cora_cowork_common::{AgentKillReason, ConversationStatus};
    use cora_cowork_session::{
        Admission, BackendError, Capabilities, Command, CommandReceipt, SessionBackend, SessionEnvelope,
    };
    use futures_util::stream::BoxStream;
...
mod cold_start_effort_tests {
    ...
    fn handshake_with(models: serde_json::Value) -> cora_cowork_api_types::AgentHandshake {
        cora_cowork_api_types::AgentHandshake {
...
mod catalog_writeback_tests {
    ...
    use super::*;
    use cora_cowork_session::{Admission, Capabilities, CommandReceipt, ModeInfo, ModelInfo};
    use std::sync::atomic::{AtomicUsize, Ordering};
