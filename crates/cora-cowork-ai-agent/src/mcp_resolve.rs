...
//! `Vec<agent_client_protocol::McpServer>`, `factory::corars::load_user_mcp_servers`
//! → `HashMap<String, McpServerConfig>`) emit SDK/engine-specific types. This
//! module emits the NEUTRAL `cora_cowork_api_types::SessionMcpServer` so the app
//! boundary (`cora-cowork-app`) can convert it once into the crate-local
//! `cora_cowork_session::McpServerSpec`, and each backend serializes that into its own
//! wire shape. Same row-walking + selection + stdio-launch-resolution logic as
//! the legacy ACP path, but vendor-neutral.

use std::sync::Arc;

use cora_cowork_api_types::{SessionMcpServer, SessionMcpTransport, TEAM_MCP_SERVER_NAME};
use cora_cowork_db::IMcpServerRepository;
use cora_cowork_db::models::McpServerRow;
use cora_cowork_realtime::EventBroadcaster;
use cora_cowork_runtime::ensure_runtime_command;
...
pub async fn resolve_session_mcp_servers(
    ...
    for row in rows {
        ...
        // `cora-cowork-team` is a reserved wire-level name: the team coordination MCP
...
///
/// This is the SINGLE shared row→neutral conversion: `cora-cowork-conversation` (team
...
mod tests {
    ...
    impl IMcpServerRepository for MockRepo {
        async fn list(&self, user_id: &str) -> Result<Vec<McpServerRow>, cora_cowork_db::DbError> {
            if self.fail {
                return Err(cora_cowork_db::DbError::Init("boom".into()));
            }
            Ok(self.rows.iter().filter(|row| row.user_id == user_id).cloned().collect())
        }
        async fn find_by_id(&self, user_id: &str, id: &str) -> Result<Option<McpServerRow>, cora_cowork_db::DbError> {
            Ok(self
                .rows
                .iter()
        ...
        async fn find_by_name(&self, user_id: &str, name: &str) -> Result<Option<McpServerRow>, cora_cowork_db::DbError> {
            Ok(self
                .rows
                .iter()
    ...
    fn test_broadcaster() -> Arc<dyn EventBroadcaster> {
        Arc::new(cora_cowork_realtime::BroadcastEventBus::new(16))
    }
