...
use crate::manager::corars::CorarsAgentManager;
...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<cora_cowork_api_types::AgentModeResponse, AgentError> {
...
impl AgentInstance {
    ...
    pub fn kill_and_wait(
        ...
        match self {
            ...
            Self::Corars(m) => m.kill_and_wait(reason),
            #[cfg(any(test, feature = "test-support"))]
            Self::Mock(m) => {
                let _ = m.kill(reason);
    ...
    pub fn confirm(
        ...
        match self {
            ...
            Self::Corars(m) => m.confirm(msg_id, call_id, data, always_allow),
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub fn check_approval(&self, action: &str, command_type: Option<&str>) -> bool {
        match self {
            Self::Acp(_) => false,
            Self::Corars(m) => m.check_approval(action, command_type),
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_mode(&self) -> Result<cora_cowork_api_types::AgentModeResponse, AgentError> {
        match self {
            Self::Acp(m) => m.mode().await,
            Self::Corars(m) => m.mode().await,
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_model(&self) -> Result<GetModelInfoResponse, AgentError> {
        match self {
            ...
            Self::Corars(_) => Ok(GetModelInfoResponse { model_info: None }),
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_config_options(&self) -> Result<GetConfigOptionsResponse, AgentError> {
        match self {
            ...
            Self::Corars(m) => m.config_options().await,
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn set_config_option(&self, option_id: &str, value: &str) -> Result<SetConfigOptionResponse, AgentError> {
        ...
        match self {
            ...
            Self::Corars(m) => m.set_config_option(option_id, value).await,
            #[cfg(any(test, feature = "test-support"))]
    ...
    /// [`cora_cowork_common::normalize_keys_to_snake_case`] so keys land as
    /// `used` / `size` / `cost` to match the CoraCowork wire convention —
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AgentError> {
        match self {
            Self::Acp(m) => {
                ...
                cora_cowork_common::normalize_keys_to_snake_case(&mut value);
                Ok(Some(value))
            }
            Self::Corars(_) => Ok(None),
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<SlashCommandItem>, AgentError> {
        match self {
            ...
            Self::Corars(m) => m.get_slash_commands().await,
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, AgentError> {
        ...
        match self {
            ...
            Self::Corars(_) => Ok(SideQuestionResponse {
                status: "unsupported".into(),
                answer: None,
            }),
            #[cfg(any(test, feature = "test-support"))]
...
#[cfg(test)]
mod corars_config_option_tests {
    use std::sync::Arc;

    use cora_cowork_api_types::ConfigOptionConfirmation;
    ...
    fn make_test_config() -> CorarsResolvedConfig {
        CorarsResolvedConfig {
            ...
            session_directory: std::env::temp_dir().join("corars-agent-task-test-sessions"),
            session_mode: None,
            skills: Vec::new(),
    ...
    async fn corars_instance() -> AgentInstance {
        let manager = CorarsAgentManager::new("conv-corars-config".into(), "/project".into(), make_test_config(), None)
            .await
            .expect("corars manager should start in tests");
        AgentInstance::Corars(Arc::new(manager))
    }

    #[tokio::test]
    async fn corars_exposes_mode_as_config_option() {
        let instance = corars_instance().await;
        ...
        let mode = response
            ...
            .expect("corars should expose a mode config option");
    ...
    #[tokio::test]
    async fn corars_set_config_option_mode_switches_session_mode() {
        let instance = corars_instance().await;
    ...
    #[tokio::test]
    async fn corars_set_config_option_rejects_invalid_mode() {
        let instance = corars_instance().await;
    ...
    #[tokio::test]
    async fn corars_set_config_option_rejects_unavailable_option() {
        let instance = corars_instance().await;
