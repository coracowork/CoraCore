...
pub(crate) fn normalize_requested_mode(metadata: &AgentMetadata, mode: &str) -> String {
    ...
    // CoraCowork persists the legacy aliases `yolo` / `yoloNoSandbox` while
...
mod tests {
    use super::*;
    use cora_cowork_api_types::AgentHandshake;
    use cora_cowork_common::AgentType;

    fn metadata_with_yolo_id(yolo_id: Option<&str>) -> AgentMetadata {
        use cora_cowork_api_types::{AgentSource, AgentSourceInfo, BehaviorPolicy};
        AgentMetadata {
            id: "test".into(),
            icon: None,
            name: "Test".into(),
    ...
    fn uses_meta_resume_true_when_policy_flag_set() {
        use cora_cowork_api_types::BehaviorPolicy;
