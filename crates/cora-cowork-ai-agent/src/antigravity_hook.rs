//! Writes the `hooks.json` that registers CoraCowork's own binary as agy's
...
use cora_cowork_api_types::AntigravityHookConfig;
...
///
/// The `matcher: "*"` is deliberate — CoraCowork decides per call, so agy must ask
/// about all of them.
///
...
/// (it knows the workspace, not where CoraCowork's binary lives). Handing it the
...
mod tests {
    ...
    fn registers_the_bridge_without_embedding_any_policy() {
        ...
        write_hooks_json(dir.path(), Path::new("/opt/coracowork/backend")).unwrap();
        ...
        assert_eq!(
            ...
            "\"/opt/coracowork/backend\" antigravity-hook"
        );

        // The decision must never be baked into the file: CoraCowork owns it at
    ...
    fn an_install_path_with_spaces_stays_one_argument() {
        ...
        // unquoted path like /Applications/Cora UI.app/... would be split into
        // two arguments and the hook would never run.
        let dir = tempfile::tempdir().unwrap();
        write_hooks_json(dir.path(), Path::new("/Apps/Cora UI/backend")).unwrap();
        let raw = std::fs::read_to_string(dir.path().join(".agents/hooks.json")).unwrap();
        let v: serde_json::Value = serde_json::from_str(&raw).unwrap();
        assert_eq!(
            v[AntigravityHookConfig::HOOK_NAME]["PreToolUse"][0]["hooks"][0]["command"],
            "\"/Apps/Cora UI/backend\" antigravity-hook"
        );
    }
    ...
    fn removing_the_hook_clears_a_previous_install() {
        ...
        let dir = tempfile::tempdir().unwrap();
        write_hooks_json(dir.path(), std::path::Path::new("/opt/coracowork/coracore")).unwrap();
