    ...
    fn from_acp_non_internal(err: &AcpError, detail: String) -> Self {
        match err {
            ...
            AcpError::NotConnected => Self::new(
                "CoraCowork lost its Agent protocol connection",
                AgentErrorCode::UserAgentDisconnected,
                AgentErrorOwnership::UserAgent,
                Some(detail),
                true,
            ...
            let classified = acp_agent_internal_texts(message, data.as_ref())
                ...
                .find_map(|text| {
                    ...
                    classify_agent_lifecycle(&lower)
                        ...
                        .or_else(|| classify_cora_cowork_state(&lower))
...
fn classify_cora_cowork_state(lower: &str) -> Option<ClassifiedError> {
...
mod tests {
    use super::*;
    use cora_cowork_api_types::{AgentErrorResolutionKind, AgentErrorResolutionTarget};
    use serde_json::json;
    ...
    #[test]
    fn preserves_runtime_workspace_validation_as_structured_cora_cowork_error() {
        let err =
