...
use cora_cowork_runtime::Builder;
...
mod tests {
    ...
    fn the_verified_release_is_at_or_above_the_floor() {
        let verified = parse_version(cora_cowork_session::VERIFIED_CLAUDE_VERSION).expect("verified version must parse");
        assert!(
            verified >= THINKING_DISPLAY_MIN_VERSION,
            "VERIFIED_CLAUDE_VERSION {} is below the --thinking-display floor {:?}",
            cora_cowork_session::VERIFIED_CLAUDE_VERSION,
