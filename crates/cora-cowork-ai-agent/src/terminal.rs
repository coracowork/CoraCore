...
/// would hand out `cora-cowork-term-1` again — colliding with the card the
...
pub struct TerminalRegistry {
    ...
    /// coracore process's cwd (the app bundle), which is never what the
    /// agent means.
...
impl TerminalRegistry {
    ...
    pub async fn create(&self, params: CreateTerminalParams) -> Result<String, String> {
        ...
        let mut builder = if params.args.is_empty() {
            #[cfg(unix)]
            {
                let mut b = cora_cowork_runtime::Builder::new("/bin/sh");
                b.arg("-c").arg(&params.command);
                b
            }
            #[cfg(windows)]
            {
                let mut b = cora_cowork_runtime::Builder::new("cmd");
                b.arg("/C").arg(&params.command);
                b
            }
        } else {
            let mut b = cora_cowork_runtime::Builder::new(&params.command);
            b.args(&params.args);
            b
        };
        builder
        ...
        let id = format!("cora-cowork-term-{}", NEXT_TERMINAL_SEQ.fetch_add(1, Ordering::Relaxed) + 1);
    ...
    pub async fn kill(&self, terminal_id: &str, source: &str) -> bool {
        ...
        if let Some(c) = guard.as_mut() {
            ...
            let _ = cora_cowork_runtime::kill_process_tree(c).await;
            *guard = None;
...
mod tests {
    ...
    async fn default_cwd_applies_when_agent_sends_none() {
        ...
        // (`temp_dir()/cora-cowork-term-cwd-test`) was raced by every parallel test
