...
use cora_cowork_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use cora_cowork_ai_agent::manager::acp::AcpAgentManager;
use cora_cowork_ai_agent::registry::AgentRegistry;
use cora_cowork_ai_agent::{AgentInstance, AgentStreamEvent, IAgentTask};
use cora_cowork_common::ConversationStatus;
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = cora_cowork_ai_agent::AcpBuildExtra {
        agent_id: None,
    ...
    let skill_paths = std::sync::Arc::new(cora_cowork_extension::resolve_skill_paths(
        tmp_skills.path(),
        tmp_skills.path(),
    ));
    let skill_manager = cora_cowork_ai_agent::AcpSkillManager::new(skill_paths);
    ...
    let params = Arc::new(
        assemble_acp_params(
            ...
            metadata,
            cora_cowork_common::CommandSpec {
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: cora_cowork_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), cora_cowork_common::AgentType::Acp);
