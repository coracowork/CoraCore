...
use cora_cowork_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use cora_cowork_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use cora_cowork_ai_agent::manager::acp::{AcpSession, SessionNewPreludeHook};
use cora_cowork_ai_agent::registry::AgentRegistry;
use cora_cowork_ai_agent::shared_kernel::ModelId;
use cora_cowork_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntime};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(cora_cowork_extension::resolve_skill_paths(tmp.path(), tmp.path()));
