...
use cora_cowork_extension::{BUILTIN_SKILLS_ENV_VAR, resolve_skill_paths};
...
fn build_index_text_contains_load_protocol() {
    let skills = vec![
        cora_cowork_ai_agent::SkillIndex {
            name: "security".into(),
...
fn system_instructions_with_loaded_skills() {
    let skills = vec![cora_cowork_ai_agent::SkillDefinition {
        name: "helper".into(),
        description: "A helper".into(),
        location: std::path::PathBuf::new(),
        source: cora_cowork_extension::SkillSource::Custom,
...
fn first_message_with_skills_index_for_acp() {
    let skills = vec![cora_cowork_ai_agent::SkillIndex {
        name: "review".into(),
