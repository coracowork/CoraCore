...
use cora_cowork_extension::{ExtensionSource, ScanPath, filter_by_engine_compatibility, load_all};
...
fn el2_engine_incompatible_extension_filtered_out() {
    ...
    // Extension requires cora-cowork ^2.0.0 but app is 1.5.0.
...
fn el3_invalid_manifest_skipped_others_load() {
    ...
    fs::write(bad.join("cora-extension.json"), b"{ broken json").unwrap();

    // Missing required fields.
    let incomplete = create_ext_dir(tmp.path(), "incomplete");
    fs::write(
        incomplete.join("cora-extension.json"),
...
//
// Since CORA_COWORK_E2E_TEST=1 changes the behaviour of resolve_scan_paths()
