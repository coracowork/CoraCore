...
//!
//! These tests exercise the public API surface of `cora_cowork_extension::skill_service`
//! and `cora_cowork_extension::external_paths` against the functional requirements in
...
///
/// Production mode: no `CORA_COWORK_BUILTIN_SKILLS_PATH` set — the built-in
...
async fn sm11_get_skill_paths() {
    ...
    let data_dir = Path::new("/home/user/.cora-cowork");
...
async fn cp4_enable_skills_market() {
    ...
    let paths = mgr.get_custom_external_paths().await;
    assert_eq!(paths.len(), 1);
    assert_eq!(paths[0].name, "cora-cowork-skills");
}
