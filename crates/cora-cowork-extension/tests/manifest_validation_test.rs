...
//!
//! These test the public API surface of `cora_cowork_extension::manifest`.

use cora_cowork_extension::{ExtensionError, ExtensionManifest, parse_manifest, validate_manifest};
...
#[test]
fn mv2_reserved_prefix_cora_rejected() {
    let json = serde_json::json!({"name": "cora-my-ext", "version": "1.0.0"});
    let bytes = serde_json::to_vec(&json).unwrap();
    let err = parse_manifest(&bytes).unwrap_err();
    assert!(matches!(err, ExtensionError::ReservedNamePrefix { ref prefix, .. } if prefix == "cora-"));
}
