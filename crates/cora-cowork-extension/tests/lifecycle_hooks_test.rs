...
use cora_cowork_extension::{HookKind, LifecycleHooks, execute_hook, needs_install_hook, resolve_hook_path};
...
async fn lh6_missing_script_graceful() {
    ...
    match result.unwrap_err() {
        cora_cowork_extension::ExtensionError::HookNotFound(path) => {
            assert!(
...
async fn hook_nonzero_exit_returns_hook_failed() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        cora_cowork_extension::ExtensionError::HookFailed {
            extension_name,
            hook,
            reason,
        } => {
