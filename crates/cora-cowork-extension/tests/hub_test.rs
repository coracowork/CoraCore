...
use cora_cowork_extension::hub::{HubIndexManager, HubInstaller};
use cora_cowork_extension::registry::ExtensionRegistry;
use cora_cowork_extension::state::ExtensionStateStore;
use cora_cowork_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![cora_cowork_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: cora_cowork_extension::types::ExtensionSource::Env,
    }];
...
async fn verify_installation_catches_invalid_manifest() {
    ...
    std::fs::create_dir_all(&ext_dir).unwrap();
    std::fs::write(ext_dir.join("cora-extension.json"), b"not valid json").unwrap();
...
async fn verify_installation_catches_reserved_name() {
    let h = setup();
    let ext_dir = h.hub_dir.path().join("cora-bad");
    write_extension_manifest(&ext_dir, "cora-bad", "1.0.0");
