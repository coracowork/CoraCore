use cora_cowork_extension::{resolve_skill_paths, skill_service};
...
async fn materialize_returns_only_listed_skill_source_paths() {
    ...
    unsafe {
        std::env::set_var(cora_cowork_extension::BUILTIN_SKILLS_ENV_VAR, &builtin_root);
    }
    ...
    unsafe {
        std::env::remove_var(cora_cowork_extension::BUILTIN_SKILLS_ENV_VAR);
    }
}
