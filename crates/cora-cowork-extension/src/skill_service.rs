...
use cora_cowork_db::{CreateSkillImportRecordParams, ISkillRepository, SkillRow, UpsertSkillParams};
...
///
/// Mirrors the strategy used by `cora-cowork-assistant::builtin`: the corpus is
/// authoritative at build time; an optional on-disk override
/// (`CORA_COWORK_BUILTIN_SKILLS_PATH`) is consulted at runtime for rapid
/// iteration and E2E fixtures.
static BUILTIN_SKILLS: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/../cora-cowork-app/assets/builtin-skills");
...
pub const BUILTIN_SKILLS_ENV_VAR: &str = "CORA_COWORK_BUILTIN_SKILLS_PATH";
...
pub struct SkillPaths {
    /// Root data directory (~/.cora-cowork/).
    pub data_dir: PathBuf,
    /// User-created skills directory (~/.cora-cowork/skills/).
    pub user_skills_dir: PathBuf,
    /// Per-job cron skills directory (~/.cora-cowork/cron/skills/).
    ...
    /// Assistant-level rules directory (~/.cora-cowork/assistant-rules/).
    pub assistant_rules_dir: PathBuf,
    /// Assistant-level skills directory (~/.cora-cowork/assistant-skills/).
...
///
/// `data_dir` is the user-level data root (e.g. `~/.cora-cowork/`) and
...
pub async fn import_skills(paths: &SkillPaths, source_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    let operation_id = cora_cowork_common::generate_prefixed_id("skill_import_op");
...
async fn import_skills_from_zip(paths: &SkillPaths, archive_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    ...
    let result = async {
        ...
        import_skill_dirs_batch(
            paths,
            skill_dirs,
            cora_cowork_common::generate_prefixed_id("skill_import_op"),
        )
        .await
    }
    .await;
...
mod tests {
    use super::*;
    use cora_cowork_db::{ISkillRepository, SqliteSkillRepository, init_database_memory};
    use std::io::Write;
