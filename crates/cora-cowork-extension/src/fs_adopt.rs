//! Filesystem side of CoraCowork → CoraPro adoption.
//!
//! DB adoption (`adopt_system_default_data`) re-owns the local default user's
//! rows to the first external (CoraPro) user. Because per-user data now lives
...
pub async fn adopt_user_filesystem(
    ...
    let Ok(adopter_dir) = cora_cowork_common::user_dir_name(adopter_user_id) else {
...
async fn rewrite_adopted_skill_paths(
    ...
    for row in rows {
        ...
        let params = cora_cowork_db::UpsertSkillParams {
            name: &row.name,
...
mod tests {
    use super::*;
    use cora_cowork_db::{SqliteSkillRepository, UpsertSkillParams, init_database_memory};
    ...
    async fn seed_user(db: &cora_cowork_db::Database) -> String {
        use cora_cowork_db::IUserRepository;
        cora_cowork_db::SqliteUserRepository::new(db.pool().clone())
