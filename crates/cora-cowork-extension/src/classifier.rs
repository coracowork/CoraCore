...
//!
//! These traits live in `cora-cowork-extension` (not `cora-cowork-assistant`) so
//! `skill_routes` can depend on them without pulling `cora-cowork-assistant` into
//! the dependency graph; the concrete implementation ships from
//! `cora-cowork-assistant::AssistantService`.

use cora_cowork_api_types::AssistantSource;
...
///
/// Implemented by `cora_cowork_assistant::AssistantService`; depended on by
