...
/// `POST /api/skills/market/enable` — enable the cora-cowork skills market.
...
/// `POST /api/skills/market/disable` — disable the cora-cowork skills market.
...
mod tests {
    ...
    async fn make_state() -> SkillRouterState {
        ...
        let skill_repo = Arc::new(cora_cowork_db::SqliteSkillRepository::new(db.pool().clone()));
        std::mem::forget(tmp);
        SkillRouterState {
            skill_paths: paths,
            skill_repo,
