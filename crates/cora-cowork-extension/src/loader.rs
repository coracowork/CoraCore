...
/// 1. `$CORA_COWORK_EXTENSIONS_PATH`
/// 2. `~/.cora-cowork/extensions/` — legacy user data directory
/// 3. Platform AppData directory
///
/// In E2E test mode (`CORA_COWORK_E2E_TEST=1`), only the environment variable
/// paths are returned to ensure test isolation.
pub fn resolve_scan_paths() -> Vec<ScanPath> {
    let env_path = std::env::var("CORA_COWORK_EXTENSIONS_PATH").ok();
...
fn resolve_scan_paths_inner(
    ...
    // 2. User data directory (desktop data dir or historical ~/.cora-cowork fallback).
    if let Some(data_dir) = explicit_data_dir {
        ...
        if let Some(data_dir) = dirs::data_dir() {
            push(
                data_dir.join("cora-cowork").join(EXTENSIONS_DIR_NAME),
...
///
/// Extensions that declare `engine.coracore` with a version range incompatible
...
/// Check whether the extension's `engine.coracore` requirement is satisfied.
fn is_engine_compatible(ext: &LoadedExtension, app_version: &semver::Version) -> bool {
    ...
    let Some(required) = &engine.coracore else {
        return true; // no coracore constraint
    };
    ...
    match semver::VersionReq::parse(required) {
        ...
        Ok(_) => {
            warn!(
                ...
                required = %required,
                actual = %app_version,
                "extension filtered out: engine.coracore incompatible"
            );
            false
        }
        Err(e) => {
            warn!(
                ...
                required = %required,
                error = %e,
                "extension filtered out: invalid engine.coracore version requirement"
            );
            false
        }
    }
...
fn is_e2e_test_mode() -> bool {
    std::env::var("CORA_COWORK_E2E_TEST").map(|v| v == "1").unwrap_or(false)
}
...
mod tests {
    ...
    fn write_manifest_full(
        ...
        if let Some(eng) = engine_coracore {
            manifest["engine"] = serde_json::json!({ "coracore": eng });
        }
    ...
    #[test]
    fn scan_loads_cora_cowork_main_contract_extension() {
        let tmp = TempDir::new().unwrap();
    ...
    fn make_loaded_ext(
        ...
        LoadedExtension {
            manifest: ExtensionManifest {
                ...
                homepage: None,
                icon: None,
                engine: engine_coracore.map(|v| EngineConfig {
                    coracore: Some(v.to_string()),
                }),
    ...
    fn resolve_scan_paths_for_data_dir_prefers_env_then_data_dir_then_appdata() {
        ...
        let app_root = tmp.path().join("CoraUi-Dev");
        let data_dir = app_root.join("cora-cowork");
