...
//! given conversation; otherwise an corars job that runs fine
...
//! we return an empty `ProviderWithModel`; non-corars factory branches
//! ignore the field, and the corars branch surfaces a clear "provider
...
/// `provider_id: ""`, `model: ""`, `use_model: None`. Non-corars factories
/// ignore the field, while the corars factory surfaces a clear "Provider
...
mod tests {
    ...
    /// label `"corars"` when the conversation's model JSON was an older
    /// shape) and `send_message` parsed the row directly, so the cron
    /// path would emit `Provider 'corars' not found` while the
    ...
    fn interactive_and_cron_paths_agree_on_provider_id() {
        ...
        assert_ne!(canonical_resolved.provider_id, "corars");
        assert_ne!(legacy_resolved.provider_id, "corars");
    }
}
