...
use cora_cowork_api_types::{ConversationArtifactResponse, ConversationResponse, MessageResponse, MessageSearchItem};
...
use cora_cowork_db::MessageSearchRow;
use cora_cowork_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
mod tests {
    use super::*;
    use cora_cowork_common::{AgentType, ConversationSource, ConversationStatus};
    use serde_json::json;
    ...
    #[test]
    fn parse_provider_with_model_cora_cowork_format() {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("cora-cowork"),
            None,
            r#"{"workspace":"/srv/cora-cowork-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/cora-cowork-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("cora-cowork"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/cora-cowork-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        let row = MessageSearchRow {
            ...
            conversation_source: Some("cora-cowork".into()),
        ...
        assert_eq!(item.conversation.source, Some(ConversationSource::CoraCowork));
