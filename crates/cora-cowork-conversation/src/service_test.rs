...
use cora_cowork_db::{MessagePageCursor, MessagePageDirection, MessagePageParams, MessagePageResult};
use cora_cowork_extension::{AssistantRuleDispatcher, ExtensionError};
...
impl IConversationRepository for MockRepo {
    async fn get(&self, id: &str) -> Result<Option<ConversationRow>, cora_cowork_db::DbError> {
        let rows = self.rows.lock().unwrap();
        Ok(rows.iter().find(|r| r.id == id).cloned())
    }

    async fn create(&self, row: &ConversationRow) -> Result<(), cora_cowork_db::DbError> {
    ...
    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), cora_cowork_db::DbError> {
        ...
        let row = rows
            .iter_mut()
            .find(|r| r.id == id)
            .ok_or_else(|| cora_cowork_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, id: &str) -> Result<(), cora_cowork_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let len_before = rows.len();
        rows.retain(|r| r.id != id);
        if rows.len() == len_before {
            return Err(cora_cowork_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, cora_cowork_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, cora_cowork_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, cora_cowork_db::DbError> {
        Ok(vec![])
    }
    ...
    async fn delete_assistant_snapshot(&self, conversation_id: &str) -> Result<bool, cora_cowork_db::DbError> {
        let mut rows = self.assistant_snapshots.lock().unwrap();
        let before = rows.len();
        ...
        let items = match &params.direction {
            ...
            MessagePageDirection::Anchor { message_id } => {
                let anchor = matched
                    ...
                    .cloned()
                    .ok_or_else(|| cora_cowork_db::DbError::NotFound(format!("Message {message_id}")))?;
    ...
    async fn insert_message(&self, message: &MessageRow) -> Result<(), cora_cowork_db::DbError> {
        let mut messages = self.messages.lock().unwrap();
        messages.push(message.clone());
        Ok(())
    }

    async fn update_message(&self, id: &str, updates: &MessageRowUpdate) -> Result<(), cora_cowork_db::DbError> {
        let mut messages = self.messages.lock().unwrap();
        let message = messages
            .iter_mut()
            .find(|message| message.id == id)
            .ok_or_else(|| cora_cowork_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, conv_id: &str) -> Result<(), cora_cowork_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, cora_cowork_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_stale_runtime_messages(&self) -> Result<Vec<MessageRow>, cora_cowork_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .filter(|message| {
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, cora_cowork_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    ) -> Result<ConversationArtifactRow, cora_cowork_db::DbError> {
    ...
    async fn delete_artifacts_by_conversation(&self, conversation_id: &str) -> Result<(), cora_cowork_db::DbError> {
        self.artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, cora_cowork_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
fn stub_agent_metadata_rows() -> Vec<AgentMetadataRow> {
    [
        ...
        ("632f31d2", None, "corars", "Cora CLI", 200),
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("cora-cowork-conversation-service-test-project");
...
fn unique_test_workspace_path(label: &str) -> PathBuf {
    let workspace = std::env::temp_dir()
        .join(format!("cora-cowork-conversation-service-test-{label}"))
...
async fn create_assistant_backed_conversation(
    ...
    if conversation_type == Some("corars") {
        payload["model"] = json!({
...
async fn create_auto_provisions_workspace_under_date_partition() {
    ...
    let workspace_root = temp.path().join("cora-cowork-data");
...
async fn create_rejects_unavailable_workspace_with_trailing_whitespace_in_request() {
    let (svc, _broadcaster, _repo, _task_mgr) = make_service();
    let dir = std::env::temp_dir().join(format!("cora-cowork-test-{}", cora_cowork_common::generate_short_id()));
...
async fn create_stores_model_as_json() {
    ...
    // Top-level model is only valid for corars conversations.
    let req: CreateConversationRequest = serde_json::from_value(json!({
        "type": "corars",
...
#[tokio::test]
async fn create_derives_corars_type_from_assistant_backend_when_type_is_missing() {
    ...
    upsert_test_assistant_definition(
        &definition_repo,
        "asstdef_corars_missing_type",
        "assistant-corars-missing-type",
        "corars",
        "auto",
        "auto",
    )
    ...
    overlay_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_corars_missing_type",
            enabled: true,
            sort_order: 0,
    ...
    let req: CreateConversationRequest = serde_json::from_value(json!({
        "assistant": {
            "id": "assistant-corars-missing-type",
            "locale": "en-US"
        },
        "model": {
            "provider_id": "provider-1",
    ...
    let resp = svc.create("user_1", req).await.unwrap();
    assert_eq!(resp.r#type, AgentType::Corars);
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, cora_cowork_api_types::ConversationRuntimeStateKind::Idle);
...
async fn update_extra_merge() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "cora-cowork-conversation-update-extra-merge-{}",
        cora_cowork_common::generate_short_id()
    ));
...
async fn update_model() {
    ...
    // Top-level model updates are only valid on corars conversations
    // (Task 8 enforces the corars-only rule in update).
...
fn assert_conversation_runtime_context(options: &BuildTaskOptions, user_id: &str, conversation_id: &str) {
    assert!(
        options
            .context
            .runtime_env
            .contains(&("CORA_COWORK_USER_ID".to_owned(), user_id.to_owned())),
        "runtime env should include CORA_COWORK_USER_ID"
    );
    assert!(
        options
            .context
            .runtime_env
            .contains(&("CORA_COWORK_CONVERSATION_ID".to_owned(), conversation_id.to_owned())),
        "runtime env should include CORA_COWORK_CONVERSATION_ID"
    );
    assert_eq!(
...
async fn send_message_injects_configured_runtime_helper_context() {
    ...
    assert!(
        options[0]
            .context
            .runtime_env
            .contains(&(CORA_COWORK_BASE_URL_ENV.to_owned(), "http://127.0.0.1:51234".to_owned())),
        "runtime env should include CORA_COWORK_BASE_URL"
    );
}
...
#[tokio::test]
async fn update_corars_model_updates_assistant_preference_only_when_snapshot_model_mode_is_auto() {
    let task_mgr = Arc::new(MockTaskManager::new());
    let (svc, _broadcaster, repo, definition_repo, overlay_repo, preference_repo) =
        make_service_with_mock_task_manager_and_assistant_support(task_mgr.clone()).await;
    ...
    overlay_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_corars_auto",
            enabled: true,
            sort_order: 0,
            agent_id_override: None,
            last_used_at: None,
    ...
    preference_repo
        .upsert(&UpsertAssistantPreferenceParams {
            assistant_definition_id: "asstdef_corars_auto",
            last_model_id: Some("legacy-corars-model"),
    ...
    let auto_conv =
        create_assistant_backed_conversation(&svc, "user_1", Some("corars"), "corars", "assistant-corars-auto").await;
    let updated = svc
        .update(
            "user_1",
            &auto_conv.id,
    ...
    let auto_pref = preference_repo.get("asstdef_corars_auto").await.unwrap().unwrap();
    ...
    upsert_test_assistant_definition(
        &definition_repo,
        "asstdef_corars_fixed",
        "assistant-corars-fixed",
        "corars",
        "fixed",
        "auto",
    )
    ...
    overlay_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_corars_fixed",
            enabled: true,
            sort_order: 0,
            agent_id_override: None,
            last_used_at: None,
    ...
    preference_repo
        .upsert(&UpsertAssistantPreferenceParams {
            assistant_definition_id: "asstdef_corars_fixed",
            last_model_id: Some("legacy-corars-fixed-model"),
            last_permission_value: None,
            last_thought_level_value: None,
            last_skill_ids: "[]",
            last_disabled_builtin_skill_ids: "[]",
    ...
    let fixed_conv =
        create_assistant_backed_conversation(&svc, "user_1", Some("corars"), "corars", "assistant-corars-fixed").await;
    let _ = svc
        .update(
            "user_1",
            &fixed_conv.id,
    ...
    let fixed_pref = preference_repo.get("asstdef_corars_fixed").await.unwrap().unwrap();
    assert_eq!(fixed_pref.last_model_id.as_deref(), Some("legacy-corars-fixed-model"));
...
async fn send_message_missing_workspace_persists_message_and_failure_tip() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    broadcaster.take_events();
    let legacy_workspace = format!("/tmp/does-not-exist-{}", cora_cowork_common::generate_short_id());
    repo.update(
        &conv.id,
        &ConversationRowUpdate {
...
async fn auto_replay_rebuild_keeps_existing_acp_session_id_in_build_options() {
    ...
    for options in options {
        match options.context.kind {
            ...
            AgentSessionKind::Corars(_) => panic!("test conversation should build ACP options"),
        }
    }
}
...
async fn cancel_timeout_does_not_kill_non_acp_task() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    let agent = Arc::new(BlockingCancelAgent::new_with_type(&conv.id, AgentType::Corars));
    task_mgr.insert_agent(&conv.id, AgentInstance::Mock(agent));
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = cora_cowork_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
async fn create_resolves_assistant_snapshot_and_updates_preferences() {
    ...
    assert_eq!(
        resp.assistant,
        Some(cora_cowork_api_types::ConversationAssistantIdentityResponse {
            id: "preset-1".into(),
            source: "builtin".into(),
            name: "Preset".into(),
            avatar: "🤖".into(),
...
#[tokio::test]
async fn assistant_backed_corars_build_options_include_snapshot_rule_as_preset_rules() {
    let resolver = Arc::new(FixedSkillResolver { names: vec![] });
    let dispatcher = Arc::new(StaticAssistantDispatcher {
        rules: std::collections::HashMap::from([("preset-corars-rule".to_string(), "assistant rule body".to_string())]),
    });
    let (svc, _broadcaster, repo, definition_repo, state_repo, _preference_repo) =
        make_service_with_assistant_support(resolver, dispatcher).await;
    ...
    state_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_preset_corars_rule",
            enabled: true,
            sort_order: 0,
            agent_id_override: None,
            last_used_at: None,
    ...
    let conv =
        create_assistant_backed_conversation(&svc, "user_1", Some("corars"), "corars", "preset-corars-rule").await;
    let row = repo.get(&conv.id).await.unwrap().unwrap();
    let options = svc.build_task_options(&row).await.unwrap();

    match options.context.kind {
        AgentSessionKind::Acp(_) => panic!("test conversation should build Corars options"),
        AgentSessionKind::Corars(ctx) => {
...
async fn warmup_restores_skill_links_for_recreated_auto_workspace() {
    ...
    assert!(workspace.join(".corars/skills/cron").is_dir());
    ...
    assert_eq!(calls.len(), 1);
    assert_eq!(calls[0].workspace, workspace);
    assert_eq!(calls[0].rel_dirs, vec![".corars/skills"]);
    assert_eq!(calls[0].skill_names, vec!["cron"]);
}
...
// ── corars rebuild permission seed (Sentry 135525584) ──────────────

/// Inserts an corars conversation whose `extra.session_mode` is the create-time
...
async fn seed_corars_conversation_with_snapshot(
    repo: &Arc<MockRepo>,
    session_mode: &str,
    default_permission_mode: &str,
    ...
    let row = ConversationRow {
        id: format!("corars-seed-{}", cora_cowork_common::generate_short_id()),
        user_id: "user_1".into(),
        name: "corars seed".into(),
        r#type: "corars".into(),
        extra: json!({
...
fn corars_session_mode(options: &BuildTaskOptions) -> Option<String> {
    match &options.context.kind {
        AgentSessionKind::Corars(ctx) => ctx.config.session_mode.clone(),
        AgentSessionKind::Acp(_) => panic!("expected Corars build options"),
    }
}

#[tokio::test]
async fn corars_rebuild_auto_mode_preserves_runtime_yolo() {
    // AC#1: an `auto` corars session that was switched to yolo at runtime must
    // keep yolo after a rebuild (model switch / agent restart).
    let (svc, _broadcaster, repo, _task_mgr) = make_service();
    let row = seed_corars_conversation_with_snapshot(&repo, "default", "auto", Some("yolo")).await;

    let options = svc.build_task_options(&row).await.unwrap();

    assert_eq!(corars_session_mode(&options).as_deref(), Some("yolo"));
}

#[tokio::test]
async fn corars_rebuild_existing_data_auto_overrides_create_time_seed() {
    // AC#2: existing data — create-time non-yolo seed is overridden by the
    // authoritative resolved runtime value.
    let (svc, _broadcaster, repo, _task_mgr) = make_service();
    let row = seed_corars_conversation_with_snapshot(&repo, "auto_edit", "auto", Some("yolo")).await;
...
#[tokio::test]
async fn corars_rebuild_fixed_mode_blocks_runtime_escalation() {
    ...
    let (svc, _broadcaster, repo, _task_mgr) = make_service();
    let row = seed_corars_conversation_with_snapshot(&repo, "default", "fixed", Some("yolo")).await;

    let options = svc.build_task_options(&row).await.unwrap();

    assert_eq!(corars_session_mode(&options).as_deref(), Some("default"));
}
...
async fn cron_required_runtime_mode_wins_over_resolved_permission_seed() {
    ...
    // an `auto` corars session whose snapshot resolved value is yolo, a cron
