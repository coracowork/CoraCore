...
use cora_cowork_ai_agent::protocol::events::{ErrorEventData, TipType};
use cora_cowork_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use cora_cowork_api_types::{AgentErrorCode, WebSocketMessage};
use cora_cowork_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
...
use cora_cowork_db::IConversationRepository;
...
mod tests {
    ...
    use cora_cowork_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    ...
    async fn needs_auth_tip_sets_summary_flag_on_finish() {
        use cora_cowork_ai_agent::protocol::events::{TipType, TipsEventData};
    ...
    async fn run_warning_tip_with_finish_persists_warning_tip() {
        ...
        tx.send(AgentStreamEvent::Tips(
            cora_cowork_ai_agent::protocol::events::TipsEventData {
                content: String::new(),
                tip_type: cora_cowork_ai_agent::protocol::events::TipType::Warning,
                code: Some("ACP_EMPTY_TURN".into()),
                params: None,
            },
        ))
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(cora_cowork_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use cora_cowork_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use cora_cowork_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(cora_cowork_realtime::BroadcastEventBus::new(64));
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, cora_cowork_db::MessageRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, cora_cowork_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _id: &str) -> Result<Option<cora_cowork_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn create(&self, _row: &cora_cowork_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(&self, _id: &str, _updates: &cora_cowork_db::ConversationRowUpdate) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &cora_cowork_db::ConversationFilters,
        ) -> Result<cora_cowork_common::PaginatedResult<cora_cowork_db::models::ConversationRow>, DbError> {
            Ok(cora_cowork_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<cora_cowork_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<cora_cowork_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            &self,
            _conv_id: &str,
            _params: &cora_cowork_db::MessagePageParams,
        ) -> Result<cora_cowork_db::MessagePageResult, DbError> {
            Ok(cora_cowork_db::MessagePageResult {
                items: vec![],
        ...
        async fn upsert_message(&self, row: &MessageRow) -> Result<(), DbError> {
            ...
            if let Some(existing) = inserts.iter().find(|message| message.id == row.id) {
                ...
                self.updates.lock().unwrap().push((
                    row.id.clone(),
                    cora_cowork_db::MessageRowUpdate {
        ...
        async fn update_message(&self, id: &str, updates: &cora_cowork_db::MessageRowUpdate) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<cora_cowork_common::PaginatedResult<cora_cowork_db::MessageSearchRow>, DbError> {
            Ok(cora_cowork_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
