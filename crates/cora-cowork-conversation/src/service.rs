...
use cora_cowork_ai_agent::session_context::{AgentSessionContext, AgentSessionKind};
...
use cora_cowork_db::models::{AssistantDefinitionRow, ConversationAssistantSnapshotRow, ConversationRow, MessageRow};
use cora_cowork_db::{
...
fn assistant_snapshot_modes<'a>(
    ...
    definition: &'a cora_cowork_db::AssistantDefinitionRow,
...
impl ConversationService {
    ...
    /// `delete()` removes the conversation row. Used by `cora-cowork-app` to wire up `WorkerTaskManagerImpl`
...
impl ConversationService {
    ...
    async fn assistant_identity_from_snapshot(
    ...
    ) -> Result<cora_cowork_api_types::ConversationAssistantIdentityResponse, ConversationError> {
        ...
        Ok(cora_cowork_api_types::ConversationAssistantIdentityResponse {
    ...
    /// to `cora-cowork`, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        // Type-aware rule: top-level `model` is corars-only. Other agent types
        ...
        if effective_type != AgentType::Corars && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "top-level `model` is only accepted for corars conversations; pass model via `extra` for {}",
                    effective_type.serde_name()
                ),
            });
        }

        // corars source-of-truth rule: top-level `model` wins. If an older client
        // still packs `extra.model`, strip it before persist so the stored row
        // has a single canonical model representation.
        if effective_type == AgentType::Corars
            && let Some(obj) = extra.as_object_mut()
            && obj.remove("model").is_some()
        {
            warn!("corars create: stripped legacy `extra.model`; top-level `model` is canonical");
        }
        ...
        let row = cora_cowork_db::models::ConversationRow {
            id: id.clone(),
        ...
        if let Some(snapshot) = assistant_snapshot.as_ref() {
            response.assistant = Some(cora_cowork_api_types::ConversationAssistantIdentityResponse {
                id: snapshot.assistant_id.clone(),
    ...
    async fn persist_assistant_preferences_from_snapshot(
        ...
        preference_repo
            .upsert(&cora_cowork_db::UpsertAssistantPreferenceParams {
                assistant_definition_id: &snapshot.assistant_definition_id,
    ...
    pub async fn update(
        ...
        // Type-aware rule: top-level `model` is corars-only. For non-corars
        // conversations, model/mode must be updated via `extra` (see spec
        // 2026-05-12).
        if existing_type != AgentType::Corars && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "top-level `model` is only accepted for corars conversations; pass model via `extra` for {}",
                    existing.r#type
        ...
        // Merge extra if provided. For corars, strip `extra.model` post-merge
        ...
        let merged_extra = if let Some(new_extra) = &req.extra {
            ...
            if existing_type == AgentType::Corars
                && let Some(obj) = existing_extra.as_object_mut()
                && obj.remove("model").is_some()
            {
                warn!("corars update: stripped legacy `extra.model` from merged extra");
            }
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        let user_msg = cora_cowork_db::models::MessageRow {
...
impl ConversationService {
    ...
    /// workspaces are constrained to CoraUi's generated path; custom
...
/// name (e.g. `"corars"`). Falls back to the agent type's serde name
...
impl ConversationService {
    async fn resolve_mcp_support_policy(
        ...
        match agent_type {
            ...
            AgentType::Corars => Ok(McpSupportPolicy::CORARS),
            _ => Ok(McpSupportPolicy::CORARS),
        }
    }
}
...
mod tests {
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::CoraCowork).unwrap(), "cora-cowork");
    ...
    fn response_with_type(agent_type: cora_cowork_common::AgentType) -> ConversationResponse {
    ...
    #[test]
    fn assistant_lineage_extracts_corars_preset_id() {
        use cora_cowork_common::AgentType;
        let response = response_with_type(AgentType::Corars);
        let extra = json!({ "preset_assistant_id": "preset-xyz" });
        let lineage = AssistantLineage::from_response_and_extra(&response, &extra);
        assert_eq!(lineage.agent_type, "corars");
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_cora_cowork_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
            McpSupportPolicy::CORARS,
        );
