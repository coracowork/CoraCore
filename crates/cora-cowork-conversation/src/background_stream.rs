...
use cora_cowork_ai_agent::protocol::events::MessageLifecyclePhase;
use cora_cowork_ai_agent::protocol::events::{AgentStreamEvent, WorkflowProgressData};
use cora_cowork_common::{ErrorChain, normalize_keys_to_snake_case};
...
impl BackgroundStreamWatcher {
    ...
    fn forward_card_frames(&self, data: &WorkflowProgressData) {
        for (kind, msg_id, body) in [
            ...
            self.broadcaster.broadcast(cora_cowork_api_types::WebSocketMessage::new(
                "message.stream",
                json!({
                    "conversation_id": self.conversation_id,
...
mod tests {
    ...
    use cora_cowork_ai_agent::protocol::events::{FinishEventData, MessageLifecycleData, TextEventData};
    ...
    struct Rig {
        ...
        bus: Arc<cora_cowork_realtime::BroadcastEventBus>,
    ...
    async fn rows_of_type(rig: &Rig, ty: &str) -> Vec<cora_cowork_db::models::MessageRow> {
        rig.repo
            .list_messages_page(
                &rig.user_id,
                "conv-1",
                &cora_cowork_db::MessagePageParams {
                    limit: 100,
                    direction: cora_cowork_db::MessagePageDirection::InitialLatest,
                },
            )
            .await
            .unwrap()
    ...
    async fn lifecycle_started_flips_pending_user_message_and_broadcasts() {
        ...
        rig.repo
            .insert_message(
                &rig.user_id,
                &cora_cowork_db::models::MessageRow {
                    id: "um-1".into(),
                    conversation_id: "conv-1".into(),
    ...
    async fn between_turns_config_option_snapshot_is_forwarded() {
        ...
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(3);
        let mut forwarded: Option<cora_cowork_api_types::WebSocketMessage<serde_json::Value>> = None;
