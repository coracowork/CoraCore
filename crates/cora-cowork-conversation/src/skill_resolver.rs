...
/// Production adapter backed by `cora_cowork_extension::skill_service`.
...
impl ExtensionSkillResolver {
    pub fn new(paths: Arc<cora_cowork_extension::SkillPaths>, skill_repo: Arc<dyn ISkillRepository>) -> Self {
...
impl SkillResolver for ExtensionSkillResolver {
    async fn auto_inject_names(&self) -> Vec<String> {
        match cora_cowork_extension::list_available_skills_with_repo(&self.paths, self.skill_repo.as_ref()).await {
            Ok(items) => {
                let mut names: Vec<String> = items
                    .into_iter()
                    .filter(|item| {
                        item.source == cora_cowork_extension::SkillSource::Builtin
                            && item
                                .relative_location
                                .as_deref()
    ...
    async fn resolve_skills(&self, names: &[String]) -> Vec<ResolvedAgentSkill> {
        ...
        match cora_cowork_extension::materialize_skills_for_agent_with_repo(
            &self.paths,
    ...
    async fn link_workspace_skills(&self, workspace: &Path, rel_dirs: &[&str], skills: &[ResolvedAgentSkill]) -> usize {
        ...
        match cora_cowork_extension::link_workspace_skills(workspace, rel_dirs, skills).await {
            Ok(n) => n,
            Err(e) => {
                tracing::warn!(
...
mod tests {
    use super::*;
    use cora_cowork_db::SqliteSkillRepository;
    ...
    async fn extension_resolver_reads_auto_inject_names_from_skill_catalog() {
        ...
        let paths = Arc::new(cora_cowork_extension::SkillPaths {
        ...
        cora_cowork_extension::sync_skill_catalog_into_repo(paths.as_ref(), repo.as_ref())
            .await
            .unwrap();
