...
use cora_cowork_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_prefixed_id, now_ms};
...
use cora_cowork_db::{IConversationRepository, SqliteConversationRepository, init_database_memory};
...
fn ensure_test_workspace_path() -> String {
    ensure_named_workspace_path("cora-cowork-conversation-extended-test-project")
}
...
async fn t9_6_search_result_includes_conversation_model() {
    ...
    // Search surfaces conversation.model only for corars (the only type that
    // carries a top-level model under the corars-only rule).
    let corars_req: CreateConversationRequest = serde_json::from_value(json!({
        "type": "corars",
        "model": { "provider_id": "p1", "model": "claude-sonnet-4-20250514" },
        "extra": { "workspace": workspace }
    }))
    .unwrap();
    let conv = svc.create(USER_ID, corars_req).await.unwrap();
...
async fn t10_1_same_workspace() {
    let (svc, _repo, _b) = setup().await;
    let shared_workspace = ensure_named_workspace_path("cora-cowork-conversation-extended-shared-workspace");
    let other_workspace = ensure_named_workspace_path("cora-cowork-conversation-extended-other-workspace");
...
async fn t10_2_no_associated() {
    let (svc, _repo, _b) = setup().await;
    let workspace = ensure_named_workspace_path("cora-cowork-conversation-extended-unique-workspace");
