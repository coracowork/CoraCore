...
use cora_cowork_common::{AgentKillReason, AgentType, ConversationSource, ConversationStatus, TimestampMs};
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("cora-cowork-conversation-crud-test-project");
...
async fn t1_1_create_with_defaults() {
    ...
    // Non-corars: top-level model is None.
...
async fn t1_2_create_each_agent_type() {
    ...
    let types = vec![("acp", AgentType::Acp), ("corars", AgentType::Corars)];

    for (type_str, expected_type) in types {
        let body = if type_str == "corars" {
            json!({
                "type": type_str,
        ...
        if type_str == "corars" {
            assert!(resp.model.is_some(), "corars should keep top-level model");
        } else {
...
async fn t4_4_extra_merge_preserves_existing_keys() {
    let (svc, _, task_mgr) = setup().await;
    let dir = std::env::temp_dir().join("cora-cowork-conversation-crud-extra-merge");
...
async fn full_lifecycle_create_get_update_delete() {
    let (svc, broadcaster, task_mgr) = setup().await;
    let updated_workspace = std::env::temp_dir().join("cora-cowork-conversation-crud-updated-workspace");
...
#[tokio::test]
async fn create_accepts_top_level_model_for_corars() {
    let (svc, _, _task_mgr) = setup().await;
    ...
    let model = resp.model.expect("corars response should carry top-level model");
    assert_eq!(model.provider_id, "p1");
    assert_eq!(model.model, "gpt-4o");
}

#[tokio::test]
async fn create_corars_strips_extra_model_field() {
    let (svc, _, _task_mgr) = setup().await;
    let workspace = ensure_test_workspace_path();
    ...
    assert!(
        ...
        "corars create must strip extra.model to avoid dual source of truth; got {:?}",
        resp.extra
    );
...
#[tokio::test]
async fn update_accepts_top_level_model_for_corars() {
    let (svc, _, task_mgr) = setup().await;
...
#[tokio::test]
async fn update_corars_strips_extra_model_from_patch() {
    let (svc, _, task_mgr) = setup().await;
    ...
    // Client mistakenly sends extra.model on an corars PATCH. It should be
    ...
    assert!(
        ...
        "corars PATCH must strip extra.model; got {:?}",
        updated.extra
    );
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use cora_cowork_db::{SqliteAcpSessionRepository, init_database_memory};

    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(cora_cowork_db::SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let agent_metadata_repo: Arc<dyn cora_cowork_db::IAgentMetadataRepository> =
        Arc::new(cora_cowork_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn cora_cowork_db::IAcpSessionRepository> =
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = cora_cowork_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
