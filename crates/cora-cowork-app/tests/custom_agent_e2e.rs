...
//!
//! Gates the probe with CORA_COWORK_BYPASS_PROBE for happy paths so CI does not
//! need an ACP CLI installed. Unset bypass for the single test that
//! verifies the CLI-not-found path.
//!
//! Thread safety: `CORA_COWORK_BYPASS_PROBE` is a process-wide env var.  Tests
...
//
// Any test that reads or writes CORA_COWORK_BYPASS_PROBE must hold this lock for
...
async fn custom_agent_full_roundtrip() {
    ...
    unsafe {
        std::env::set_var("CORA_COWORK_BYPASS_PROBE", "1");
    }
    ...
    unsafe {
        std::env::remove_var("CORA_COWORK_BYPASS_PROBE");
    }
}
...
// probe, so they do not need CORA_COWORK_BYPASS_PROBE and do not need the lock.
...
async fn test_on_save_cli_not_found_blocks_upsert() {
    // Hold ENV_MUTEX to guarantee CORA_COWORK_BYPASS_PROBE is unset for the
    ...
    let (status, json) = create_agent(
        ...
        json!({
            "name": "bad",
            "command": "cora-cowork-definitely-nonexistent-xyz"
        }),
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
