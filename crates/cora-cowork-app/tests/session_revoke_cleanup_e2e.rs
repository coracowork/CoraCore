...
//! services — revokes an CoraPro session over HTTP, and asserts observable
...
fn bootstrap_json(method: &str, uri: &str, body: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("x-coracore-bootstrap-secret", BOOTSTRAP)
...
async fn http_revoke_runs_the_real_cleanup_hook_end_to_end() {
    ...
    // Provision an CoraPro user + session over the internal API.
    let resp = app
        .clone()
        .oneshot(bootstrap_json(
            "PUT",
            "/api/auth/internal/external-users/revoke-e2e",
            r#"{"user_type":"corapro","username":"Revoke E2E"}"#,
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(bootstrap_json(
            ...
            r#"{"user_type":"corapro","external_user_id":"revoke-e2e"}"#,
        ))
        .await
        .unwrap();
