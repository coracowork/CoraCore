//! Shared test helpers for cora-cowork-app E2E tests.
#![allow(dead_code)]
...
use cora_cowork_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use cora_cowork_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use cora_cowork_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use cora_cowork_file::FileService;
use cora_cowork_system::VersionCheckService;
...
pub async fn sync_skill_catalog_for_test(services: &AppServices, paths: &SkillPaths) {
    let repo = cora_cowork_db::SqliteSkillRepository::new(services.database.pool().clone());
    cora_cowork_extension::sync_skill_catalog_into_repo(paths, &repo)
        .await
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await.expect("build module states");
    states.shell.shell_service = std::sync::Arc::new(cora_cowork_shell::ShellService::new(std::sync::Arc::new(
        cora_cowork_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                cora_cowork_ai_agent::types::BuildTaskOptions,
            )
                -> futures_util::future::BoxFuture<'static, Result<AgentInstance, cora_cowork_ai_agent::AgentError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn cora_cowork_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> cora_cowork_common::AgentType {
        cora_cowork_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<cora_cowork_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> cora_cowork_common::TimestampMs {
        cora_cowork_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<cora_cowork_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: cora_cowork_ai_agent::types::SendMessageData,
    ) -> Result<(), cora_cowork_ai_agent::AgentSendError> {
        Ok(())
    }
...
impl IMockAgent for NoopMockAgent {
    async fn get_config_options(
        &self,
    ) -> Result<cora_cowork_api_types::GetConfigOptionsResponse, cora_cowork_ai_agent::AgentError> {
        Ok(cora_cowork_api_types::GetConfigOptionsResponse {
            config_options: vec![cora_cowork_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                ...
                options: vec![cora_cowork_api_types::AcpConfigSelectOptionDto {
    ...
    async fn set_config_option(
        ...
        _option_id: &str,
        _value: &str,
    ) -> Result<cora_cowork_api_types::SetConfigOptionResponse, cora_cowork_ai_agent::AgentError> {
        Ok(cora_cowork_api_types::SetConfigOptionResponse {
            confirmation: cora_cowork_api_types::ConfigOptionConfirmation::Observed,
            config_options: Some(vec![cora_cowork_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                name: Some("Model".to_owned()),
                label: None,
                description: None,
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("cora-cowork-csrf-token="))
        .map(|s| {
            s.strip_prefix("cora-cowork-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub async fn setup_and_login(
    ...
    let hash = cora_cowork_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
