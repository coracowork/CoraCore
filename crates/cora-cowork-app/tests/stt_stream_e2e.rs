...
//! mirroring `cora-cowork-shell/tests/stt_stream_deepgram_integration.rs`).
//!
...
async fn seed_stt_prefs(app: &TestApp, config: Value) {
    let repo = Arc::new(cora_cowork_db::SqliteClientPreferenceRepository::new(
        app.services.database.pool().clone(),
    ));
    let service = cora_cowork_system::ClientPrefService::new(repo);
    let mut req = cora_cowork_api_types::UpdateClientPreferencesRequest::new();
...
// ---------------------------------------------------------------------------
// Mock Deepgram live server (minimal copy of the cora-cowork-shell integration
