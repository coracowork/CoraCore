...
async fn second_start_with_same_version_is_noop() {
    ...
    let first =
        cora_cowork_extension::materialize_if_needed(data_dir, cora_cowork_extension::builtin_skills_corpus(), "test-1.0.0")
            .await
            .unwrap();
...
async fn version_bump_triggers_rewrite() {
    ...
    let second =
        cora_cowork_extension::materialize_if_needed(data_dir, cora_cowork_extension::builtin_skills_corpus(), "test-2.0.0")
            .await
            .unwrap();
