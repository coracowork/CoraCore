...
use cora_cowork_db::SqliteProviderRepository;
...
async fn upgrade_from_pre_user_scope_keeps_encryption_key() {
    ...
    // ── Phase 1: a genuine pre-user-scope (≤028) database ────────────────
    {
        ...
        let mut migrator: Migrator = sqlx::migrate!("../cora-cowork-db/migrations");
        ...
        let key = cora_cowork_app::derive_encryption_key("legacy-secret-0123456789");
        let enc = cora_cowork_common::encrypt_string("sk-upgrade-SECRET", &key).unwrap();
        sqlx::query(
    ...
    let db = cora_cowork_db::init_database(&db_path).await.unwrap();
