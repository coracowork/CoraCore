...
//! - WP-4: Word preview start when corecli not available
//!
//! Items requiring real corecli or mock HTTP backends (WP-1..3, WP-5..6, EP-1..2,
//! PP-1..3, RP-1/RP-3, RP-5..7, DC-5..8) are tested at the service
//! integration level in `cora-cowork-office/tests/`.
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use cora_cowork_office::error::OfficeError;
    use cora_cowork_office::types::DocType;
    use cora_cowork_office::{ProcessHandle, ProcessSpawner};
...
// ── WP-4: Word preview start (corecli not available) ───────────────

#[tokio::test]
async fn wp4_word_preview_corecli_not_available() {
    ...
    assert!(url.is_empty(), "url should be empty when corecli unavailable");
    assert_eq!(json["data"]["error"], "CORECLI_INSTALL_FAILED");
}
