...
//! auth routes) via `cora_cowork_app::create_router`, covering test-plan items
...
fn extract_session_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .iter()
        .filter_map(|v| v.to_str().ok())
        .find(|s| s.starts_with("cora-cowork-session="))
        .and_then(|s| {
            let value = s.strip_prefix("cora-cowork-session=")?.split(';').next()?.to_owned();
...
fn get_with_cookie(uri: &str, token: &str) -> Request<Body> {
    Request::builder()
        .method("GET")
        .uri(uri)
        .header("cookie", format!("cora-cowork-session={token}"))
        .body(Body::empty())
        .unwrap()
}
...
async fn t12_3_session_cookie_attributes() {
    ...
    let hash = cora_cowork_auth::hash_password("StrongP@ss1").unwrap();
...
async fn t13_1_authorization_header_takes_priority() {
    ...
    let req = Request::builder()
        ...
        .uri("/api/auth/user")
        .header("authorization", format!("Bearer {token}"))
        .header("cookie", "cora-cowork-session=invalid_token")
        .body(Body::empty())
        .unwrap();
    let resp = app.oneshot(req).await.unwrap();
...
async fn t14_2_setup_then_login() {
    ...
    let hash = cora_cowork_auth::hash_password("Admin@Pass1").unwrap();
    services
        .user_repo
        .set_system_user_credentials("admin", &hash)
        .await
...
async fn full_auth_flow_e2e() {
    ...
    // 2. Setup user
    let hash = cora_cowork_auth::hash_password("Initial@Pass1").unwrap();
    services
        .user_repo
        .set_system_user_credentials("admin", &hash)
        .await
