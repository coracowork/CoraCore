...
//! against a real `cora_cowork_app::create_router_with_states` instance backed by
...
impl AssistantAgentCatalogPort for TestAgentCatalog {
    async fn list_management_agents(&self) -> Result<Vec<AgentManagementRow>, cora_cowork_assistant::AssistantError> {
        Ok(self.rows.clone())
    }
}
...
fn test_agent_row(id: &str, backend: Option<&str>, agent_type: AgentType, name: &str) -> AgentManagementRow {
    AgentManagementRow {
        ...
        agent_source: match agent_type {
            AgentType::Corars => AgentSource::Internal,
...
async fn fixture() -> Fixture {
    ...
    // `build_module_states` point at `~/.cora-cowork/` for the state store and
    ...
    // `~/.cora-cowork/` for user data — neither is appropriate for tests.
    ...
    // an explicit `agent_id` resolve to `"corars"` instead of
    // erroring out — mirroring a configured production setup.
    provider_repo
        .create(cora_cowork_db::CreateProviderParams {
            id: None,
            platform: "openai",
            name: "Test OpenAI",
        ...
        cora_cowork_assistant::service::AssistantServiceDeps {
            ...
            agent_catalog: Some(Arc::new(TestAgentCatalog {
                rows: vec![
                    ...
                    test_agent_row("632f31d2", None, AgentType::Corars, "Cora CLI"),
                ],
            })),
        },
        user_data_dir.clone(),
