//! CoraPro-mode auth for the in-conversation helper CLI (`coracore config`).
//!
...
use cora_cowork_ai_agent::{RuntimeTokenScope, TEAM_RUNTIME_TOKEN_SESSION_GENERATION};

const CONVERSATION_ID: &str = "conv-helper-auth";

async fn build_corapro_app() -> (axum::Router, cora_cowork_app::AppServices, String) {
    ...
    let services = cora_cowork_app::AppServices::from_config(db, &config).await.unwrap();
    let router = cora_cowork_app::create_router(&services).await.expect("build router");
    ...
    let provision_response = router
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(r#"{"user_type":"corapro","username":"Helper Pro User"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
...
fn helper_get(path: &str, user_id: &str, conversation_id: &str, token: Option<&str>) -> Request<Body> {
    let mut builder = Request::builder()
        ...
        .uri(path)
        .header("content-type", "application/json")
        .header("x-cora-cowork-user-id", user_id)
        .header("x-cora-cowork-conversation-id", conversation_id);
    if let Some(token) = token {
        builder = builder.header("x-cora-cowork-runtime-token", token);
    }
...
async fn helper_token_channel_passes_auth_and_reaches_cron_domain() {
    let (app, services, user_id) = build_corapro_app().await;
...
#[tokio::test]
async fn helper_without_token_is_rejected_in_corapro_mode() {
    let (app, services, user_id) = build_corapro_app().await;
    ...
    let resp = app
        .oneshot(
            Request::builder()
                ...
                .header("x-cora-cowork-user-id", &user_id)
                .header("x-cora-cowork-conversation-id", CONVERSATION_ID)
                .header("x-cora-cowork-runtime-token", &issue.token)
                .body(Body::from("{}"))
                .unwrap(),
        )
        .await
