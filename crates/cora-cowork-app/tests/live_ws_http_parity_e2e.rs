...
//!
//! ```sh
//! cargo test -p cora-cowork-app --test live_ws_http_parity_e2e -- --ignored --nocapture
//! ```
...
use cora_cowork_app::{AppConfig, AppServices, create_router};
...
async fn start_live_app() -> LiveApp {
    start_live_app_on(cora_cowork_db::init_database_memory().await.unwrap(), true).await
}
...
/// it again.
async fn start_live_app_on(db: cora_cowork_db::Database, create_user: bool) -> LiveApp {
    ...
    let services = AppServices::from_config_with_backend_binary_path(
        db,
        &config,
        std::path::PathBuf::from(env!("CARGO_BIN_EXE_coracore")),
    )
    .await
    .unwrap();
...
async fn run_backend_parity(backend: &str, prompt: &str) {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-parity-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    std::fs::write(ws_dir.join("hello.txt"), "CORA_PARITY_42\n").unwrap();
...
async fn run_backend_cancel(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-cancel-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_usage(backend: &str, expects_window_size: bool) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-usage-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_set_model(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-model-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_session_title(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-title-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_resume(backend: &str) {
    ...
    let secret = format!("CORA-{}", cora_cowork_common::now_ms() % 100_000);

    let ws_dir = std::env::temp_dir().join(format!("live-resume-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_mcp_provisioning(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-mcp-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    ...
    let created_server = http_json(
        ...
        "/api/mcp/servers",
        json!({
            "name": format!("live-probe-{}", cora_cowork_common::now_ms()),
            "description": "unlaunchable on purpose — proves a failure is reported, not swallowed",
            "transport": {"type": "stdio", "command": "cora-cowork-no-such-mcp-server", "args": []}
        }),
    )
    .await;
...
async fn run_direct_backend_team_mcp_and_runtime_env(backend: &str, agent_id: &str) {
    const SENTINEL: &str = "CORA_COWORK_DIRECT_CLI_E2E_42";
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "cora_cowork_app=info,cora_cowork_ai_agent=info,cora_cowork_session=info,cora_cowork_team=info",
        ))
        .with_test_writer()
        .try_init();
    assert_eq!(
        std::env::var("CORA_COWORK_E2E_SENTINEL").as_deref(),
        Ok(SENTINEL),
        "run this ignored release gate with CORA_COWORK_E2E_SENTINEL={SENTINEL}"
    );

    let app = start_live_app().await;
    let suffix = cora_cowork_common::now_ms();
    ...
    let shell_frames = drive_and_collect_from_uri(
        ...
        "Now use your real shell/command-execution tool to run exactly: \
         printf '{}\\n' | \"$CORA_COWORK_HELPER_BIN\" team members >/dev/null && \
         test \"$CORA_COWORK_E2E_SENTINEL\" = \"CORA_COWORK_DIRECT_CLI_E2E_42\" && echo CORA_COWORK_ENV_OK. \
         Only after the Team CLI fallback helper and sentinel check both succeed, reply with exactly: CORA_COWORK_ENV_OK",
        300,
    )
    .await;
    ...
    let mcp_prompt = "Call the team_members MCP tool from the cora-cowork-team MCP server now. \
         Do not use CORA_COWORK_HELPER_BIN or any CLI fallback for that call. \
    ...
    let shell_reply = shell_frames
        ...
        .collect::<String>();
    assert!(
        shell_reply.contains("CORA_COWORK_ENV_OK"),
...
async fn run_backend_permission_prompt(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-perm-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    ...
    let outside = std::env::temp_dir().join(format!("cora-approval-{}.txt", cora_cowork_common::now_ms()));
    let frames = connect_ws_recorder(app.addr, &app.token).await;
    http_json(
        &app,
        "POST",
        ...
        json!({
            "content": format!(
                "Run the shell command `echo CORA_APPROVED > {}` using your command-execution tool. \
...
async fn conversation_for(app: &LiveApp, backend: &str, label: &str) -> String {
    let ws_dir = std::env::temp_dir().join(format!("live-{label}-{backend}-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    let created = http_json(
        app,
        "POST",
...
async fn live_claude_ask_user_question_round_trip() {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-ask-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_codex_full_access_writes_without_approval() {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-fullaccess-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    ...
    let sent = http_json(
        ...
        json!({
            "content": "Create a file named written_by_agent.txt in this workspace \
                containing exactly CORA_FULL_ACCESS_OK, then reply DONE."
        }),
    )
    .await;
    assert!(sent["data"]["turn_id"].is_string(), "send failed: {sent}");
    ...
    assert!(
        written.contains("CORA_FULL_ACCESS_OK"),
...
async fn live_claude_workflow_natural_completion_still_finishes() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "cora_cowork_ai_agent=debug,cora_cowork_session=info",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-wf-natural-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_cancel_recovers_conversation() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "cora_cowork_ai_agent=debug,cora_cowork_session=debug",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-wf-cancel-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_progress_streams() {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-wf-progress-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_codex_background_work_probe() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "cora_cowork_ai_agent=debug,cora_cowork_session=debug,cora_cowork_conversation=info",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-codex-bg-{}", cora_cowork_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
