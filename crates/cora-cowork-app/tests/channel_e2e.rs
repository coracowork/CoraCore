...
#[tokio::test]
async fn get_channel_settings_defaults_to_generated_corars_assistant() {
    let (mut app, services) = build_app().await;
    let (token, _csrf) = setup_and_login(&mut app, &services, "admin", "StrongP@ss1").await;
    ...
    // corars bare assistant (see channel "default to bare assistant bindings");
    // only the assistant_id is canonical, legacy fields are omitted.
    let assistant_id = json["data"]["assistant"]["assistant_id"]
        .as_str()
        .expect("default channel assistant should be the generated corars bare assistant");
    assert!(
...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn cora_cowork_db::IChannelRepository> =
        std::sync::Arc::new(cora_cowork_db::SqliteChannelRepository::new(pool));
    let pairing_svc = cora_cowork_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
