...
async fn management_list_returns_array() {
    ...
    assert!(agents.iter().any(|a| a["agent_type"] == "corars"));
}
...
async fn management_list_includes_missing_custom_agents() {
    ...
    repo.upsert(&UpsertAgentMetadataParams {
        ...
        agent_type: "acp",
        agent_source: "custom",
        agent_source_info: Some(r#"{"binary_name":"cora-cowork-missing-agent-binary"}"#),
        enabled: true,
        command: Some("cora-cowork-missing-agent-binary"),
        args: Some("[]"),
        env: Some("[]"),
