//! CoraPro-mode gate: machine startup must not mint local-default-user rows.
//!
...
//! account that never logs in on an CoraPro machine. This e2e boots the full
//! router in CoraPro mode and then sweeps EVERY ownership column in the live
...
#[tokio::test]
async fn corapro_startup_writes_no_system_default_user_rows() {
    ...
    let _router = cora_cowork_app::create_router(&services).await.expect("build router");
    ...
    assert!(
        offenders.is_empty(),
        "CoraPro startup minted local-default-user rows — a machine-level \
