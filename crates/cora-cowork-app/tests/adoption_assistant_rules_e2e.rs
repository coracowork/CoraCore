//! End-to-end reproduction of the CoraCowork → CoraPro adoption path for
...
use cora_cowork_assistant::service::AssistantServiceDeps;
use cora_cowork_assistant::{AssistantService, BuiltinAssistantRegistry};
use cora_cowork_db::{
...
use cora_cowork_extension::{SkillPaths, fs_adopt};
...
/// cora-cowork-auth does: provision external user → DB adoption (stamps the
...
async fn run_real_adoption(db: &Database, paths: &SkillPaths) -> String {
    ...
    let adopter = user_repo
        .ensure_external_user(UserType::Corapro, "ext-user-1", ExternalUserProjection::default())
        .await
