...
//! `cora_cowork_app::create_router_with_states` against an in-memory DB.
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("CORA_COWORK_BUILTIN_SKILLS_PATH");
    }
    ...
    // `build_module_states` builds a state pointing at `~/.cora-cowork/`,
    ...
    cora_cowork_extension::sync_skill_catalog_into_repo(&states.skill.skill_paths, skill_repo.as_ref())
        .await
...
async fn unified_skill_list_includes_auto_inject_entries_from_embedded_corpus() {
    ...
    assert!(
        names.contains(&"cora-cowork-config"),
        "cora-cowork-config should be shipped as an auto-inject builtin skill: {names:?}",
    );
    assert!(
        !names.contains(&"cora-cowork-skills"),
        "cora-cowork-skills should not be shipped as an auto-inject builtin skill: {names:?}",
    );
