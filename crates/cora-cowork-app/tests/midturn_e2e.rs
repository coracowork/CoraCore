...
use cora_cowork_app::{AppConfig, AppServices};
...
struct MidturnMockAgent {
    ...
    tx: tokio::sync::broadcast::Sender<cora_cowork_ai_agent::AgentStreamEvent>,
...
impl IMockAgent for MidturnMockAgent {
    async fn deliver_midturn(
        &self,
        data: cora_cowork_ai_agent::types::SendMessageData,
    ) -> Result<(), cora_cowork_ai_agent::AgentSendError> {
...
async fn build_midturn_app(supports_midturn: bool) -> MidturnRig {
    ...
    let app = cora_cowork_app::create_router(&services).await.expect("build router");
    MidturnRig {
        app,
        services,
        delivered,
