...
use cora_cowork_ai_agent::{AgentError, AgentStreamEvent, IWorkerTaskManager};
...
async fn build_app_with_mock_tasks() -> (axum::Router, cora_cowork_app::AppServices, Arc<MockTaskManager>) {
...
async fn upsert_visible_agent_metadata(services: &cora_cowork_app::AppServices, id: &str, agent_type: &str) {
    services
        .agent_registry
        .repo_handle()
...
async fn agent_logos_endpoint_returns_backend_to_logo_catalog() {
    ...
    // Cora CLI has no vendor `backend` (NULL); it must still be keyed by its
    // agent_type ("corars") so corars conversations resolve a logo.
    assert_eq!(logo_for("corars").as_deref(), Some("/api/assets/logos/brand/cora.svg"));
...
async fn agent_logos_endpoint_includes_disabled_and_missing_rows() {
    ...
    services
        ...
        .upsert(&UpsertAgentMetadataParams {
            id: "logo-only-row",
            icon: Some("/api/assets/logos/brand/cora.svg"),
            name: "Logo Only",
            name_i18n: None,
            description: None,
            description_i18n: None,
    ...
    assert_eq!(entry.unwrap()["logo"], "/api/assets/logos/brand/cora.svg");
}
...
#[tokio::test]
async fn internal_cora_cli_rejects_overrides() {
    let (mut app, services, _mock_tm) = build_app_with_mock_tasks().await;
    let (token, csrf) = setup_and_login(&mut app, &services, "admin", "Pass123!").await;
    upsert_visible_agent_metadata(&services, "632f31d2", "corars").await;
    services.agent_registry.hydrate().await.unwrap();
    services.agent_registry.refresh_availability().await;
