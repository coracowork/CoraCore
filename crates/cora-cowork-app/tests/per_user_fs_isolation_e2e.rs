...
//! CoraCowork → CoraPro adoption moves the default user's files to the adopter.
//!
//! Boots the real AppServices (CoraPro mode) so the assertions run against the
//! production filesystem layout, not hand-built paths.

use std::path::Path;

use cora_cowork_extension::skill_service::{
...
async fn corapro_services(data_dir: &Path) -> cora_cowork_app::AppServices {
    let db = cora_cowork_db::init_database_memory().await.unwrap();
    let config = cora_cowork_app::AppConfig {
        identity_mode: cora_cowork_app::IdentityMode::CoraPro,
        bootstrap_secret: Some("bootstrap-secret".to_string()),
        data_dir: data_dir.to_path_buf(),
        work_dir: data_dir.to_path_buf(),
        ..Default::default()
    };
    cora_cowork_app::AppServices::from_config(db, &config).await.unwrap()
}

async fn seed_user(services: &cora_cowork_app::AppServices, username: &str) -> String {
    use cora_cowork_db::IUserRepository;
    cora_cowork_db::SqliteUserRepository::new(services.database.pool().clone())
...
async fn same_named_skill_is_physically_isolated_per_user() {
    ...
    let services = corapro_services(data.path()).await;
    ...
    let dir_a = cora_cowork_common::user_dir_name(&user_a).unwrap();
    let dir_b = cora_cowork_common::user_dir_name(&user_b).unwrap();
...
#[tokio::test]
async fn provisioning_first_corapro_user_adopts_default_user_files_on_disk() {
    let data = tempfile::tempdir().unwrap();
    let services = corapro_services(data.path()).await;
    let (app, _runtime) = cora_cowork_app::create_router_with_runtime(&services).await.unwrap();
    ...
    // Provision the first external (CoraPro) user via the bootstrap-secret API
    ...
    let resp = app
        .oneshot(
            axum::http::Request::builder()
                ...
                .header("x-coracore-bootstrap-secret", "bootstrap-secret")
                .body(axum::body::Body::from(r#"{"user_type":"corapro","username":"Pro"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let adopter_dir = cora_cowork_common::user_dir_name(adopter).unwrap();
