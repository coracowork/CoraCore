...
async fn insert_message(
    ...
    let msg = cora_cowork_db::models::MessageRow {
        id: msg_id.into(),
    ...
    cora_cowork_db::IConversationRepository::insert_message(&repo, &msg)
        .await
        .unwrap();
}

async fn update_conversation_workspace(services: &cora_cowork_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = cora_cowork_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn upsert_artifact(services: &cora_cowork_app::AppServices, artifact: cora_cowork_db::ConversationArtifactRow) {
    let repo = cora_cowork_db::SqliteConversationRepository::new(services.database.pool().clone());
    cora_cowork_db::IConversationRepository::upsert_artifact(&repo, &artifact)
        .await
        .unwrap();
}
