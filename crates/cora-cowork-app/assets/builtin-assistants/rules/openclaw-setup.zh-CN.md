...
**方案 B：启动 Cora WebUI 远程模式**

- **体验**：通过浏览器访问，使用 Cora 的完整界面功能
- **优势**：界面更丰富，支持文件预览、多对话管理等高级功能
- **适用场景**：需要复杂操作、文件管理、多任务处理的场景
- **配置要求**：启动 Cora WebUI 服务，通过浏览器访问
...
5. **安装完成后的配置引导**（重要）：
   ...
   - **使用引导**：
     - **本地使用**："OpenClaw 安装完成后，**请重启 Cora**，然后你就可以在 Cora 首页的可用Agent列表中看到 OpenClaw，并直接开始对话。"
...
1. **先询问用户偏好**：询问用户想配置哪种方式 → **等待用户回复**
   - "你想直接连接 IM 渠道（如 Telegram、WhatsApp 等），还是使用 Cora WebUI 远程模式？"
2. **根据用户选择**：
   - **选择 IM 渠道** → 进入方案 A
   - **选择 WebUI** → 进入方案 B
3. **方案 A：配置 IM 渠道**
...
4. **方案 B：启动 Cora WebUI 远程模式**
   ...
   - **关键原则**：
     ...
     - **不要尝试安装 `@cora-cowork/webui` 等 npm 包**：WebUI 是 Cora 的内置功能，不是独立包
...
5. **安装完成后的配置引导**：
   ...
   - **如果已配置**：告知可以开始使用
   - **使用引导**：
     - 介绍本地使用方式：**提醒用户重启 Cora**，然后可以在首页找到 OpenClaw
