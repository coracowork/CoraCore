...
**方案 B：启动 CoraUi WebUI 远程模式**

- **体验**：通过浏览器访问，使用 CoraUi 的完整界面功能
- **优势**：界面更丰富，支持文件预览、多对话管理等高级功能
- **适用场景**：需要复杂操作、文件管理、多任务处理的场景
- **配置要求**：启动 CoraUi WebUI 服务，通过浏览器访问
...
5. **安装完成后的配置引导**（重要）：
   ...
   - **使用引导**：
     - **本地使用**："OpenClaw 安装完成后，**请重启 CoraUi**，然后你就可以在 CoraUi 首页的可用Agent列表中看到 OpenClaw，并直接开始对话。"
...
1. **先询问用户偏好**：询问用户想配置哪种方式 → **等待用户回复**
   - "你想直接连接 IM 渠道（如 Telegram、WhatsApp 等），还是使用 CoraUi WebUI 远程模式？"
2. **根据用户选择**：
   - **选择 IM 渠道** → 进入方案 A
   - **选择 WebUI** → 进入方案 B
3. **方案 A：配置 IM 渠道**
...
4. **方案 B：启动 CoraUi WebUI 远程模式**
   - **必须使用 cora-cowork-webui-setup 技能**：查阅 `references/cora-cowork-webui.md`
   - **工作流程**：
     1. 询问用户需求：同一 WiFi、跨网络访问，还是服务器部署？→ **等待用户回复**
     2. 用户回复后，**引导用户到 CoraUi 设置界面配置**：
        - **打开设置界面**：明确告诉用户如何打开
          - "请点击 CoraUi 左下角的**设置图标**（齿轮图标）"
          - "在设置菜单中，点击 **'WebUI'** 选项"
          - "进入 WebUI 配置界面"
        - **配置步骤**：按照 `cora-cowork-webui-setup` 技能的 `references/cora-cowork-webui.md` 文档，引导用户完成：
   ...
   - **关键原则**：
     ...
     - **不要尝试安装 `@cora-cowork/webui` 等 npm 包**：WebUI 是 CoraUi 的内置功能，不是独立包
...
### cora-cowork-webui-setup 技能

**核心文档**：`references/cora-cowork-webui.md`

**使用时机**：用户选择 WebUI 方案时立即使用

**使用方式**：

1. **直接查阅 `references/cora-cowork-webui.md`**，按照文档引导用户完成配置
2. 文档包含完整的引导式说明：
...
5. **安装完成后的配置引导**：
   ...
   - **如果已配置**：告知可以开始使用
   - **使用引导**：
     - 介绍本地使用方式：**提醒用户重启 CoraUi**，然后可以在首页找到 OpenClaw
...
7. **技能使用**：
   ...
   - WebUI 配置 → **必须使用 `cora-cowork-webui-setup` 技能**（直接查阅 `references/cora-cowork-webui.md` 并按文档执行，不要重复文档中的详细步骤）
