...
**Вариант B: Запуск CoraUi WebUI в удалённом режиме**

- **Опыт**: Доступ через браузер с полным интерфейсом CoraUi
...
- **Требования к настройке**: Запуск сервиса CoraUi WebUI, доступ через браузер
...
5. **Руководство по настройке после установки** (ВАЖНО):
   ...
   - **Руководство по использованию**:
     - **Локальное использование**: «После завершения установки OpenClaw, **перезапустите CoraUi**, затем вы сможете увидеть OpenClaw в списке доступных агентов на главной странице CoraUi и начать общаться напрямую.»
...
1. **Сначала спросить предпочтение пользователя**: Спросить пользователя, какой метод он хочет настроить → **Дождаться ответа пользователя**
   - «Хотите ли вы подключиться напрямую к IM-каналам (таким как Telegram, WhatsApp и др.) или использовать удалённый режим CoraUi WebUI?»
...
4. **Вариант B: Запуск CoraUi WebUI в удалённом режиме**
   - **Обязательно использовать навык cora-cowork-webui-setup**: Обратиться к `references/cora-cowork-webui.md`
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к интерфейсу настроек CoraUi**:
        - **Открыть интерфейс настроек**: Чётко сообщить пользователю, как его открыть
          - «Пожалуйста, нажмите на **значок настроек** (значок шестерёнки) в левом нижнем углу CoraUi»
          - «В меню настроек нажмите на опцию \*\*'WebUI'»
          - «Войдите в интерфейс конфигурации WebUI»
        - **Шаги настройки**: Следуйте документации `cora-cowork-webui-setup` навыка `references/cora-cowork-webui.md` для направления пользователя:
   ...
   - **Ключевые принципы**:
     ...
     - **Не пытайтесь установить `@cora-cowork/webui` или подобные npm-пакеты**: WebUI — это встроенная функция CoraUi, а не отдельный пакет
...
### Навык cora-cowork-webui-setup

**Основная документация**: `references/cora-cowork-webui.md`

**Когда использовать**: Когда пользователь выбирает вариант WebUI, использовать немедленно

**Как использовать**:

1. **Напрямую обратиться к `references/cora-cowork-webui.md`** и направить пользователя на завершение конфигурации согласно документации
...
5. **Руководство по настройке после установки**:
   ...
   - **Руководство по использованию**:
     - Представить локальное использование (вернуться на главную страницу CoraUi)
...
3. **Выбрал WebUI**: Использовать навык `cora-cowork-webui-setup` → Спросить потребности → Выбрать решение → Выполнить настройку → Предоставить инструкции по использованию
...
7. **Использование навыков**:
   ...
   - Настройка WebUI → **Обязательно использовать навык `cora-cowork-webui-setup`** (напрямую обратиться к `references/cora-cowork-webui.md` и следовать документации, не повторять подробные шаги из документации)
