...
**Вариант B: Запуск Cora WebUI в удалённом режиме**

- **Опыт**: Доступ через браузер с полным интерфейсом Cora
...
- **Требования к настройке**: Запуск сервиса Cora WebUI, доступ через браузер
...
5. **Руководство по настройке после установки** (ВАЖНО):
   ...
   - **Руководство по использованию**:
     - **Локальное использование**: «После завершения установки OpenClaw, **перезапустите Cora**, затем вы сможете увидеть OpenClaw в списке доступных агентов на главной странице Cora и начать общаться напрямую.»
...
1. **Сначала спросить предпочтение пользователя**: Спросить пользователя, какой метод он хочет настроить → **Дождаться ответа пользователя**
   - «Хотите ли вы подключиться напрямую к IM-каналам (таким как Telegram, WhatsApp и др.) или использовать удалённый режим Cora WebUI?»
...
4. **Вариант B: Запуск Cora WebUI в удалённом режиме**
   ...
   - **Рабочий процесс**:
     ...
     2. После ответа пользователя, **направить пользователя к интерфейсу настроек Cora**:
        - **Открыть интерфейс настроек**: Чётко сообщить пользователю, как его открыть
          - «Пожалуйста, нажмите на **значок настроек** (значок шестерёнки) в левом нижнем углу Cora»
   ...
   - **Ключевые принципы**:
     ...
     - **Не пытайтесь установить `@cora-cowork/webui` или подобные npm-пакеты**: WebUI — это встроенная функция Cora, а не отдельный пакет
...
5. **Руководство по настройке после установки**:
   ...
   - **Руководство по использованию**:
     - Представить локальное использование (вернуться на главную страницу Cora)
