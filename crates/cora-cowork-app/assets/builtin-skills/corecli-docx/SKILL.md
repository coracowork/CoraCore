...
```bash
corecli help docx                         # List all docx elements
corecli help docx <element>               # Full element schema (e.g. paragraph, field, numbering, watermark, toc)
corecli help docx <verb> <element>        # Verb-scoped (e.g. add field, set section)
corecli help docx <element> --json        # Machine-readable schema
```
...
A `.docx` is a ZIP of XML parts (`document.xml`, `styles.xml`, `numbering.xml`, `header*.xml`, `footer*.xml`, `comments.xml`, …). Everything the user sees — headings, tables, page numbers, TOC, tracked changes — is XML inside that ZIP. `corecli` gives you a semantic-path API (`/body/p[1]/r[2]`) over it, so you almost never touch raw XML; when you must, use `raw-set` (see the XML appendix).
...
**Incremental execution.** `corecli` mutates the file on every call. Run commands one at a time and check each exit code — a 50-command script that fails at command 3 cascades silently. After any structural op (new style, table, TOC, section break) run `get` on it before stacking more.

**Resident mode is the default**, not an optimization: `corecli open <file>` at the start, `corecli close <file>` at the end — it avoids re-parsing the XML every call. For many paragraphs of one style, use `batch` (one open/save cycle for the whole array).
...
Before declaring done, run `corecli view "$FILE" html` and Read the returned HTML path to confirm ALL of these:
...
1. **Open the file.** `corecli open "$FILE"` (resident default). New file: `corecli create "$FILE"` first.
2. **Orient.** Existing file: `corecli view "$FILE" outline` — heading tree, section count, whether a TOC / watermark / tracked changes already exist. Never edit blind.
...
5. **Close, then trust structure over cached text.** `corecli close "$FILE"` writes the XML. TOC / PAGE / NUMPAGES / SEQ / PAGEREF fields carry **cached values** that may be stale or empty until a human recalculates (F9 in Word). Confirm fields *exist* (`get --depth 3` finds `<w:fldChar>`) rather than trusting the visible text.
...
corecli create "$FILE"
corecli open "$FILE"
corecli add "$FILE" /body --type paragraph --prop text="Q4 2026 Review" --prop style=Heading1 --prop size=20pt --prop bold=true --prop spaceAfter=12pt
corecli add "$FILE" /body --type paragraph --prop text="Revenue grew 18% year-over-year, ahead of plan." --prop size=11pt --prop spaceAfter=8pt
corecli add "$FILE" /body --type paragraph --prop text="Key Drivers" --prop style=Heading2 --prop size=14pt --prop bold=true --prop spaceBefore=12pt --prop spaceAfter=6pt
corecli add "$FILE" /body --type paragraph --prop text="Enterprise renewals, upsell, and a new EMEA region." --prop size=11pt
corecli add "$FILE" / --type footer --prop type=default --prop size=9pt --prop text="Page " --prop field=page
corecli set "$FILE" "/footer[1]/p[1]" --prop align=center
corecli close "$FILE"
...
```bash
corecli view "$FILE" outline            # heading tree, section count, table/image counts, watermark, tracked-changes presence — orient here first
corecli view "$FILE" html               # Read the returned HTML path: first visual check after a batch of edits (hierarchy, empty-para spacing, missing TOC)
corecli view "$FILE" text --start 1 --end 80   # text for content QA; paths shown as [/body/p[N]] so you can jump back with get
corecli view "$FILE" annotated          # values + style/font/size + warnings per run
corecli view "$FILE" stats              # paragraph counts, font usage, style distribution
corecli view "$FILE" issues             # empty paras, missing alt text, spacing anomalies
```

`corecli watch "$FILE"` keeps a live preview running for the human user to open at their discretion — agent self-check uses `view html`. Final visual verification is the user opening the `.docx` in Word / WPS / Pages.
...
```bash
corecli get "$FILE" /                          # document root: metadata, page setup
corecli get "$FILE" "/body/p[1]"                # one paragraph
corecli get "$FILE" "/body/p[1]/r[1]"           # one run (character-level formatting)
corecli get "$FILE" "/body/tbl[1]" --depth 3    # table with rows and cells
corecli get "$FILE" "/footer[1]" --depth 3      # footer — check for fldChar
corecli get "$FILE" "/styles/Heading1"          # style definition
corecli get "$FILE" /numbering --depth 2        # numbering abstractNum + num bindings
```

**Query across the document.** CSS-like selectors, for systematic checks rather than hand-walking. Operators: `=`, `!=`, `~=` (contains), `>=`, `<=`, `[attr]` (exists). Full reference: `corecli query --help`.

```bash
corecli query "$FILE" 'paragraph[style=Heading1]'       # all H1s
corecli query "$FILE" 'p:contains("quarterly")'         # text match
corecli query "$FILE" 'p:empty'                         # empty paragraphs (clutter)
corecli query "$FILE" 'image:no-alt'                    # accessibility gaps
corecli query "$FILE" 'paragraph[size>=24pt]'           # numeric comparison
corecli query "$FILE" 'field[fieldType!=page]'          # fields other than PAGE
```
...
```bash
corecli add "$FILE" /body --type paragraph --prop text="Executive Summary" --prop style=Heading1 --prop size=18pt --prop bold=true --prop spaceAfter=12pt
corecli set "$FILE" "/body/p[1]/r[1]" --prop color=1F4E79
```
...
```bash
corecli add "$FILE" /body --type table --prop rows=4 --prop cols=3 --prop width=100%
corecli set "$FILE" "/body/tbl[1]/tr[1]" --prop header=true --prop c1=Quarter --prop c2="Revenue" --prop c3="Growth"
corecli set "$FILE" "/body/tbl[1]/tr[1]/tc[1]/p[1]/r[1]" --prop bold=true
```
...
```bash
corecli set "$FILE" "/body/p[3]" --prop pbdr.bottom="single;6;2E75B6"
```
...
```bash
corecli add "$FILE" /body --type paragraph --prop text="First item" --prop listStyle=bullet
```

For multi-level (legal-style 1 / 1.1 / 1.1.1), add an `abstractNum`, then a `num`, then reference the `numId` per paragraph:

```bash
corecli add "$FILE" /numbering --type abstractnum --prop format=decimal     # → abstractNum id=0
corecli add "$FILE" /numbering --type num --prop abstractNumId=0             # → num id=1
corecli add "$FILE" /body --type paragraph --prop text="Section one" --prop numId=1 --prop ilvl=0
```

IDs are 0-based: the first `abstractNum` is id=0; the `num` references it via `abstractNumId=0` and is itself assigned id=1. A non-existent `abstractNumId` errors, so check ids after creating. Verify with `corecli query "$FILE" 'paragraph[numId>0]'`. See `help docx abstractnum` / `help docx num` for level and format options.
...
```bash
corecli add "$FILE" "/body/p[1]" --type tab --prop pos=6in --prop val=right --prop leader=dot
```
...
```bash
corecli add "$FILE" "/body/p[3]" --type field --prop fieldType=mergefield --prop name=customer_name
...
**SEQ / PAGEREF / TOC field values.** corecli doesn't store rendered field values at write time. Recompute by what each path needs:

- **SEQ numbering** (`Figure 1/2/3`): `corecli set "$FILE" / --prop recalcFields=seq` counts SEQ fields in body document order and writes the cached values (`evaluated` flips true; switches/formats in `help docx document`). Heading-relative `\s` and SEQ in headers/footers defer to Word.
- **PAGE / PAGEREF / NUMPAGES / TOC page numbers** need pagination, which corecli has no engine for — `corecli set "$FILE" /settings --prop updateFields=true` defers them to Word on open.

Use both on a multi-figure document. Academic papers: see the `corecli-academic-paper` skill.
...
corecli add "$FILE" / --type footer --prop type=first --prop text=""
# Default footer with live page number
corecli add "$FILE" / --type footer --prop type=default --prop align=center --prop size=9pt --prop text="Page " --prop field=page
```
...
```bash
corecli add "$FILE" /body --type toc --prop levels="1-3" --prop title="Table of Contents" --prop hyperlinks=true --index 0
```
...
- **Will recalculate (or press F9):** run `corecli set "$FILE" /settings --prop updateFields=true` so Word recomputes the TOC (and all fields) on open, and/or add a visible "Press F9 to refresh the TOC and page numbers" instruction. Done.
...
Ship-check: `corecli query "$FILE" 'p:contains("Update field to see")'` must return empty whenever the reader won't recalculate. A match means switch to recipe (f).
...
```bash
corecli add "$FILE" "/body/p[5]" --type picture --prop src=logo.png --prop width=1.5in --prop alt="Acme logo"
```

Confirm `corecli query "$FILE" 'image:no-alt'` is empty before delivery.
...
```bash
corecli add "$FILE" /body --type chart --prop chartType=bar --prop title="Revenue by Region" --prop categories="EMEA,APAC,Americas" --prop data="2026:120,150,180"
```

`chartType` ∈ bar / column / line / pie / area / scatter (`help docx chart` for axis/legend/series styling). A PNG via `--type picture` is only a fallback for an exotic chart corecli can't build.
...
```bash
corecli add "$FILE" "/body/p[2]" --type hyperlink --prop url="https://example.com" --prop text="our site"
```

**Internal links** (to a bookmark) use `--prop anchor=bookmarkName` — not a `#fragment` in `url`:

```bash
corecli add "$FILE" "/body/p[2]" --type hyperlink --prop anchor=chapter1 --prop text="See Chapter 1"
```
...
```bash
corecli set "$FILE" / --prop pageWidth=12240 --prop pageHeight=15840 --prop marginTop=1440 --prop marginLeft=1440
# Newspaper-style multi-column flow (columnSpace in twips; 720 = 0.5in):
corecli set "$FILE" / --prop columns=2 --prop columnSpace=720
```
...
```bash
corecli add "$FILE" /body --type pagebreak --index <N>          # 1. pagebreak element BEFORE the heading
corecli set "$FILE" "/body/p[<N+1>]" --prop pageBreakBefore=true # 2. on the heading itself
```
...
```bash
corecli add "$FILE" /body --type paragraph --prop text="CONFIDENTIAL — CLIENT USE ONLY" --prop align=center --prop size=9pt --prop color=C00000 --prop spaceAfter=24pt
corecli add "$FILE" /body --type paragraph --prop text="Strategic Growth Review" --prop style=Title --prop size=32pt --prop bold=true --prop align=center --prop font=Cambria --prop spaceAfter=8pt
corecli add "$FILE" /body --type paragraph --prop text="FY26 Outlook and Scenario Planning" --prop italic=true --prop size=16pt --prop align=center --prop spaceAfter=36pt
corecli add "$FILE" /body --type paragraph --prop text='Prepared for: Acme Corp. Leadership Team' --prop align=center --prop size=11pt
corecli add "$FILE" /body --type paragraph --prop text='Engagement: 2026-04 — 2026-06' --prop align=center --prop size=11pt --prop spaceAfter=36pt
corecli add "$FILE" /body --type paragraph --prop text="Key themes: 1) margin resilience, 2) EMEA expansion, 3) capital allocation." --prop align=center --prop italic=true --prop size=10pt
corecli add "$FILE" /body --type pagebreak
corecli set "$FILE" "/body/p[last()]" --prop pageBreakBefore=true
```
...
```bash
corecli add "$FILE" / --type footer --prop type=default --prop text="Page " --prop align=center --prop size=9pt
corecli add "$FILE" "/footer[1]/p[1]" --type field --prop fieldType=page
corecli add "$FILE" "/footer[1]/p[1]" --type run --prop text=" of "
corecli add "$FILE" "/footer[1]/p[1]" --type field --prop fieldType=numpages
corecli get "$FILE" "/footer[1]/p[1]" --depth 1 | grep -o fldChar | wc -l   # expect ≥ 4; use grep -o ... | wc -l, NOT grep -c (single-line XML returns 1)
```
...
corecli set "$FILE" "/body/tbl[1]/tr[1]" --prop header=true --prop c1=Quarter --prop c2=Revenue --prop c3=Growth --prop c4=Status
for col in 1 2 3 4; do
  corecli set "$FILE" "/body/tbl[1]/tr[1]/tc[$col]" --prop fill=1F4E79
  corecli set "$FILE" "/body/tbl[1]/tr[1]/tc[$col]/p[1]/r[1]" --prop bold=true --prop color=FFFFFF
done
for row in 3 5; do for col in 1 2 3 4; do
  corecli set "$FILE" "/body/tbl[1]/tr[$row]/tc[$col]" --prop fill=D9E2F3      # zebra stripe
done; done
```
...
for row in 2 3 4 5; do for col in 2 3 4; do
  corecli set "$FILE" "/body/tbl[1]/tr[$row]/tc[$col]/p[1]" --prop align=right
done; done
for col in 1 2 3 4; do
  corecli set "$FILE" "/body/tbl[1]/tr[5]/tc[$col]/p[1]/r[1]" --prop bold=true
  corecli set "$FILE" "/body/tbl[1]/tr[4]/tc[$col]/p[1]" --prop pbdr.bottom="single;6;000000;0"
done
```
...
```bash
corecli set "$FILE" "/body/tbl[1]/tr[1]" --prop c1="Installed base of 18k enterprise seats"
corecli add "$FILE" "/body/tbl[1]/tr[1]/tc[1]" --type paragraph --prop text="Margin structure above peer median" --prop listStyle=bullet
corecli set "$FILE" "/body/tbl[1]/tr[1]/tc[1]/p[1]" --prop listStyle=bullet
```

If the seeded line lands at the bottom, re-order: `corecli move "$FILE" "/body/tbl[1]/tr[1]/tc[1]/p[N]" --index 0`.
...
```bash
corecli query "$FILE" 'p:contains("Update field to see")'        # note the /body/p[N] paths, then:
corecli remove "$FILE" "/body/p[N]"                              # repeat per hit
corecli add "$FILE" /body --type paragraph --prop text="Contents" --prop style=TOCHeading --prop size=14pt --prop bold=true --index <pos>
corecli add "$FILE" /body --type paragraph --prop text="1. Executive Summary ......................................... 3" --prop size=11pt --index <pos+1>
...
```bash
corecli add "$FILE" /body --type equation --prop formula="\\frac{a}{b} + \\sum_{i=1}^{n} x_i"
corecli add "$FILE" "/body/p[3]" --type footnote --prop text="See Appendix A for methodology."
```
...
**When to switch skills.** Stay in docx for chapter drafts, ≤ 3 footnotes, ≤ 2 equations, no bibliography/cross-refs. Switch to **`academic-paper`** for citation styles (APA / Chicago / IEEE / GB 7714), in-text↔reference auto-linking, numbered equations with `\ref`, "List of Figures", or auto-updating cross-refs. Switch to **`corecli-word-form`** when the document's purpose is **data capture** — fillable forms, contracts with user-fill slots, questionnaires, mail-merge templates (`<w:sdt>` content controls, `<w:ffData>`, `documentProtection=forms`).
...
# Gate 1 — schema.
corecli close "$FILE" 2>/dev/null
corecli validate "$FILE" | grep -q "no errors found" || { echo "REJECT Gate 1: validate failed"; exit 1; }
echo "Gate 1 OK"

# Gate 2 — token leak (shell-escape / template tokens / TOC placeholder / literal \$ \t \n). grep -c never false-PASSes.
LEAK=$(corecli view "$FILE" text | grep -cE '(\$[A-Za-z_]+\$|\{\{[^}]+\}\}|<TODO>|xxxx|lorem|Update field to see|\\[\$tn])')
[ "$LEAK" -eq 0 ] && echo "Gate 2 OK" || { echo "REJECT Gate 2: $LEAK leak line(s)"; corecli view "$FILE" text | grep -nE '(\$[A-Za-z_]+\$|\{\{[^}]+\}\}|<TODO>|xxxx|lorem|Update field to see|\\[\$tn])'; exit 1; }

# Gate 3 — live PAGE field exists when a footer is expected.
FLD=$(corecli query "$FILE" 'field[fieldType=page]' --json | jq '.data.results | length')
...
| Echo into batch breaks on `$`/`'` | Heredoc with single-quoted delimiter: `cat <<'EOF' \| corecli batch …` |
...
Force field recalc on open with `corecli set "$FILE" /settings --prop updateFields=true` (writes `<w:updateFields w:val="true"/>`; covers the layout-dependent fields PAGE / PAGEREF / NUMPAGES / TOC page numbers — no raw-set needed). For SEQ numbering, prefer `set / --prop recalcFields=seq`, which writes correct cached values now without waiting on Word.

### Help pointer

When in doubt: `corecli help docx`, `corecli help docx <element>`, `corecli help docx <verb> <element>`, `--json` for agents. Help is the authoritative schema; this skill is the decision guide.
