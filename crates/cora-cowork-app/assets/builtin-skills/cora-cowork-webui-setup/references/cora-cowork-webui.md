...
CoraUi 支持 WebUI 模式，允许通过浏览器访问应用。这对于远程使用 OpenClaw 非常有用。CoraUi 提供三种远程连接方式，满足不同场景的需求。

**重要**：WebUI 配置应通过 CoraUi 设置界面完成，无需使用命令行。本指南将引导你如何在设置界面中完成配置。
...
- 在 CoraUi 主界面，使用快捷键打开设置（具体快捷键请查看 CoraUi 帮助文档）
...
1. 确保远程设备与 CoraUi 电脑在同一 WiFi 网络
...
#### Step 1: 在 CoraUi 电脑上配置 WebUI

1. **打开 WebUI 设置界面**：
   - 在 CoraUi 主界面，点击左下角的**设置图标**
   - 点击 **"WebUI"** 选项
...
#### Step 2: 在 CoraUi 电脑上安装并登录 Tailscale
...
1. 在 CoraUi 电脑上，打开 Tailscale 应用
...
#### Step 1: 在服务器上安装 CoraUi

按照 CoraUi 安装指南在服务器上安装 CoraUi 应用。

#### Step 2: 通过设置界面配置 WebUI

1. **打开 WebUI 设置界面**：
   - 如果服务器有图形界面，直接打开 CoraUi 应用
...
如果需要 CoraUi 开机自启，可以配置 systemd 服务。但建议通过 CoraUi 设置界面管理 WebUI，而不是通过命令行。
...
按照 CoraUi 安装指南在 macOS 服务器上安装 CoraUi 应用。
...
```bash
# 允许端口 25808
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add /Applications/CoraUi.app/Contents/MacOS/CoraUi
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp /Applications/CoraUi.app/Contents/MacOS/CoraUi
```
...
| 平台    | 配置文件位置                                             |
| ------- | -------------------------------------------------------- |
| Windows | `%APPDATA%/CoraUi/webui.config.json`                     |
| macOS   | `~/Library/Application Support/CoraUi/webui.config.json` |
| Linux   | `~/.config/CoraUi/webui.config.json`                     |
...
启动 WebUI 后，可以通过浏览器访问 CoraUi，然后：
...
1. **打开设置**：点击 CoraUi 左下角的**设置图标** → 点击 **"WebUI"**
