...
Cora 支持 WebUI 模式，允许通过浏览器访问应用。这对于远程使用 OpenClaw 非常有用。Cora 提供三种远程连接方式，满足不同场景的需求。

**重要**：WebUI 配置应通过 Cora 设置界面完成，无需使用命令行。本指南将引导你如何在设置界面中完成配置。
...
- 在 Cora 主界面，使用快捷键打开设置（具体快捷键请查看 Cora 帮助文档）
...
#### Step 1: 在 Cora 电脑上配置 WebUI

1. **打开 WebUI 设置界面**：
   - 在 Cora 主界面，点击左下角的**设置图标**
   - 点击 **"WebUI"** 选项
...
#### Step 2: 在 Cora 电脑上安装并登录 Tailscale
...
按照 Cora 安装指南在服务器上安装 Cora 应用。

#### Step 2: 通过设置界面配置 WebUI

1. **打开 WebUI 设置界面**：
   - 如果服务器有图形界面，直接打开 Cora 应用
...
如果需要 Cora 开机自启，可以配置 systemd 服务。但建议通过 Cora 设置界面管理 WebUI，而不是通过命令行。
...
按照 Cora 安装指南在 macOS 服务器上安装 Cora 应用。
...
```bash
# 允许端口 25808
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add /Applications/Cora.app/Contents/MacOS/Cora
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp /Applications/Cora.app/Contents/MacOS/Cora
```
...
| 平台    | 配置文件位置                                             |
| ------- | -------------------------------------------------------- |
| Windows | `%APPDATA%/Cora/webui.config.json`                     |
| macOS   | `~/Library/Application Support/Cora/webui.config.json` |
| Linux   | `~/.config/Cora/webui.config.json`                     |
...
1. **打开设置**：点击 Cora 左下角的**设置图标** → 点击 **"WebUI"**
