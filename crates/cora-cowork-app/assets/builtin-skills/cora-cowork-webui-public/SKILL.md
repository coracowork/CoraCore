...
```bash
mkdir -p ~/.cora-cowork/tools && cd ~/.cora-cowork/tools
...
(Use the path to the binary you installed, e.g. `~/.cora-cowork/tools/cloudflared`.)
...
> These `/api/webui/*` credential endpoints only work when coracore runs in
> **local mode** (the normal desktop/Electron case, which is exactly the scenario
> this skill targets). If coracore is deployed as a standalone server, they
