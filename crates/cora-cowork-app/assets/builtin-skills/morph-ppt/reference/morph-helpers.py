...
def _has_morph_transition(json_str):
    """Check whether JSON output from corecli contains transition=morph."""
...
def morph_clone_slide(deck, from_slide, to_slide):
    ...
    _run("corecli", "add", deck, "/", "--from", f"/slide[{from_slide}]")

    print(f"{BLUE}Setting morph transition...{NC}")
    _run("corecli", "set", deck, f"/slide[{to_slide}]", "--prop", "transition=morph")

    print(f"{BLUE}Listing shapes for ghosting reference:{NC}")
    rc, out, _ = _run("corecli", "get", deck, f"/slide[{to_slide}]", "--depth", "1")
    print(out)

    # Verify
    print(f"{BLUE}Verifying transition...{NC}")
    rc, out, _ = _run("corecli", "get", deck, f"/slide[{to_slide}]", "--json")
...
def morph_ghost_content(deck, slide, *shapes):
    ...
    for idx in shapes:
        rc, _, _ = _run("corecli", "set", deck, f"/slide[{slide}]/shape[{idx}]", "--prop", "x=36cm")
        if rc == 0:
...
def morph_verify_slide(deck, slide):
    ...
    # --- Check transition ---
    rc, out, _ = _run("corecli", "get", deck, f"/slide[{slide}]", "--json")
    curr_json_str = out
    ...
    if prev_slide >= 1:
        ...
        try:
            rc2, out2, _ = _run("corecli", "get", deck, f"/slide[{prev_slide}]", "--json")
...
def morph_final_check(deck):
    ...
    rc, out, _ = _run("corecli", "view", deck, "outline")
    total_slides = 0
    ...
    rc, out, _ = _run("corecli", "query", deck, "shape[x>=34cm]", "--json")
    try:
...
def clean_ghost_accumulation(deck, threshold=50):
    ...
    try:
        ...
        for shape in to_delete:
            ...
            if shape_id:
                _run("corecli", "remove", deck, f"/shape[@id={shape_id}]")
