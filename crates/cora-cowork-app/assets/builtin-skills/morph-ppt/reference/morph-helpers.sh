...
morph_clone_slide() {
    ...
    corecli add "$deck" '/' --from "/slide[$from_slide]"

    echo -e "${BLUE}⚡ Setting morph transition...${NC}"
    corecli set "$deck" "/slide[$to_slide]" --prop transition=morph

    echo -e "${BLUE}📊 Listing shapes for ghosting reference:${NC}"
    corecli get "$deck" "/slide[$to_slide]" --depth 1

    # Verify transition was set
    echo -e "${BLUE}🔍 Verifying transition...${NC}"
    local trans=$(corecli get "$deck" "/slide[$to_slide]" --json 2>/dev/null | grep '"transition": "morph"')
    if [ -z "$trans" ]; then
...
morph_ghost_content() {
    ...
    for shape_idx in "${shapes[@]}"; do
        corecli set "$deck" "/slide[$slide]/shape[$shape_idx]" --prop x=36cm 2>/dev/null
        if [ $? -eq 0 ]; then
...
morph_verify_slide() {
    ...
    # Check transition
    local trans=$(corecli get "$deck" "/slide[$slide]" --json 2>/dev/null | grep '"transition": "morph"')
    if [ -z "$trans" ]; then
    ...
    if [ $prev_slide -ge 1 ]; then
        ...
        local shapes_json=$(corecli get "$deck" "/slide[$slide]" --json 2>/dev/null)
...
")
    ...
    if [ $prev_slide -ge 1 ]; then
        local prev_json=$(corecli get "$deck" "/slide[$prev_slide]" --json 2>/dev/null)
...
morph_final_check() {
    ...
    # Get total slides
    local total_slides=$(corecli view "$deck" outline 2>/dev/null | head -1 | grep -oE '[0-9]+' | head -1 || echo "0")
