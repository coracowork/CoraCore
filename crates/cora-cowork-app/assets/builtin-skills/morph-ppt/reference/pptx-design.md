...
**Shape types** available via `--prop preset=`: `ellipse | rect | roundRect | triangle | diamond | star5 | hexagon`. Full list: `corecli help pptx shape`.
...
**Debugging:** `corecli get $FILE '/slide[N]' --depth 1` to inspect actual indices.
...
1. Slides 2+ must have `transition=morph` (`corecli set /slide[N] --prop transition=morph`).
...
Format: `EFFECT[-DIRECTION][-DURATION][-TRIGGER]`. See `corecli help pptx animation` for preset list.
