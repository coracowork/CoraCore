---
name: corecli-pitch-deck
description: "Use this skill when the user is building a fundraising / investor pitch deck — seed, Series A / B / C, convertible note, SAFE round, strategic raise. Trigger on: 'pitch deck', 'investor deck', 'Series A deck', 'Series B deck', 'Series C deck', 'fundraising deck', 'seed pitch', 'VC deck', 'raising capital', 'term sheet presentation'. Output is a single .pptx. This skill is a scene layer on top of corecli-pptx — inherits every pptx v2 rule (visual floor, grid, palettes, connector canon, Delivery Gate). DO NOT invoke for a generic board review, sales deck, all-hands, or product launch — route those to corecli-pptx base."
---
...
```bash
corecli help pptx                          # All pptx elements
corecli help pptx <element>                # Full schema (e.g. chart, shape, connector, picture)
corecli help pptx <element> --json         # Machine-readable
```

Help reflects the installed CLI version. When this skill and help disagree, **help wins.** Every `--prop X=` in this file has been grep-verified against `corecli help pptx <element>` — if help adds / renames a prop in a later version, trust help.

## Mental Model & Inheritance

**Inherits pptx v2.** You should have read `skills/corecli-pptx/SKILL.md` first. This skill assumes you know how to: add slides + shapes + charts + connectors; address by `@name=` / `@id=`; quote paths; use `batch` heredocs; write `--prop tailEnd=triangle` on every flow connector; and run the 5-gate Delivery Gate. If any of those are unfamiliar, open a pptx v2 session before continuing.
...
**Speaker notes rule.** Every content slide (non-cover, non-closing) MUST carry speaker notes via `corecli add "$FILE" /slide[N] --type notes --prop text='…'`. Missing notes = not shippable — inherits pptx v2 §Hard rules (H7). Run `corecli help pptx notes` to confirm prop names before building.
...
Sub-labels ≥ 16pt (H4 floor). For 5 stats stacked, drop number size to 44pt; 6+ stats means pick a different pattern. Post-batch for column/bar charts: `corecli set "$FILE" "/slide[N]/chart[1]" --prop gap=80` to tighten bar spacing.
...
> **Chart `series1.color=` on `add` works** (applies to every chart recipe below). Passing `--prop series1.color=` (or `series2.color=`, …) on chart `add` applies the series color and exits 0. Verify with a readback if you like: `corecli get "$FILE" "/slide[N]/chart[1]/series[1]" --json | jq '.data.results[0].format.color'`.
...
```bash
corecli add "$FILE" / --type slide --prop layout=blank --prop background=1E2761
corecli add "$FILE" "/slide[1]" --type shape --prop name=BrandBand \
  --prop geometry=rect --prop fill=CADCFC \
  --prop x=0cm --prop y=18.5cm --prop width=33.87cm --prop height=0.55cm
corecli add "$FILE" "/slide[1]" --type shape --prop name=CoverTitle --prop text="Acme DevOps" \
...
corecli add "$FILE" "/slide[1]" --type shape --prop name=Tagline --prop text="Kubernetes observability, built for production at scale" \
...
corecli add "$FILE" "/slide[1]" --type shape --prop name=CoverMeta --prop text='Series B · $35M · April 2026' \
...
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Kubernetes debugging burns 12 engineering hours / incident" \
...
**QA.** `corecli query "$FILE" 'shape:contains("Source")'` returns ≥ 3 (every claim carries a source). If zero sources, VCs will not trust a single number.
...
SLIDE=3
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Why now: three converging triggers" \
  --prop x=1.5cm --prop y=1.2cm --prop width=30.87cm --prop height=2.5cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761 --prop fill=none
...
**QA.** 3 cards, each with a date/year citation in the source footnote, each card ≤ 30 words. `corecli query "$FILE" 'shape:contains("2024")'` + `'shape:contains("2025")'` ≥ 2 combined (timing anchors visible).
...
SLIDE=4
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop name=SolTitle \
...
**QA.** Count connectors: `corecli query "$FILE" 'connector' --json | jq '.data.results | length'` ≥ (step_count − 1). Every connector must have `tailEnd=triangle` — `view annotated` confirms arrowhead direction. Title must be ≤ 12 words (one breath).
...
SLIDE=5
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="$42B observability market, 18% CAGR" \
  --prop x=1.5cm --prop y=1.2cm --prop width=30.87cm --prop height=2cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761 --prop fill=none
corecli add "$FILE" "/slide[$SLIDE]" --type chart --prop chartType=bar \
...
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text='Source: Gartner 2025 APM Magic Quadrant; SAM = 20% of TAM (K8s-first shops); SOM = 7.4% of SAM over 5 years at 18-24% share.' \
...
corecli add "$FILE" "/slide[$SLIDE]" --type picture --prop src=product_hero.png \
  --prop x=1cm --prop y=4cm --prop width=18cm --prop height=13cm
corecli set "$FILE" "/slide[$SLIDE]/picture[1]" --prop alt="Product UI: dashboard with 12 K8s clusters, live correlation graph"
# Right column bullets (each as a separate shape so sizes stay explicit)
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Auto-correlate across 3 data planes" \
...
**QA.** For Series B+, all four of {CAC, LTV, payback, GM} present: `corecli query "$FILE" 'shape:contains("CAC")'` ≥ 1 AND `shape:contains("LTV")'` ≥ 1 AND `shape:contains("payback")'` ≥ 1 AND `shape:contains("gross margin")'` ≥ 1.
...
SLIDE=7
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text='ARR: $0 → $18M in 24 months' \
  --prop x=1.5cm --prop y=1.2cm --prop width=30.87cm --prop height=2cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761 --prop fill=none
corecli add "$FILE" "/slide[$SLIDE]" --type chart --prop chartType=line \
  ...
  --prop axismin=0
# Right callout
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop geometry=roundRect --prop fill=1E2761 --prop line=none \
  --prop x=23.5cm --prop y=4cm --prop width=8.8cm --prop height=13cm
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text='$18M' \
...
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="ARR · +312% YoY · NRR 128%" \
...
**QA.** ARR curve chart must carry `axismin=0`. `corecli get "$FILE" "/slide[$SLIDE]/chart[1]" --json | jq .format.axisMin` returns `0` (CLI emits camelCase `axisMin` in readback even though input prop is lowercase `axismin`).
...
SLIDE=11
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Team: 3 prior exits, 42 years combined K8s" \
  --prop x=1.5cm --prop y=1.2cm --prop width=30.87cm --prop height=2cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761 --prop fill=none
# Card 1 — CEO
corecli add "$FILE" "/slide[$SLIDE]" --type picture --prop src=alice.jpg \
  --prop x=2cm --prop y=5cm --prop width=6cm --prop height=6cm
corecli set "$FILE" "/slide[$SLIDE]/picture[1]" --prop alt="Alice Chen, CEO — portrait"
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Alice Chen" \
...
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="CEO" \
...
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="ex-Datadog Director (Series C → IPO); led K8s observability GTM $40M → $200M ARR" \
...
**QA.** `corecli query "$FILE" 'shape:contains("ex-")'` + `'shape:contains("prior")'` + `'shape:contains("former")'` ≥ 1 per team member. If zero, you have a portfolio, not a team.
...
SLIDE=17
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text='$18M → $85M ARR by FY29' \
  --prop x=1.5cm --prop y=1.2cm --prop width=30.87cm --prop height=2cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761 --prop fill=none
corecli add "$FILE" "/slide[$SLIDE]" --type chart --prop chartType=column \
...
```bash
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop geometry=roundRect --prop fill=F5F7FA --prop line=none \
  --prop x=22.5cm --prop y=4cm --prop width=9.8cm --prop height=13cm
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Key Assumptions" \
...
**QA.** `corecli query "$FILE" 'shape:contains("assumption")'` OR `'shape:contains("Assumes")'` ≥ 1. If zero, add the panel.
...
SLIDE=20
corecli add "$FILE" / --type slide --prop layout=blank --prop background=1E2761
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text='$35M Series B' \
...
corecli add "$FILE" "/slide[$SLIDE]" --type chart --prop chartType=pie \
...
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text='18 months runway to $40M ARR and Series C' \
...
**QA.** `corecli query "$FILE" 'shape:contains("Use of Funds")'` ≥ 1. Pie chart present on ask slide. Runway + milestone on ask slide.
...
SLIDE=6
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Pipeline: 3 candidates across Ph1–Ph3" \
  --prop x=1.5cm --prop y=1.2cm --prop width=30.87cm --prop height=2cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761 --prop fill=none
...
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text='NCT05021323 (HLX-201, Ph2, n=48) · NCT06142091 (HLX-304, Ph1, n=24) · IND-filed Q1-26 for HLX-412' \
  --prop x=1.5cm --prop y=16.8cm --prop width=30.87cm --prop height=1.2cm \
  --prop font=Calibri --prop size=12 --prop italic=true --prop color=666666 --prop fill=none
```

**QA.** `corecli query "$FILE" 'shape:contains("NCT")' --json | jq '.data.results | length'` ≥ 1. Bar colors darken across stages (`CADCFC` preclinical-only, `1E2761` Ph2-reached).
...
SLIDE=13
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[$SLIDE]" --type shape --prop text="Competitive landscape" \
  --prop x=1.5cm --prop y=1.2cm --prop width=30.87cm --prop height=2cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761 --prop fill=none
...
corecli add "$FILE" "/slide[$SLIDE]" --type table \
...
**QA.** `corecli query "$FILE" 'table' --json | jq '.data.results | length'` ≥ 1. Row count ≥ 4 (you + ≥ 3 named competitors). Your row visually distinct via cell fill (Gate 5b visual check — table style alone does not highlight one row).
...
STRIP=$(corecli view "$FILE" text | grep -niE '(^|[^A-Za-z0-9])M (ARR|raised|Series|runway|round|raise)|Series [A-C] · M( |$)|runway · M|raised · M|raising ·? M')
...
Run `corecli view "$FILE" html` and Read the returned HTML. Walk every slide and answer, for EACH (inherits pptx v2 Gate 5b checklist; pitch-specific additions marked ⭐):
...
**Instruction.** Run `corecli view "$FILE" html` and Read the HTML. Walk every slide against the questions below. If rendering chart colors, animations, or zoom — those only show in the target viewer (PowerPoint / Keynote / WPS); ask the user to open `.pptx` directly for those runtime features.
...
**Human preview (optional).** If you want the user to visually preview the deck, run `corecli watch "$FILE"` for a live preview the user can open at their own discretion, or have them open the `.pptx` directly in PowerPoint / WPS / Keynote. For final visual verification, open the file in the target presentation viewer.
...
LEAK=$(corecli view "$FILE" text | grep -niE 'TBD|lorem|\(fill in\)|xxxx|coming soon|placeholder')
[ -z "$LEAK" ] && echo "Gate 6.1 OK (no placeholder tokens)" || { echo "REJECT Gate 6.1:"; echo "$LEAK"; exit 1; }

# 6.2 — TAM / SAM / SOM presence (Series A+)
TAM_HIT=$(corecli query "$FILE" 'shape:contains("TAM")' --json | jq '.data.results | length')
...
CAC_HIT=$(corecli query "$FILE" 'shape:contains("CAC")' --json | jq '.data.results | length')
LTV_HIT=$(corecli query "$FILE" 'shape:contains("LTV")' --json | jq '.data.results | length')
...
UOF_HIT=$(corecli query "$FILE" 'shape:contains("Use of Funds")' --json | jq '.data.results | length')
...
PRIOR_HIT=$(corecli view "$FILE" text | grep -ciE '\b(ex-|former|prior|previously)\b')
...
AXISMIN_HIT=$(corecli query "$FILE" 'chart' --json | jq '[.data.results[]? | select(.format.axisMin == "0" or .format.axisMin == 0 or .format.axismin == "0" or .format.axismin == 0)] | length')
...
When in doubt: `corecli help pptx`, `corecli help pptx <element>`, `corecli help pptx <element> --json`. Help is the authoritative schema; this skill is the decision guide for fundraising deltas on top of pptx v2.
