---
name: corecli
description: Create, analyze, proofread, and modify Office documents (.docx, .xlsx, .pptx) using the corecli CLI tool. Use when the user wants to create, inspect, check formatting, find issues, add charts, or modify Office documents.
---
...
Verify with `corecli --version`. If still not found after install, open a new terminal.
...
`corecli help` ≡ `corecli --help`, and `corecli <cmd> --help` ≡ `corecli help <cmd>` — same content.

```bash
corecli help                                  # All commands + global options + schema entry points
corecli help docx                             # List all docx elements
corecli help docx paragraph                   # Full schema: properties, aliases, examples, readbacks
corecli help docx set paragraph               # Verb-filtered: only props usable with `set`
corecli help docx paragraph --json            # Structured schema (machine-readable)
```
...
```bash
corecli open report.docx       # explicitly keep in memory
corecli set report.docx ...    # no file I/O overhead
corecli close report.docx      # save and release
```

Opt out of auto-start: `CORECLI_NO_AUTO_RESIDENT=1`.
...
**PPT:**
```bash
corecli create slides.pptx
corecli add slides.pptx / --type slide --prop title="Q4 Report" --prop background=1A1A2E
corecli add slides.pptx '/slide[1]' --type shape --prop text="Revenue grew 25%" --prop x=2cm --prop y=5cm --prop font=Arial --prop size=24 --prop color=FFFFFF
```

**Word:**
```bash
corecli create report.docx
corecli add report.docx /body --type paragraph --prop text="Executive Summary" --prop style=Heading1
corecli add report.docx /body --type paragraph --prop text="Revenue increased by 25% year-over-year."
```

**Excel:**
```bash
corecli create data.xlsx
corecli set data.xlsx /Sheet1/A1 --prop value="Name" --prop bold=true
corecli set data.xlsx /Sheet1/A2 --prop value="Alice"
```
...
```bash
corecli create <file>               # Create blank .docx/.xlsx/.pptx (type from extension)
corecli view <file> <mode>          # outline | stats | issues | text | annotated | html
corecli get <file> <path> --depth N # Get a node and its children [--json]
corecli query <file> <selector>     # CSS-like query
corecli validate <file>             # Validate against OpenXML schema
```
...
```bash
corecli get report.docx '/body/p[3]' --depth 2 --json
corecli get slides.pptx '/slide[1]' --depth 1          # list all shapes on slide 1
corecli get data.xlsx '/Sheet1/B2' --json
```
...
```bash
corecli query report.docx 'paragraph[style=Normal] > run[font!=Arial]'
corecli query slides.pptx 'shape[fill=FF0000]'
```
...
```bash
corecli watch <file> [--port N]      # Start preview server (default port 26315)
corecli unwatch <file>               # Stop
corecli goto <file> <path>           # Scroll watching browser(s) to element (docx: p / table / tr / tc)
```
...
```bash
corecli get <file> selected [--json]
```
...
PATHS=$(corecli get deck.pptx selected --json | jq -r '.data.Results[].path')
for p in $PATHS; do corecli set deck.pptx "$p" --prop fill=FF0000; done
```
...
```bash
corecli mark <file> <path> [--prop find=... color=... note=... tofix=... regex=true] [--json]
corecli unmark <file> [--path <p> | --all] [--json]
corecli get-marks <file> [--json]
```
...
```bash
corecli set <file> <path> --prop key=value [--prop ...]
```
...
**Dotted-attr aliases** — `font.<attr>` forms accepted on shape/run/paragraph/table/row/cell/section/styles, e.g. `--prop font.color=red --prop font.bold=true --prop font.size=14pt`. Run `corecli help <fmt> <element>` for the full list.
...
corecli set doc.docx '/body/p[1]' --find weather --prop bold=true --prop color=red

# Regex matching (regex= still a prop flag)
corecli set doc.docx '/body/p[1]' --find '\d+%' --prop regex=true --prop color=red

# Replace text (use `/` for whole-document scope)
corecli set doc.docx / --find draft --replace final

# docx: tracked Find&Replace
corecli set doc.docx / --find draft --replace final --prop revision.author=Alice

# PPT — same syntax, different paths
corecli set slides.pptx / --find draft --replace final
```
...
```bash
corecli add <file> <parent> --type <type> [--prop ...]
corecli add <file> <parent> --type <type> --after <path> [--prop ...]   # insert after anchor
corecli add <file> <parent> --type <type> --before <path> [--prop ...]  # insert before anchor
corecli add <file> <parent> --type <type> --index N [--prop ...]        # 0-based position (legacy)
corecli add <file> <parent> --from <path>                               # clone existing element
```
...
```bash
corecli add data.xlsx /Sheet1 --type pivottable \
...
Key props: `rows`, `cols`, `values` (Field:func[:showDataAs]), `filters`, `source`, `position`, `layout` (compact/outline/tabular), `repeatLabels`, `blankRows`, `aggregate`, `showDataAs` (percent_of_total/row/col, running_total), `grandTotals`, `subtotals`, `sort`. Aggregators: sum, count, average, max, min, product, stdDev, stdDevp, var, varp, countNums. Date columns auto-group. Run `corecli help xlsx pivottable` for full schema.

### Document-level properties (all formats)

```bash
corecli set doc.docx / --prop docDefaults.font=Arial --prop docDefaults.fontSize=11pt
corecli set doc.docx / --prop protection=forms --prop evenAndOddHeaders=true
corecli set data.xlsx / --prop calc.mode=manual --prop calc.refMode=r1c1
corecli set slides.pptx / --prop defaultFont=Arial --prop show.loop=true --prop print.what=handouts
```

Run `corecli help <format> /` for all document-level properties (docDefaults, docGrid, CJK spacing, calc, print, show, theme, extended).

### Sort (xlsx)

```bash
corecli set data.xlsx /Sheet1 --prop sort="C desc" --prop sortHeader=true
corecli set data.xlsx '/Sheet1/A1:D100' --prop sort="A asc" --prop sortHeader=true
```
...
corecli add doc.docx '/body/p[1]' --type run --after find:weather --prop text=" (sunny)"

# Word: block table after matched text (auto-splits paragraph)
corecli add doc.docx '/body/p[1]' --type table --after "find:First sentence." --prop rows=2 --prop cols=2
```

### Clone

`corecli add <file> / --from '/slide[1]'` — copies with all cross-part relationships.

### move, swap, remove

```bash
corecli move <file> <path> [--to <parent>] [--index N] [--after <path>] [--before <path>]
corecli swap <file> <path1> <path2>
corecli remove <file> '/body/p[4]'
```
...
`corecli dump <file> [<path>]` emits a replayable batch JSON for round-trip — `.docx` (full coverage) and `.pptx` (text/tables/pictures/charts/notes/theme + OLE/3D/video/audio/SmartArt/morph/p15 transitions via raw-set passthrough). Path defaults to `/` (whole document); pass a subtree path (`/body`, `/body/p[N]`, `/body/tbl[N]`, `/theme`, `/settings`, `/numbering`, `/styles`) to scope the dump. `corecli refresh <file.docx>` recalculates TOC page numbers / PAGE / cross-references after replay (Word backend on Windows; headless-HTML fallback elsewhere). `corecli plugins list` extends support to `.doc`, `.hwpx`, `.pdf` export.

```bash
echo '[
  {"command":"set","path":"/Sheet1/A1","props":{"value":"Name","bold":"true"}},
  {"command":"set","path":"/Sheet1/B1","props":{"value":"Score","bold":"true"}}
]' | corecli batch data.xlsx --json

corecli batch data.xlsx --commands '[{"op":"set","path":"/Sheet1/A1","props":{"value":"Done"}}]' --json
corecli batch data.xlsx --input updates.json --force --json
```
...
```bash
corecli raw <file> <part>                          # view raw XML
corecli raw-set <file> <part> --xpath "..." --action replace --xml '<w:p>...</w:p>'
corecli add-part <file> <parent>                   # create new document part (returns rId)
```

`raw-set` actions: `append`, `prepend`, `insertbefore`, `insertafter`, `replace`, `remove`, `setattr`. Run `corecli help <format> raw` for available parts.
...
| Guessing property names | Run `corecli help <format> <element>` to see exact names |
...
`corecli load_skill <name>` — output is a SKILL.md, follow its rules.
...
Example: a fundraising deck task → `corecli load_skill pitch-deck` → use the printed rules.
...
- **When unsure**, run `corecli help <format> <element>` instead of guessing
