...
```bash
corecli add deck.pptx '/slide[N]' --type notes --prop text="..."
```
...
corecli remove deck.pptx '/slide[2]/model3d[1]'  # remove the frozen clone
corecli add deck.pptx '/slide[2]' --type 3dmodel ...  # add fresh model
```
...
5. **After building each slide, verify coordinates:**
   ```bash
   corecli get deck.pptx '/slide[N]' --depth 1
...
for pos in model_positions:
    run("corecli", "add", OUTPUT, f"/slide[{pos['slide']}]", "--type", "3dmodel",
