---
name: corecli-data-dashboard
description: "Use this skill to build a multi-element Excel dashboard — Dashboard sheet on open, multiple formula-driven KPI cards, multiple charts, sparklines, and conditional formatting — from CSV or tabular input. Trigger on: 'dashboard', 'KPI dashboard', 'analytics dashboard', 'executive dashboard', 'metrics dashboard', 'CSV to dashboard', 'data visualization'. Output is a single .xlsx. Scene-layer on corecli-xlsx: inherits every xlsx hard rule. DO NOT invoke for: a single budget tracker / one-sheet CSV-with-formatting (use xlsx), a 3-statement / DCF / LBO financial model (use financial-model), a weekly report with ≤ 1 chart and < 10 rows (use xlsx)."
---
...
```bash
corecli help xlsx                          # element list
corecli help xlsx chart                    # full schema for charts
corecli help xlsx sparkline                # sparklines
corecli help xlsx conditionalformatting    # all CF rule types
```
...
This skill **inherits every xlsx hard rule** from `corecli-xlsx` — shell quoting, zero formula errors, visual delivery floor, batch JSON shape (`{"command":"set"|"add","path":...,"props":{...}}` — key is `command`, NOT `action`), batch JSON dotted-name rule, chart data-feed forms, batch+resident limits, `validate` discipline. Read corecli-xlsx first; honour those rules, do not re-teach them here.

**Reverse handoff — do NOT use this skill when:**

- The ask is a **single-sheet CSV-with-formatting tracker** (no Dashboard sheet, no KPI cards, ≤ 1 chart) → go back to `corecli-xlsx`.
- The ask is a **3-statement / DCF / LBO financial model** with blue-inputs / black-formulas / cross-sheet drivers → use `corecli-financial-model`.
- The ask is a **weekly status report** with one SUMIF summary and one chart over < 10 rows → `corecli-xlsx`.
...
→ see corecli-xlsx §Shell & Execution Discipline for the baseline (quoting, heredoc for `!`, incremental execution).
...
- **Multi-instance counts use `query --json | jq length`, never `raw-get | grep -c`.** Example: `corecli query "$FILE" chart --json | jq '.data.results | length'` for "how many charts do I have?".
...
All `corecli-xlsx` requirements apply (→ see corecli-xlsx §Requirements for Outputs). Dashboards add these:

- **Dashboard sheet is the active tab on open.** Confirm 0-based sheet index with `corecli query "$FILE" sheet` BEFORE filling `activeTab="N"`. Never guess the index.
- **`calc.fullCalcOnLoad=true`.** Set via `corecli set "$FILE" / --prop calc.fullCalcOnLoad=true`. Do NOT `raw-set` `<calcPr>` — it produces duplicate elements that fail validate.
...
- **Every KPI value cell has a formula.** Verifiable: `corecli query "$FILE" 'Dashboard!:has(formula)' --json | jq '.data.results | length'` should equal your planned KPI count.
...
corecli create "$FILE"
corecli import "$FILE" /Sheet1 --file sales.csv --header
corecli set "$FILE" '/Sheet1/col[A]' --prop width=12
corecli set "$FILE" '/Sheet1/col[B]' --prop width=15
corecli set "$FILE" '/Sheet1/B2:B13' --prop numFmt='$#,##0'
corecli set "$FILE" '/Sheet1/A1:B1' --prop fill=1F3864 --prop font.color=FFFFFF --prop font.bold=true
```

**Phase 2 — Dashboard sheet + one KPI card.**

```bash
corecli add "$FILE" / --type sheet --prop name=Dashboard
corecli set "$FILE" '/Dashboard/col[A]' --prop width=22
corecli set "$FILE" '/Dashboard/col[B]' --prop width=12
corecli set "$FILE" /Dashboard/A1 --prop value="Total Revenue" --prop font.size=9 --prop font.color=666666 --prop bold=true
corecli set "$FILE" /Dashboard/A2 --prop 'formula==SUM(Sheet1!B2:B13)' --prop numFmt='$#,##0' --prop font.size=24 --prop bold=true --prop font.color=2E7D32
```

**Phase 3 — Sparkline + chart.**

```bash
corecli add "$FILE" /Dashboard --type sparkline --prop cell=B2 --prop range='Sheet1!B2:B13' --prop type=line --prop color=4472C4 --prop highPoint=true --prop highMarkerColor=FF0000
corecli add "$FILE" /Dashboard --type chart \
  --prop chartType=line \
...
```bash
corecli set "$FILE" / --prop calc.fullCalcOnLoad=true

# Resolve Dashboard's 0-based index from the actual sheet list — never hardcode.
DASH_IDX=$(corecli query "$FILE" sheet --json \
  | jq '[.data.results[].path] | index("/Dashboard")')
corecli raw-set "$FILE" /workbook --xpath "//x:sheets" --action insertbefore \
...
`corecli get chart[N]` exposes numeric `width` (e.g. `width=480pt`) alongside `anchor` (e.g. `"A6:K21"`) at `.data.results[0].format`. Either is usable for Gate 2 — derive column span from anchor letters (A→K = 10 cols) when you need a column-unit budget.
...
corecli add "$FILE" / --type namedrange --prop name=_xlnm.Print_Area --prop scope=Dashboard --prop 'refersTo=Dashboard!$A$1:$H$36'
# 2. fit-to-page on Dashboard.
corecli raw-set "$FILE" /Dashboard --xpath "//x:worksheet" --action prepend --xml '<sheetPr xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><pageSetUpPr fitToPage="1"/></sheetPr>'
# 3. Landscape page setup.
corecli raw-set "$FILE" /Dashboard --xpath "//x:sheetData" --action insertafter --xml '<pageSetup orientation="landscape" paperSize="9" fitToWidth="1" fitToHeight="1"/>'
...
Inherit the xlsx baseline (`view issues`, formula error queries, `validate`, HTML preview scan): → see corecli-xlsx §QA minimum cycle.
...
```bash
KPI_FORMULAS=$(corecli query "$FILE" 'Dashboard!:has(formula)' --json | jq '.data.results | length')
...
```bash
CHART_COUNT=$(corecli query "$FILE" chart --json | jq '.data.results | length')
...
for i in $(seq 1 "$CHART_COUNT"); do
  JSON=$(corecli get "$FILE" "/Dashboard/chart[$i]" --json)
...
```bash
for i in $(seq 1 "$CHART_COUNT"); do
  BAD=$(corecli get "$FILE" "/Dashboard/chart[$i]" --json | jq '[.data.results[0].children[]? | select(.type == "series") | select((.format.name // "") | test("^Series[0-9]+$"; "i"))] | length')
...
```bash
CF_COUNT=$(corecli query "$FILE" conditionalformatting --json | jq '.data.results | length')
...
```bash
DASH_IDX=$(corecli query "$FILE" sheet --json | jq '[.data.results[].path] | index("/Dashboard")')
ACTIVE=$(corecli get "$FILE" /workbook --json | jq '.data.results[0].format.activeTab // -1')
FULLCALC=$(corecli get "$FILE" /workbook --json | jq -r '.data.results[0].format["calc.fullCalcOnLoad"] // false')
...
```bash
LEAKS=$(corecli view "$FILE" text 2>/dev/null | grep -niE '\{\{|\$fy\$|<TODO>|xxxx|TBD' | wc -l | tr -d ' ')
[ "$LEAKS" -gt 0 ] && { echo "REJECT Gate 6: $LEAKS placeholder tokens"; exit 1; }
```

**Gate 7 — Visual delivery floor (ported from xlsx).** Run `corecli view "$FILE" html` and Read the returned HTML path. Confirm:
...
- Pie / doughnut slices render with distinct fill colors (if collapsed in LibreOffice, verify in the user's target viewer before declaring broken — → see corecli-xlsx §Known Issues/Renderer caveats).
...
```bash
# a) Token / ### sweep.
corecli view "$FILE" text 2>/dev/null | grep -nE '###|\{\{|<TODO>|\$fy\$|xxxx' && { echo "REJECT Gate 7: tokens or ### present"; exit 1; }
# b) Per-KPI: cachedValue length × coef must fit col width. coef=0.55 fit-to-page, 0.85 otherwise.
for CELL in A2 C2 E2 G2; do
  CV=$(corecli get "$FILE" "/Dashboard/$CELL" --json | jq -r '.data.results[0].format.cachedValue // .data.results[0].text // ""')
  W=$(corecli get "$FILE" "/Dashboard/col[${CELL%%[0-9]*}]" --json | jq -r '.data.results[0].format.width // 0')
...
if echo "$USER_REQ" | grep -qiE 'print|一页|投资人|董事会|board'; then
  ...
  while IFS=$'\t' read -r SPATH SNAME; do
    ...
    HIDDEN=$(corecli get "$FILE" "$SPATH" --json | jq -r '.data.results[0].format.hidden // false')
    [ "$HIDDEN" != "true" ] && LEAKING="$LEAKING $SNAME"
  done < <(corecli query "$FILE" 'sheet' --json | jq -r '.data.results[] | [.path, .preview] | @tsv')
  ...
  PA=$(corecli query "$FILE" 'namedrange[name="_xlnm.Print_Area"]' --json | jq '.data.results | length')
...
```bash
for CELL in A2 C2 E2 G2; do
  JSON=$(corecli get "$FILE" "/Dashboard/$CELL" --json)
...
- **Full schemas live in help:** `corecli help xlsx chart` / `sparkline` / `conditionalformatting`. This skill does not mirror them.
...
| D-7 | `calc.fullCalcOnLoad` via `raw-set` creates duplicate `<calcPr>` → validate fails | Use `corecli set "$FILE" / --prop calc.fullCalcOnLoad=true`. |
...
| D-16 | `fullCalcOnLoad=true` guarantees a **runtime** recalc when the end user opens the file; it does NOT refresh the build-time `cachedValue` in XML. Build sequence `set B=100 → set E==B+D → fix B=150` leaves `E.cachedValue` stale (board sees "Net Change = 0"). | After all upstream edits are final, re-issue every downstream formula (`corecli set "$FILE" /Sheet/E2 --prop formula==B2+D2`) OR `close` + re-open the file. Gate 8 verifies. |
...
Cross-sheet `!` trap, batch + resident for formulas, `labelRotation` on axis-by-role, `chartType=pareto`, `validate` while resident, data bar without explicit `min`/`max`, chart `anchor` / series immutability after create → see corecli-xlsx §Known Issues.
