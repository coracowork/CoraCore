...
```powershell
if (-not (Get-Command corecli -ErrorAction SilentlyContinue)) {
    irm https://d.officecli.ai/install.ps1 | iex
}
```

Verify: `corecli --version`

If `corecli` is still not found after first install, open a new terminal and run the verify command again.

If the install command above fails (e.g. blocked by security policy, no network access, or insufficient permissions), install manually — download the binary for your platform from https://github.com/coracowork/CoraCLI/releases — then re-run the verify command.

## Help-First Rule

This skill teaches what a real form needs, not every CLI flag. When a prop / alias / enum is uncertain, consult help BEFORE guessing: `corecli help docx [element] [--json]` (e.g. `sdt`, `formfield`, `field`). Help is pinned to the installed CLI version and is authoritative — when this skill and help disagree, **help wins** (the prop set on `sdt` in particular has grown over time; trust `help docx sdt`, not a hardcoded list).
...
**Reverse handoff to docx.** Route back to `corecli-docx` for reports / letters / memos / thesis / pitch decks / any document with no editable fields. Use **this** skill when the document's purpose is data capture or template merge.

## Shell & Execution Discipline

**One command at a time. Read output before the next.** CoreCLI is incremental — every `add` / `set` / `remove` immediately mutates the file. All recipes below use `FILE=form.docx` as a shell variable.

**Three shell-escape layers:**

1. **Quote every path with `[N]`** — zsh/bash glob-expand brackets. `corecli get "$FILE" /body/sdt[1]` fails with `no matches found`. Correct: `corecli get "$FILE" '/body/sdt[1]'`.
...
2. **After insert, `/body/p[last()]` is unreliable** — the find insertion changes `<w:body>` child order. To continue operating on the new paragraph, read its real paraId: `corecli query "$FILE" paragraph --json | jq -r '.data.results[-1].format.paraId'`.
3. **Chinese + full-width parens `（）`** match literally in `find`, but when unsure, `corecli view "$FILE" text | grep -n "锚点"` first to confirm the exact bytes in the file.

```bash
# Trap: first-match hits 甲方 only, 乙方 missed
corecli add "$FILE" /body --type sdt --after 'find:签字'

# Fix: two signatories, two unique anchors
corecli add "$FILE" /body --type sdt --prop alias=Party_A_Name --prop tag=party_a \
  --after 'find:甲方签字（Service Provider）'
PID_A=$(corecli query "$FILE" paragraph --json | jq -r '.data.results[-1].format.paraId')
corecli add "$FILE" "/body/p[@paraId='$PID_A']" --type sdt --prop alias=Party_A_Title --prop tag=party_a_title
```
...
corecli add "$FILE" /body --type sdt \
...
corecli open "$FILE"
corecli view "$FILE" forms                 # inspect embedded controls + paths
corecli set "$FILE" '/body/sdt[@sdtId=3]' --prop text="Jane Smith"
corecli set "$FILE" / --prop protection=forms
```
...
```bash
FILE=intake.docx
corecli close "$FILE" 2>/dev/null; rm -f "$FILE"   # preflight: clear stale resident / prior file (cold-start after CLI upgrade commonly leaks a resident)
corecli create "$FILE"
corecli open "$FILE"

corecli set "$FILE" / --prop title="Employee Onboarding Intake" \
  --prop docDefaults.font="Calibri" --prop docDefaults.fontSize="12pt"

corecli add "$FILE" /body --type paragraph \
...
corecli add "$FILE" /body --type sdt --prop type=text \
...
corecli add "$FILE" /body --type sdt --prop type=date \
  --prop alias="Start Date" --prop tag=start_date

corecli add "$FILE" /body --type paragraph \
  --prop text="Read and agree to employee handbook" --prop size=11 --prop spaceAfter=4pt
corecli add "$FILE" /body --type formfield \
  --prop type=checkbox --prop name=agree_handbook --prop checked=false

corecli set "$FILE" '/body/sdt[1]' --prop lock=sdtlocked
corecli set "$FILE" '/body/sdt[2]' --prop lock=sdtlocked
corecli set "$FILE" / --prop protection=forms
corecli close "$FILE"
corecli view "$FILE" forms
```
...
```bash
# Skeleton (Path A)
corecli add "$FILE" /body --type sdt --prop type=dropdown \
...
# Verify
corecli get "$FILE" '/body/sdt[1]'   # expect: type=dropdown items=Engineering,Finance,HR
```
...
```bash
corecli add "$FILE" /body --type sdt --prop type=combobox \
...
corecli get "$FILE" '/body/sdt[N]'   # expect: type=date format=yyyy年MM月dd日
```
...
| Symptom | Fix |
|---|---|
| `raw-set: 0 element(s) affected` | XPath did not match. Check the `tag` value and whether the SDT is block or inline. Fall back to `corecli raw $FILE /document` to read the real XML. |
...
cp templates/onboarding_with_signature.docx "$FILE"
corecli open "$FILE"
corecli view "$FILE" forms                                    # see /body/... paths + sdtId values
corecli set "$FILE" '/body/sdt[@sdtId=3]' --prop text="Jane Smith"
corecli set "$FILE" / --prop protection=forms
corecli close "$FILE"
```
...
```bash
corecli add "$FILE" /body --type paragraph --prop text="Dear "
corecli add "$FILE" '/body/p[1]' --type field --prop fieldType=mergefield --prop name=CustomerName
corecli add "$FILE" '/body/p[1]' --type run --prop text=", "
corecli add "$FILE" '/body/p[1]' --type field --prop fieldType=mergefield --prop name=CompanyName
# Readback: "Dear «CustomerName», «CompanyName»"
```

**Element-type shortcut** (equivalent): `corecli add "$FILE" '/body/p[1]' --type mergefield --prop name=CustomerName`.
...
```bash
corecli add "$FILE" /body --type paragraph --prop text=""
corecli add "$FILE" '/body/p[last()]' --type field --prop fieldType=if \
  --prop expression='{ MERGEFIELD Gender } = "Male"' \
  --prop trueText="Mr." --prop falseText="Ms."
corecli add "$FILE" '/body/p[last()]' --type run --prop text=" "
corecli add "$FILE" '/body/p[last()]' --type field --prop fieldType=mergefield --prop name=LastName
...
corecli add "$FILE" /body --type formfield --prop type=checkbox \
  --prop name=agree_terms --prop checked=false

# TEXT formfield
corecli add "$FILE" /body --type formfield --prop type=text \
  --prop name=emp_name --prop text="Enter name"

# DROPDOWN formfield — items NOT settable via CLI; use Word template or SDT Path B
corecli add "$FILE" /body --type formfield --prop type=dropdown --prop name=dept_select

# Read / modify by name (stable) or 1-based index
corecli get "$FILE" '/formfield[agree_terms]'
corecli set "$FILE" '/formfield[agree_terms]' --prop checked=true
corecli set "$FILE" '/formfield[emp_name]' --prop text="Jane Smith"
corecli set "$FILE" '/formfield[dept_select]' --prop text="Engineering"
```
...
```bash
corecli set "$FILE" / --prop protection=forms
corecli get "$FILE" /                                  # look for: protectionEnforced=True
```
...
```bash
corecli set "$FILE" '/body/sdt[1]' --prop lock=sdtlocked           # content editable; control cannot be deleted
corecli set "$FILE" '/body/sdt[1]' --prop lock=contentlocked       # content read-only; control can be deleted
corecli set "$FILE" '/body/sdt[1]' --prop lock=sdtcontentlocked    # both locked
...
PID=$(corecli query "$FILE" paragraph --json | jq -r '.data.results[-1].format.paraId')
...
# Simpler alternative: read the paragraph XML via `corecli raw`, then `replace` the whole <w:p> with <w:sdt>...<w:sdtContent>[original w:p]</w:sdtContent></w:sdt>:
...
PARA_XML=$(corecli raw "$FILE" /document | python3 -c '
...
corecli set "$FILE" '/body/sdt[1]' --prop lock=sdtlocked      # patient_name
corecli set "$FILE" '/body/sdt[2]' --prop lock=sdtlocked      # patient_dob

# Physician section — locked against patient edits
corecli set "$FILE" '/body/sdt[14]' --prop lock=contentLocked # physician_diagnosis
corecli set "$FILE" '/body/sdt[15]' --prop lock=contentLocked # physician_signature
```
...
corecli create "$FILE"
corecli open "$FILE"
corecli set "$FILE" / --prop title="Statement of Work" \
  --prop docDefaults.font="Calibri" --prop docDefaults.fontSize="12pt"

corecli add "$FILE" /body --type paragraph --prop text="Statement of Work" \
...
corecli add "$FILE" /body --type paragraph --prop text="Customer: "
corecli add "$FILE" '/body/p[last()]' --type field \
  --prop fieldType=mergefield --prop name=CustomerName
corecli add "$FILE" /body --type paragraph --prop text="Contract #: "
corecli add "$FILE" '/body/p[last()]' --type field \
...
corecli add "$FILE" /body --type paragraph --prop text="Client Signature:" \
...
```bash
corecli add "$FILE" / --type watermark \
  --prop text="CONFIDENTIAL" --prop color=FF0000 --prop rotation=315

corecli set "$FILE" '/body/sdt[1]' --prop lock=sdtlocked
corecli set "$FILE" '/body/sdt[2]' --prop lock=sdtlocked
corecli set "$FILE" '/body/sdt[3]' --prop lock=sdtlocked
corecli view "$FILE" forms   # copy signatory_name path, then: set '/body/p[@paraId=...]/sdt[1]' --prop lock=sdtlocked

corecli set "$FILE" / --prop protection=forms
corecli close "$FILE"
corecli query "$FILE" field     # expect 2 MERGEFIELDs: CustomerName, ContractNo
```
...
**Header / footer note.** Headers/footers are **predefined** when the section is created (default/first/even, 3 each). The first mutation must be `set` against the existing part, not `add` — `add $FILE /header ...` returns `already exists` or silently no-ops. Inspect first with `corecli query "$FILE" header --json` to read the `type` values, then `corecli set "$FILE" '/header[@type=default]' --prop text=...`. Only use `add` when creating an additional section with its own header/footer.
...
```bash
# Assumes FILE=<your-form.docx>, document has been closed with corecli close "$FILE"

# Gate 1 — Validate (must be clean; protection=forms no longer produces a schema error)
VAL_OUT=$(corecli validate "$FILE" 2>&1)
...
LEAK=$(corecli view "$FILE" text | grep -niE '_{3,}|TBD|\(fill in\)|\{\{|xxxx|lorem|placeholder')
...
SDT_N=$(corecli query "$FILE" sdt --json | jq '.data.results | length')
FF_N=$(corecli query "$FILE" formfield --json | jq '.data.results | length')
FLD_N=$(corecli query "$FILE" field --json | jq '.data.results | length')
...
SDT_MISSING=$(corecli query "$FILE" sdt --json | jq '[.data.results[] | select(.format.alias == null or .format.alias == "" or .format.tag == null or .format.tag == "")] | length')
...
PROT=$(corecli get "$FILE" / --json | jq -r '.data.results[0].format.protection // "none"')
[ "$PROT" = "forms" ] && echo "Gate 5 OK (protection=forms enforced)" || { echo "REJECT Gate 5: protection is '$PROT', expected 'forms'"; exit 1; }
corecli view "$FILE" forms | head -40   # visual spot-check: every dropdown shows items=; every date shows format=; every locked SDT shows lock=

# Gate 6 — No type=checkbox leaked onto any SDT
BAD_CB=$(corecli query "$FILE" sdt --json | jq '[.data.results[] | select(.format.type == "checkbox")] | length')
...
| K16 | Header / footer are predefined at section creation (default/first/even, 3 each) | `add $FILE /header ...` returns `already exists` or silently no-ops on the first call | First mutation uses `set` against the existing part: `corecli query $FILE header --json` to read `type`, then `set '/header[@type=default]' --prop text=...`. Only use `add` for a brand-new section's header/footer |
| K17 | Watermark injected into header emits `<w:noProof>` child that is schema-invalid | `validate` adds an extra `[Schema]` error at `/header[N]/w:sdt/.../w:noProof` — a real schema error to fix, not a benign one | After `add $FILE / --type watermark`, run once per header part: `corecli raw-set $FILE /word/header1.xml --xpath "//w:noProof" --action remove` (repeat for `header2.xml`, `header3.xml` if present) |
...
When in doubt: `corecli help docx`, `corecli help docx <element>`, `corecli help docx <element> --json`. Help is the authoritative schema; this skill is the decision guide for building real fillable Word forms on top of it.
