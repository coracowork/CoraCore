...
```bash
corecli help pptx                         # List all pptx elements
corecli help pptx <element>               # Full element schema (e.g. shape, chart, animation, connector, zoom, group)
corecli help pptx <verb> <element>        # Verb-scoped (e.g. add shape, set slide)
...
1. **Open/close mode.** Always `corecli open <file>` at start + `corecli close <file>` at end. Resident is the default, not an optimization. Use `batch` for repetitive shape grids.
2. **Orient.** New deck: `corecli create "$FILE"`. Existing: `corecli view "$FILE" outline` first. Never edit blind.
...
7. **Close + verify.** `corecli close` writes the ZIP. Always open in the target presentation viewer before shipping — chart colors, animations, fonts, and zoom are runtime features `view html` can't render. Full verification in QA below.
...
corecli add "$FILE" /slide[1] --type shape --prop text="FY26 Strategic Review" \
...
corecli add "$FILE" /slide[2] --type shape --prop text="Revenue grew 18% YoY" \
  --prop x=1.5cm --prop y=1.2cm --prop width=30cm --prop height=2cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761
corecli add "$FILE" /slide[2] --type shape --prop text="Enterprise renewals + new EMEA region drove the beat; NRR held at 118%." \
  --prop x=1.5cm --prop y=4cm --prop width=30cm --prop height=3cm \
  --prop font=Calibri --prop size=20 --prop color=333333
corecli add "$FILE" /slide[2] --type notes --prop text="Lead with the 18% beat, preview EMEA."
...
```bash
corecli view "$FILE" outline          # slide count + titles
corecli view "$FILE" annotated        # complete per-slide breakdown with fonts, sizes, tables, charts
corecli view "$FILE" text --start 1 --end 5   # text dump (includes table cell text)
corecli view "$FILE" issues           # empty slides, overflow hints
corecli view "$FILE" stats            # counts + totals (incl. pictures missing alt)
```
...
```bash
corecli get "$FILE" "/slide[1]" --depth 1              # shape list with IDs and names
corecli get "$FILE" "/slide[1]/shape[@name=Title]"
corecli get "$FILE" "/slide[1]/table[1]" --depth 3     # table rows / cells
```

**Query across the deck.** CSS-like selectors; operators `=`, `!=`, `~=`, `>=`, `<=`, `[attr]`, `:contains()`, `:no-alt`. `help pptx query` lists queryable element types.

```bash
corecli query "$FILE" 'shape:contains("Revenue")'
corecli query "$FILE" 'picture:no-alt'                 # accessibility gap
corecli query "$FILE" 'shape[fill=1E2761]'             # color match
corecli query "$FILE" 'shape[width>=10cm]'             # numeric
```
...
```bash
corecli view "$FILE" html                # prints an HTML preview path; Read it for per-slide visual audit (best structural ground truth)
corecli view "$FILE" svg --start 3 --end 3   # single slide SVG (charts + gradients do NOT render in SVG)
```
...
```bash
corecli add "$FILE" / --type slide --prop layout=blank --prop background=1E2761                 # solid
corecli add "$FILE" / --type slide --prop layout=blank --prop "background=1E2761-CADCFC-180"   # gradient (start-end-angle)
corecli add "$FILE" / --type slide --prop layout=blank --prop background=image:/path/to/hero.jpg  # image background (LEAD)
```
...
```bash
corecli add "$FILE" /slide[2] --type shape --prop name=Title --prop text="Key Insight" \
...
```bash
corecli add "$FILE" "/slide[2]/shape[@name=Card1]/paragraph[1]" --type run \
...
```bash
corecli add "$FILE" /slide[3] --type chart --prop chartType=column \
...
```bash
corecli add "$FILE" /slide[4] --type picture --prop src=hero.jpg \
  --prop x=1cm --prop y=1cm --prop width=32cm --prop height=18cm \
  --prop alt="Product hero, gradient lit from right"
```

Confirm with `corecli query "$FILE" 'picture:no-alt'` — must be empty before delivery.
...
```bash
corecli add "$FILE" /slide[5] --type connector \
...
```bash
corecli set "$FILE" "/slide[2]/shape[@name=HeroCard]" --prop animation=fade-entrance-400
corecli set "$FILE" "/slide[2]/shape[@name=HeroCard]" --prop animation=none    # clear all
```
...
- **Slide comments** — reviewer annotations anchored at `/slide[N]/comment[M]`. Full lifecycle (`add / set / get / query / remove`). Props: `text`, `author`, `initials` (auto-derived), `date` (ISO 8601, defaults to UtcNow), `x` / `y` (length anchor).
  ```bash
  corecli add "$FILE" "/slide[2]" --type comment --prop author="Alice" --prop text="Tighten this bullet" --prop x=20cm --prop y=3cm
  corecli query "$FILE" 'comment' --json | jq '.data.results | length'   # count all review comments
  corecli remove "$FILE" "/slide[2]/comment[1]"                           # resolve after addressing
  ```
...
```bash
corecli add "$FILE" / --type slide --prop layout=blank --prop background=1E2761
corecli add "$FILE" "/slide[last()]" --type shape --prop text="Strategic Growth Review" \
  --prop x=2cm --prop y=7cm --prop width=29.87cm --prop height=3cm \
  --prop font=Georgia --prop size=44 --prop bold=true --prop color=FFFFFF --prop align=center
corecli add "$FILE" "/slide[last()]" --type shape --prop text="Prepared for Acme Leadership — FY26 Outlook" \
...
```bash
corecli add "$FILE" / --type slide --prop layout=blank --prop background=FFFFFF
corecli add "$FILE" "/slide[last()]" --type shape --prop text="FY26 Revenue Beat Plan by 18%" \
  --prop x=1.5cm --prop y=1cm --prop width=30cm --prop height=1.8cm \
  --prop font=Georgia --prop size=36 --prop bold=true --prop color=1E2761

# Chart — left 2/3 (single-quote the title because of `$`)
corecli add "$FILE" "/slide[last()]" --type chart --prop chartType=column \
...
corecli add "$FILE" "/slide[last()]" --type shape --prop preset=roundRect --prop fill=F5F7FA --prop line=none \
  --prop x=22.5cm --prop y=3.5cm --prop width=9.8cm --prop height=14cm
corecli add "$FILE" "/slide[last()]" --type shape --prop text="Key Insight" \
  --prop x=23cm --prop y=4cm --prop width=9cm --prop height=1.2cm \
  --prop font=Georgia --prop size=20 --prop bold=true --prop color=1E2761
corecli add "$FILE" "/slide[last()]" --type shape --prop text="EMEA launch + NRR at 118% drove 12pp of the 18pp beat." \
...
for pair in "Step1 Step2" "Step2 Step3" "Step3 Step4"; do
  A=${pair% *}; B=${pair#* }
  corecli add "$FILE" "/slide[$SLIDE]" --type connector \
...
Each gate is **run a command, judge its output** — the corecli commands are identical on every OS (macOS / Linux / Windows), so no shell scripting is needed; the judging is yours.

- **Gate 1 — schema.** `corecli validate "<file>"`. Any schema error → REJECT and fix.
- **Gate 2 — overflow / format / structure.** `corecli view "<file>" issues`. If it lists *any* issue (lines tagged `[O1]`, `[C1]`, `[S1]`, …) → REJECT, fix, re-run until clean.
- **Gate 2b — leftover placeholders.** `corecli view "<file>" text`, then scan the output for `xxxx`, `lorem` / `ipsum`, `<TODO>`, `placeholder`, "this slide layout", or empty `()` / `[]`. Any hit → REJECT.

### Gate 3 — Visual audit (MANDATORY)

Pick **one** path:

**Screenshot (default)** — needs image-Read + a headless browser. Screenshot each slide in turn — `corecli view "<file>" screenshot --page 1 -o slide1.png`, then `--page 2`, … — until the page index runs past the deck (one screenshot = one slide). If it errors on page 1, there's no headless backend → use the fallback below.
...
**Optional `--grid N`** — only on user request for layout-rhythm, or when `view outline` shows anomalous layout distribution: `corecli view "<file>" screenshot --grid 3 -o grid.png`.
