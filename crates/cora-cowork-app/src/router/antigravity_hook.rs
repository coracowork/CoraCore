...
//!
//! agy cannot prompt for tool permission in headless mode, so CoraCowork registers
...
use cora_cowork_ai_agent::antigravity_hook::HookTokenRegistry;
...
async fn handle_hook(
    ...
    if !state.tokens.verify(&conversation_id, presented) {
        ...
        return Json(AntigravityHookOutput::deny(
            "coracowork rejected an unauthenticated hook callback",
        ));
    }
    ...
    let Some(AgentInstance::Session(task)) = state.task_manager.get_task(&conversation_id) else {
        ...
        return Json(AntigravityHookOutput::deny(
            "coracowork has no live session for this conversation",
        ));
    };
    ...
    tracing::info!(
        ...
        approved = matches!(
            decision,
            cora_cowork_session::PermissionDecision::Approved | cora_cowork_session::PermissionDecision::AllowAlways
        ),
    ...
    Json(match decision {
        cora_cowork_session::PermissionDecision::Approved | cora_cowork_session::PermissionDecision::AllowAlways => {
            AntigravityHookOutput::allow("approved in CoraCowork")
        }
        cora_cowork_session::PermissionDecision::Denied => AntigravityHookOutput::deny("rejected in CoraCowork"),
    })
}
...
mod tests {
    use super::*;
    use cora_cowork_ai_agent::WorkerTaskManagerImpl;
    use cora_cowork_ai_agent::task_manager::AgentFactory;
    use cora_cowork_api_types::{AntigravityHookDecision, AntigravityHookToolCall};
