...
//!
//! Adapts the transport-agnostic [`FsMonitorActor`] (in `cora-cowork-project`) to the
...
use cora_cowork_project::monitor::{FsInbound, FsMonitorActor, FsWirePush};
use cora_cowork_realtime::{ConnectionId, MessageRouter, NoopMessageRouter, WebSocketManager};
...
#[cfg(test)]
mod tests {
    use cora_cowork_realtime::{PER_CONNECTION_BUFFER, WsOutbound};
    use serde_json::json;
    use tokio::sync::mpsc;
