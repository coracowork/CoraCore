...
use cora_cowork_ai_agent::{agent_routes, remote_agent_routes};
use cora_cowork_api_types::ErrorResponse;
use cora_cowork_assets::{AssetRouterState, asset_routes};
use cora_cowork_assistant::assistant_routes;
use cora_cowork_auth::{
    AuthRouterState, AuthState, auth_middleware, auth_routes, csrf_middleware, security_headers_middleware,
};
use cora_cowork_channel::channel_routes;
#[cfg(feature = "weixin")]
use cora_cowork_channel::weixin_login_route;
use cora_cowork_common::ApiErrorLogContext;
use cora_cowork_conversation::{conversation_ops_routes, conversation_routes};
use cora_cowork_cron::cron_routes;
use cora_cowork_extension::{extension_routes, hub_routes, skill_routes};
use cora_cowork_file::file_routes;
use cora_cowork_mcp::mcp_routes;
use cora_cowork_office::{office_proxy_routes, office_routes};
use cora_cowork_realtime::{WsHandlerState, ws_upgrade_handler};
use cora_cowork_shell::shell_routes;
use cora_cowork_system::{connection_test_routes, system_routes};
use cora_cowork_team::{TeamSessionService, team_routes};
...
pub fn create_router_with_all_state(services: &AppServices, states: ModuleStates, ws_state: WsHandlerState) -> Router {
    ...
    let router = router.layer(DefaultBodyLimit::max(cora_cowork_common::constants::BODY_LIMIT));
