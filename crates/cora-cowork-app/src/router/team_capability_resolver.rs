...
use cora_cowork_team::{TeamError, TeamToolCapabilityPort};
...
impl TeamToolCapabilityPort for TeamCapabilityResolver {
    async fn resolve(
        ...
        if let Some(descriptor) = cora_cowork_session::backend_capability_descriptor(backend) {
...
mod tests {
    ...
    async fn direct_matrix(snapshot: Option<&str>) -> Vec<ResolvedBackendCapabilities> {
        ...
        for descriptor in cora_cowork_session::backend_capability_descriptors()
            .iter()
