...
use cora_cowork_auth::{CookieConfig, JwtService, QrTokenStore, resolve_jwt_secret};
use cora_cowork_common::OnConversationDelete;
use cora_cowork_conversation::{ConversationService, runtime_state::ConversationRuntimeStateService};
use cora_cowork_db::{
...
use cora_cowork_realtime::{BroadcastEventBus, WebSocketManager};
...
pub struct AppServices {
    ...
    pub skill_paths: Arc<cora_cowork_extension::SkillPaths>,
...
impl AppServices {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        ...
        let skill_paths = Arc::new(cora_cowork_extension::resolve_skill_paths(&app_resource_dir, &data_dir));
        cora_cowork_extension::sync_skill_catalog_into_repo(skill_paths.as_ref(), skill_repo.as_ref())
            .await
        ...
        let backend_binary_path =
            Arc::new(std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("coracore")));
...
struct ConversationServiceDeps<'a> {
    ...
    skill_paths: Arc<cora_cowork_extension::SkillPaths>,
