//! `coracore mcp-team-stdio` subcommand: MCP stdio server for team tools.
//!
...
mod tests {
    ...
    fn team_stdio_descriptions_match_prompt_registry() {
        ...
        let mut expected_names: Vec<_> = cora_cowork_team_prompts::tools::team_tool_specs()
            .iter()
            .map(|spec| spec.name)
            .collect();
        ...
        for spec in cora_cowork_team_prompts::tools::team_tool_specs() {
            let tool = tools
                .iter()
