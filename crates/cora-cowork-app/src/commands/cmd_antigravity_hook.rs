//! `coracore antigravity-hook`: agy's PreToolUse gate.
//!
...
//! CoraCowork backend, which raises a permission card and blocks until the user
//! answers.
//!
...
use cora_cowork_api_types::{AntigravityHookConfig, AntigravityHookInput, AntigravityHookOutput};
...
pub(crate) async fn run_antigravity_hook() -> ExitCode {
    ...
    let body = serde_json::to_string(&decision).unwrap_or_else(|_| {
        ...
        r#"{"decision":"deny","reason":"coracowork hook could not serialize its answer"}"#.to_owned()
    });
...
async fn decide() -> AntigravityHookOutput {
    ...
    if let Err(e) = tokio::io::stdin().read_to_string(&mut raw).await {
        return AntigravityHookOutput::deny(format!("coracowork hook could not read the request: {e}"));
    }
    let input: AntigravityHookInput = match serde_json::from_str(&raw) {
        Ok(v) => v,
        Err(e) => return AntigravityHookOutput::deny(format!("coracowork hook could not parse the request: {e}")),
    };

    let base_url = match std::env::var(AntigravityHookConfig::ENV_BASE_URL) {
        Ok(v) if !v.trim().is_empty() => v,
        _ => {
            return AntigravityHookOutput::deny("coracowork hook is not configured (missing callback address)");
        }
    };
    ...
    let client = match reqwest::Client::builder().timeout(CALLBACK_TIMEOUT).build() {
        Ok(c) => c,
        Err(e) => return AntigravityHookOutput::deny(format!("coracowork hook could not build its client: {e}")),
    };
        ...
        .await
    {
        ...
        // The user closed CoraCowork, the turn was cancelled, or nobody answered in
        // time. Denying is the only safe reading of "no answer".
        Err(e) => AntigravityHookOutput::deny(format!("coracowork did not answer: {e}")),
    }
}
...
/// `coracowork returned an unreadable decision: error decoding response body`,
...
fn decision_from_response(status: reqwest::StatusCode, body: &str) -> AntigravityHookOutput {
    if !status.is_success() {
        ...
        return match code {
            Some(code) => AntigravityHookOutput::deny(format!("coracowork refused the request: HTTP {status} ({code})")),
            None => AntigravityHookOutput::deny(format!("coracowork refused the request: HTTP {status}")),
        };
    }
    match serde_json::from_str::<AntigravityHookOutput>(body) {
        Ok(out) => out,
        Err(e) => AntigravityHookOutput::deny(format!("coracowork returned an unreadable decision: {e}")),
    }
}
...
mod tests {
    use super::*;
    use cora_cowork_api_types::AntigravityHookDecision;

    #[tokio::test]
    async fn unconfigured_bridge_denies_instead_of_allowing() {
        // Safety property: a hook that cannot reach CoraCowork must never let the
        // tool through.
        unsafe {
    ...
    /// saw `coracowork returned an unreadable decision: error decoding response
