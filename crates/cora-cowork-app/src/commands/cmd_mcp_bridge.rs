//! `coracore mcp-bridge` subcommand: stdio ↔ TCP bridge for the team MCP server.
//!
//! Spawned by the ACP agent CLI as an MCP server with command `coracore mcp-bridge`.
...
//! `127.0.0.1:<TEAM_MCP_PORT>` (reusing `cora_cowork_team::mcp::protocol`).
//!
...
/// Entry point for `coracore mcp-bridge`. Returns an [`ExitCode`] so the
