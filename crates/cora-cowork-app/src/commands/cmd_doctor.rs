...
//! `cora_cowork_runtime::init` + `enhance_process_path` for `Doctor` as it
//! does for the server, so managed runtimes and CLI commands resolve
//! through the same paths the server uses.
//!
//! Writes to stdout (not the rolling coracore.log) — the user
...
pub async fn run_doctor(cli: &Cli, merged_path: &str) -> Result<ExitCode, CliBoundaryError> {
    ...
    let db_path = cli.data_dir.join("cora-cowork-backend.db");
...
fn print_environment(merged_path: &str, data_dir: &Path) {
    ...
    println!("Cora-Cowork backend doctor — agent CLI detection self-check");
...
fn print_snapshot(snapshot: &[(cora_cowork_api_types::AgentMetadata, Option<UnavailableReason>)]) {
...
mod tests {
    ...
    fn doctor_database_error_uses_stable_code_without_raw_path() {
        ...
        assert!(!err.stderr_line().contains("/Users/secret/cora-cowork-backend.db"));
    }
