...
use cora_cowork_api_types::{RuntimeStatusScope, RuntimeStatusScopeKind};
use cora_cowork_app::{AppConfig, AppServices, RouterBuildError, create_router_with_runtime};
use cora_cowork_system::RuntimePrepareService;
use cora_cowork_team::TeamIdleCleanupCoordinator;

use crate::bootstrap::{BootstrapError, BootstrapErrorCode, ParentExitSignal, ServerEnvironment};

const LISTENING_EVENT_PREFIX: &str = "CORACORE_LISTENING";
...
pub(crate) async fn run_server(
    ...
    tokio::spawn(async move {
        ...
        let result = async {
            ...
            runtime_prepare_service
                ...
                .await?;
            Ok::<(), cora_cowork_system::SystemError>(())
        }
        .await;
    ...
    // 10 min / 30 min / 60 s and are overridable via CORA_COWORK_IDLE_TIMEOUT_SECS,
    // CORA_COWORK_TEAM_IDLE_TIMEOUT_SECS, and CORA_COWORK_IDLE_SCAN_INTERVAL_SECS. The
    ...
    let idle_cleanup_coordinator: Arc<dyn cora_cowork_ai_agent::IdleCleanupCoordinator> =
...
mod tests {
    ...
    fn listening_event_line_is_machine_readable() {
        ...
        let payload = line
            .strip_prefix("CORACORE_LISTENING ")
