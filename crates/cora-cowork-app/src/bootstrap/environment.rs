...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment, BootstrapError> {
    ...
    unsafe {
        std::env::set_var("CORA_COWORK_WORK_DIR", &work_dir);
    }
...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database, BootstrapError> {
    ...
    cora_cowork_db::maybe_copy_legacy_database(&db_path).map_err(|e| {
        BootstrapError::new(
            BootstrapErrorCode::DataInitFailed,
            "data.legacy_db",
            "failed to initialize application data",
    ...
    let database = cora_cowork_db::init_database_staged_with_options(
        &db_path,
        cora_cowork_db::DatabaseInitOptions {
...
mod tests {
    ...
    fn database_stage_comes_from_db_boundary_error() {
        let err = cora_cowork_db::DatabaseInitError::new(
            "database.migration",
            cora_cowork_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMismatch(42)),
        );
    ...
    fn database_schema_repair_stage_comes_from_db_boundary_error() {
        let err = cora_cowork_db::DatabaseInitError::new(
            ...
            cora_cowork_db::DbError::Init("repair failed".into()),
        );
    ...
    fn database_recoverable_corruption_stage_comes_from_db_boundary_error() {
        let err = cora_cowork_db::DatabaseInitError::new(
            ...
            cora_cowork_db::DbError::Migration(sqlx::migrate::MigrateError::ExecuteMigration(
