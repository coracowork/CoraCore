...
use cora_cowork_office::OfficeError;
use cora_cowork_office::proxy::{ProxyError, ProxyService};
use cora_cowork_office::types::DocType;
use cora_cowork_office::watch_manager::{CorecliWatchManager, ProcessHandle, ProcessSpawner};
...
async fn setup_proxy(doc_type: DocType, response_template: &str) -> (ProxyService, u16, tempfile::TempDir) {
    ...
    let mgr = Arc::new(CorecliWatchManager::new(Arc::new(spawner), Arc::new(NoopBroadcaster)));
...
async fn setup_ssrf_proxy(doc_type: DocType) -> (ProxyService, u16, tempfile::TempDir) {
    let mgr = Arc::new(CorecliWatchManager::new(
