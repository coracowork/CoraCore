...
async fn wp2_session_reuse_returns_same_port() {
    ...
    let mgr = CorecliWatchManager::new(spawner.clone(), broadcaster);
...
async fn wp3_stop_terminates_session() {
    let spawner = Arc::new(TestSpawner::new(true));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let mgr = CorecliWatchManager::new(spawner, broadcaster);
...
// ---------------------------------------------------------------------------
// WP-4: Auto-install when corecli not found
// ---------------------------------------------------------------------------
...
async fn wp4_auto_install_on_not_found() {
    ...
    let broadcaster = Arc::new(TestBroadcaster::new());
    let mgr = CorecliWatchManager::new(spawner.clone(), broadcaster.clone());
...
async fn status_events_use_correct_prefix() {
    let spawner = Arc::new(TestSpawner::new(true));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let mgr = CorecliWatchManager::new(spawner, broadcaster.clone());
