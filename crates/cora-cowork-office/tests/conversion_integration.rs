use cora_cowork_api_types::ConversionTarget;
use cora_cowork_office::ConversionService;
...
// DC-8: PPT → JSON (corecli not available)
#[tokio::test]
...
// DC-8b: PPT → JSON (corecli not installed — configured path invalid and not in PATH)
#[tokio::test]
async fn dc8b_ppt_to_json_corecli_not_installed() {
    let dir = TempDir::new().unwrap();
    let path = dir.path().join("slides.pptx");
    std::fs::write(&path, b"fake pptx content").unwrap();

    // Use a wrapper that ensures corecli is not found
    let svc = ConversionService::new(Some("/nonexistent/corecli".into()));
    let resp = svc
        .convert(path.to_str().unwrap(), ConversionTarget::PptJson)
        .await
        .unwrap();

    // Conversion should fail — either because corecli is not found or because
