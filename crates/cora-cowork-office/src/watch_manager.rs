...
use cora_cowork_api_types::{PreviewState, PreviewStatusEvent, WebSocketMessage};
...
pub trait ProcessSpawner: Send + Sync {
    ...
    async fn install_corecli(&self) -> Result<(), OfficeError>;

    async fn is_corecli_installed(&self) -> bool;
...
pub struct CorecliWatchManager {
...
impl CorecliWatchManager {
    ...
    pub async fn start(&self, file_path: &str, doc_type: DocType) -> Result<u16, OfficeError> {
        ...
        match &result {
            ...
            Err(e) => {
                ...
                Err(match e {
                    OfficeError::CorecliNotFound => OfficeError::CorecliNotFound,
    ...
    async fn try_start(&self, resolved: &str, doc_type: DocType) -> Result<u16, OfficeError> {
        for attempt in 1..=START_PORT_MAX_ATTEMPTS {
            ...
            match self.spawn_corecli_with_install(resolved, port, doc_type).await {
                Ok(process) => {
    ...
    async fn spawn_corecli_with_install(
        &self,
        resolved: &str,
        port: u16,
        doc_type: DocType,
    ) -> Result<Box<dyn ProcessHandle>, OfficeError> {
        match self.spawner.spawn_corecli(resolved, port, doc_type).await {
            Ok(process) => Ok(process),
            Err(OfficeError::CorecliNotFound) => {
                self.broadcast_status(doc_type, PreviewState::Installing, None);
                self.spawner.install_corecli().await?;
                self.spawner.spawn_corecli(resolved, port, doc_type).await
            }
            Err(error) => Err(error),
        }
    }
            ...
            *last = Some(std::time::Instant::now());
            ...
            tokio::spawn(async move {
                if let Err(e) = spawner.check_update(doc_type).await {
                    tracing::warn!("corecli version check failed: {e}");
                }
            });
        }
    }
...
impl Drop for CorecliWatchManager {
    fn drop(&mut self) {
        for entry in self.sessions.iter() {
            entry.value().process.kill();
        }
...
impl ProcessSpawner for DefaultProcessSpawner {
    async fn spawn_corecli(
        ...
        let corecli = resolve_corecli_path().ok_or(OfficeError::CorecliNotFound)?;
        if !corecli_supports_watch(&corecli).await {
            return Err(OfficeError::CorecliNotFound);
        }

        let mut builder = CmdBuilder::new(&corecli);
        builder
            .arg("watch")
            .arg(file_path)
            .arg("--port")
    ...
    async fn install_corecli(&self) -> Result<(), OfficeError> {
        ...
        tracing::info!(
            ...
            "installing corecli via official coracowork/CoraCLI installer"
        );
        ...
        if !output.status.success() {
            tracing::warn!(
                ...
                "corecli official installer failed"
            );
            return Err(OfficeError::InstallFailed("official CoreCLI installer failed".into()));
        }
        Ok(())
    }

    async fn is_corecli_installed(&self) -> bool {
        let Some(corecli) = resolve_corecli_path() else {
            return false;
        };

        corecli_supports_watch(&corecli).await
    }
    ...
    async fn check_update(&self, _doc_type: DocType) -> Result<(), OfficeError> {
        ...
        let mut builder = CmdBuilder::clean_cli(&corecli);
        builder.arg("--version");
        let output = builder
            .output()
            .await
        ...
        let local_version = normalize_corecli_version(&String::from_utf8_lossy(&output.stdout));
        let response = reqwest::Client::new()
            .get(CORECLI_LATEST_RELEASE_URL)
            .send()
            .await
            .map_err(|e| OfficeError::StartFailed(e.to_string()))?;
        ...
        if !remote_version.is_empty() && remote_version != local_version {
            tracing::info!(
                ...
                "corecli update available, installing via official installer"
            );
            self.install_corecli().await?;
        }
...
fn normalize_corecli_version(raw: &str) -> String {
...
async fn corecli_supports_watch(corecli: &Path) -> bool {
    let mut version = CmdBuilder::clean_cli(corecli);
    version.arg("--version");
    if !version.output().await.is_ok_and(|o| o.status.success()) {
        return false;
    }

    let mut watch_help = CmdBuilder::clean_cli(corecli);
    watch_help.args(["watch", "--help"]);
    let ok = watch_help.output().await.is_ok_and(|o| o.status.success());
    if !ok {
        tracing::warn!("corecli exists but does not expose watch command");
    }
    ok
}
...
fn public_preview_error_message(error: &OfficeError) -> String {
    match error {
        OfficeError::InstallFailed(_) => "corecli install failed".to_owned(),
        OfficeError::CorecliNotFound => "corecli not found".to_owned(),
        _ => error.to_string(),
    }
}
...
mod tests {
    ...
    fn make_manager(spawner: Arc<MockSpawner>, broadcaster: Arc<RecordingBroadcaster>) -> CorecliWatchManager {
        CorecliWatchManager::new(spawner, broadcaster)
    }
    ...
    fn install_failed_public_preview_message_is_sanitized() {
        ...
        assert_eq!(public_preview_error_message(&err), "corecli install failed");
    }
    ...
    fn port_in_use_start_failure_ignores_other_errors() {
        ...
        assert!(!is_port_in_use_start_failure(&OfficeError::CorecliNotFound));
    }

    #[cfg(unix)]
    #[tokio::test]
    async fn corecli_capability_probe_requires_watch_command() {
        let tmp = tempfile::tempdir().unwrap();
        let corecli = tmp.path().join("corecli");
        std::fs::write(
            &corecli,
        ...
        let mut perms = std::fs::metadata(&corecli).unwrap().permissions();
        perms.set_mode(0o755);
        std::fs::set_permissions(&corecli, perms).unwrap();

        assert!(!corecli_supports_watch(&corecli).await);
    }
