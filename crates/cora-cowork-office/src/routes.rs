...
use cora_cowork_file::{FileError, path_safety::validate_path_with_extra_root};
...
impl From<OfficeError> for ApiError {
    fn from(err: OfficeError) -> Self {
        match err {
            OfficeError::CorecliNotFound => ApiError::BadRequest("corecli not found".into()),
            OfficeError::InstallFailed(_) => ApiError::Internal("corecli install failed".into()),
...
) -> Result<Json<ApiResponse<cora_cowork_api_types::DocumentConversionResponse>>, ApiError> {
    let Json(req) = body.map_err(ApiError::from)?;
    let validated_path = validate_office_path(&state, &req.file_path, req.workspace.as_deref())?;
    let resp = state
        .conversion_service
...
fn preview_error_code(error: &OfficeError) -> &'static str {
    match error {
        OfficeError::CorecliNotFound => "CORECLI_NOT_FOUND",
        OfficeError::InstallFailed(_) => "CORECLI_INSTALL_FAILED",
        OfficeError::PortTimeout(_) => "CORECLI_PORT_TIMEOUT",
        OfficeError::StartFailed(_)
        | OfficeError::Io(_)
        | OfficeError::Snapshot(_)
        | OfficeError::Json(_)
        | OfficeError::Conversion(_)
        | OfficeError::ToolNotFound(_) => "CORECLI_START_FAILED",
    }
}
...
mod tests {
    ...
    use cora_cowork_file::FileError;
    ...
    use crate::state::OfficeRouterState;
    use crate::types::DocType;
    use crate::watch_manager::{CorecliWatchManager, ProcessHandle, ProcessSpawner};
    ...
    #[test]
    fn corecli_not_found_maps_to_bad_request() {
        let err = ApiError::from(OfficeError::CorecliNotFound);
    ...
    fn install_failed_maps_to_internal() {
        ...
        assert!(matches!(err, ApiError::Internal(msg) if msg == "corecli install failed"));
    }
    ...
    fn build_test_state() -> OfficeRouterState {
        ...
        impl cora_cowork_realtime::EventBroadcaster for NoopBroadcaster {
            fn broadcast(&self, _msg: cora_cowork_api_types::WebSocketMessage<serde_json::Value>) {}
        }

        let spawner = Arc::new(NoopSpawner);
        let bc: Arc<dyn cora_cowork_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
        let wm = Arc::new(CorecliWatchManager::new(spawner, bc));
