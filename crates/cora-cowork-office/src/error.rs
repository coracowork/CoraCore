...
pub enum OfficeError {
    #[error("corecli not found")]
    CorecliNotFound,

    #[error("corecli install failed: {0}")]
...
mod tests {
    ...
    #[test]
    fn display_messages() {
        assert_eq!(OfficeError::CorecliNotFound.to_string(), "corecli not found");
        assert_eq!(
            OfficeError::InstallFailed("installer error".into()).to_string(),
            "corecli install failed: installer error"
        );
        assert_eq!(
