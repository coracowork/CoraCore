...
impl ConversionService {
    pub fn new(corecli_path: Option<String>) -> Self {
        Self { corecli_path }
    }

    pub fn with_data_dir(corecli_path: Option<String>, _data_dir: PathBuf) -> Self {
        Self { corecli_path }
    }
    ...
    async fn ppt_to_json(&self, file_path: &str) -> Result<Value, OfficeError> {
        ...
        let corecli = resolve_corecli(&self.corecli_path).await?;
        ...
        let output = builder
            .output()
            .await
            .map_err(|e| OfficeError::Conversion(format!("failed to run corecli ppt2json: {e}")))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(OfficeError::Conversion(format!("corecli ppt2json failed: {stderr}")));
        }

        let json: Value = serde_json::from_slice(&output.stdout)
            .map_err(|e| OfficeError::Conversion(format!("failed to parse corecli ppt2json output: {e}")))?;
...
async fn resolve_corecli(configured_path: &Option<String>) -> Result<String, OfficeError> {
    ...
    if let Some(path) = resolve_corecli_path() {
        return Ok(path.to_string_lossy().into_owned());
    }

    Err(OfficeError::ToolNotFound(
        "corecli not installed. Install coracowork/CoraCLI to enable PPT -> JSON conversion".into(),
    ))
}
...
mod tests {
    ...
    fn conversion_service_new() {
        ...
        assert!(svc.corecli_path.is_none());

        let svc = ConversionService::new(Some("/usr/local/bin/corecli".into()));
        assert_eq!(svc.corecli_path.as_deref(), Some("/usr/local/bin/corecli"));

        let svc = ConversionService::with_data_dir(None, PathBuf::from("/tmp/cora-cowork"));
        assert!(svc.corecli_path.is_none());
    }

    #[tokio::test]
    async fn resolve_corecli_without_config_ignores_legacy_managed_prefix() {
        ...
        if let Ok(resolved) = resolve_corecli(&None).await {
