...
pub enum ModeSwitchEffect {
    ...
    ///
    /// The default: a backend that reconfigures synchronously (corars mutates the shared
...
/// the agent; antigravity, corars, and every ACP agent are one-prompt-at-a-time.
...
mod tests {
    ...
    fn static_backend_table_matches_capability_constructors() {
        ...
        assert!(!backend_supports_midturn_delivery("corars"));
    }
}
