...
//! the upper `cora-cowork-ai-agent` Domain crate; `cora-cowork-session` must stay
...
impl AcpConnection {
    pub fn new(spawner: Arc<dyn Spawner>, command: cora_cowork_common::CommandSpec) -> Self {
...
pub fn acp_capabilities() -> Capabilities {
    Capabilities {
        ...
        emits: SignalSet {
            ...
            // prompt response (no idle-timeout in CoraCore anyway, post-007).
            heartbeat: false,
            tool_lifecycle: true,
...
impl AcpSessionBackend {
    ...
    async fn bound_session(&self) -> Result<String, BackendError> {
        ...
        // (cora-cowork-agent-rest INIT_TIMEOUT_SECS). A cold start / untrusted project slows
...
struct AcpWakeRecipe {
    ...
    command: Option<cora_cowork_common::CommandSpec>,
    config: SessionConfig,
}
...
async fn handle_open_response(
    ...
    // protocols/design/coracore-opencode-acp-configoptions-model-mode-prompt.md.
    let mut disc = discovered.lock().unwrap_or_else(|e| e.into_inner());
...
async fn handle_reverse_rpc(
    ...
    match method {
        "session/request_permission" => {
            ...
            // card can show the approver WHAT they are approving (CoraCowork issue
...
mod tests {
    ...
    fn parse_permission_metadata_extracts_claude_title_prefix_server() {
        ...
        let params = json!({
            ...
            "toolCall": { "title": "mcp__cora-cowork-team__team_members", "rawInput": {} },
        });
        let meta = parse_permission_metadata(Some(&params)).expect("metadata present");
        assert_eq!(meta["server_name"], "cora-cowork-team");
    ...
    async fn acp_answer_permission_echoes_real_offered_option_id_by_kind() {
        ...
        // CoraCowork issue #3779: the card must show what is being approved — the
    ...
    async fn f4_suspend_then_wake_respawns_through_spawner() {
        ...
        let command = cora_cowork_common::CommandSpec {
            command: "hermes".into(),
            args: vec!["acp".into()],
            env: Vec::new(),
            cwd: None,
