...
use cora_cowork_process::Spawner;
...
fn log_codex_runtime_policy(spawn_env: &[cora_cowork_common::EnvVar]) {
    let mut runtime_env_keys = spawn_env
        .iter()
        .filter_map(|entry| entry.name.starts_with("CORA_COWORK_").then_some(entry.name.as_str()))
        .collect::<Vec<_>>();
...
impl BackendConnection for CodexConnection {
    async fn open_session(
        ...
        let cmd = cora_cowork_common::CommandSpec {
            ...
            // spawn env (per-agent overrides + CORA_COWORK_* conversation runtime
        ...
        // Report a codex whose version differs from the release CoraCowork verified.
        ...
        // cora-probe transcripts in protocols/verification fixtures):
...
fn initialize_params() -> HandshakeParams {
    HandshakeParams(json!({
        "clientInfo": { "name": "cora-cowork-session", "version": "0.1.0" },
...
pub fn codex_capabilities() -> Capabilities {
    Capabilities {
        ...
        // coraCore sends `turn/start` with `{threadId, input}` alone. Not a defect — this
...
pub struct CodexSessionBackend {
    ...
    stdin: Arc<Mutex<Option<cora_cowork_process::BoxedStdin>>>,
...
fn start_codex_reader(
    ...
    stdout: Option<cora_cowork_process::BoxedStdout>,
    io: Arc<dyn AgentIo>,
            ...
            Ok(Some(line)) => {
                ...
                match (method, has_id) {
                    ...
                    (Some(m), false) => {
                        ...
                        // terminal). See protocols/design/coracore-codex-turn-no-terminal-hang-prompt.md.
                        if m == "error"
...
///
/// codex's permission tiers are DISCOVERED, not a fixed CoraCowork enum — mirroring the
/// legacy ACP mechanism (`manager/acp/session.rs`: `availableModes[]` advertised by the
/// agent, `is_mode_valid` validates against that live list, CoraCore defines no values).
...
fn fill_discovery(kind: DiscoveryKind, result: &Value, discovered: &Arc<std::sync::Mutex<Discovered>>) {
    ...
    match kind {
        ...
        DiscoveryKind::Permissions => {
            ...
            // longer translate to a fixed CoraCowork enum or drop custom profiles: codex
            // defines the value set, CoraCore only transports it (parity with legacy ACP,
...
async fn handle_reverse_rpc(
    ...
    match method {
        ...
        "item/commandExecution/requestApproval" | "item/fileChange/requestApproval" => {
            ...
            emit(
                ...
                SessionEvent::Permission {
                    ...
                    // what they are approving (CoraCowork issue #3779 — approving blind).
        ...
        _ => {
            write_reverse_error(stdin, &id, -32601, "method not handled by cora-cowork-session").await;
            emit(
                event_tx,
                session_id,
                cur,
...
fn map_notification(method: &str, params: &Value) -> Vec<SessionEvent> {
    match method {
        ...
        // collaboration selector in CoraCowork, and that field would clobber the permission mode.
        ...
        match command {
            Command::Send { content, metadata } => {
                ...
                if matches!(route, Some(SlashRoute::Logout)) {
                    self.suspend
                        .ensure_awake(cora_cowork_common::now_ms(), || self.wake_handle())
                        .await?;
                ...
                // (verified against the cora-probe transcripts). Needs the bound
...
mod tests {
    ...
    async fn approval_reverse_rpc_surfaces_as_permission() {
        ...
        // approver what they are approving (CoraCowork issue #3779).
        for (m, title) in [
    ...
    fn command_execution_uses_codex_semantic_action_as_its_label() {
        ...
        let list = serde_json::json!({
            "type": "commandExecution",
            "commandActions": [{ "type": "listFiles", "path": "crates/cora-cowork-session" }]
        });
        assert_eq!(
            command_execution_display_name(&list),
            "List files crates/cora-cowork-session"
        );
    }
    ...
    async fn dispatch_send_writes_turn_start_with_thread_id() {
        ...
        // the cora-probe transcripts — NOT the fictional `sendUserTurn`). Requires
    ...
    async fn dispatch_set_mode_normalizes_legacy_persisted_value() {
        // A legacy persisted BARE value (`yolo`) — from an older CoraCowork that stored the
    ...
    async fn dispatch_set_mode_passes_custom_profile_id_verbatim() {
        ...
        // heart of the legacy-ACP parity: codex owns the value set, CoraCore only transports
    ...
    async fn dispatch_set_mode_obsolete_and_unknown_bare_values_fall_to_workspace() {
        // Legacy-ACP parity: CoraCore never ships a frame codex would reject. Any unknown
    ...
    fn b1_initialize_frame_opts_into_experimental_api_nested() {
        ...
        assert_eq!(frame["params"]["clientInfo"]["name"], "cora-cowork-session");
    }
    ...
    async fn r4_open_session_routes_through_injected_spawner_with_codex_app_server() {
            ...
            .open_session(
                ...
                SessionConfig {
                    ...
                    spawn_env: vec![cora_cowork_common::EnvVar {
                        name: "CORA_COWORK_CONVERSATION_ID".into(),
                        value: "conv-1".into(),
                    }],
                    ..Default::default()
                },
        ...
        // (CORA_COWORK_* runtime context + per-agent overrides) reaches the process.
        assert_eq!(
            spec.env
                .iter()
                .map(|e| (e.name.as_str(), e.value.as_str()))
                .collect::<Vec<_>>(),
            [("CORA_COWORK_CONVERSATION_ID", "conv-1")],
    ...
    async fn open_session_classifies_workspace_unavailable_spawn_failure() {
        ...
        #[async_trait::async_trait]
        impl cora_cowork_process::Spawner for WorkspaceGoneSpawner {
            async fn spawn(
                &self,
    ...
    /// ACP agent's `availableModes[]`), CoraCore does not translate or whitelist.
    #[tokio::test]
    ...
    /// UT-1b: `normalize_to_profile_id` — the ONE translation CoraCore still owns —
    ...
    fn codex_perm_normalize_to_profile_id_maps_legacy_and_passes_colon_ids() {
        ...
        // verbatim — codex, not CoraCore, owns the value set (legacy-ACP parity).
        for id in [
            ":workspace",
            ":danger-full-access",
            ":read-only",
    ...
    async fn r8_send_during_never_arriving_handshake_times_out_and_errors() {
        ...
        let saved = std::env::var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS").ok();
        unsafe { std::env::set_var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS", "1") };
        let fake = FakeAgentIo::never_exits(Vec::new());
        ...
        let res = backend
            ...
            .await;
        match saved {
            Some(v) => unsafe { std::env::set_var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS", v) },
            None => unsafe { std::env::remove_var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS") },
        }
    ...
    async fn f4_suspend_then_wake_respawns_through_spawner() {
        ...
        assert!(
            backend
                .suspend
                .suspend_if_idle(cora_cowork_common::now_ms() + 10_000, false)
                .await
        );
    ...
    /// legacy 30s (cora-cowork-agent-rest INIT_TIMEOUT_SECS), NOT the magic 2s the rewrite
    ...
    fn handshake_budget_is_legacy_30s_default_env_overridable() {
        ...
        let saved = std::env::var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS").ok();
        unsafe { std::env::remove_var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS") };
        assert!(
        ...
        unsafe { std::env::set_var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS", "45") };
        assert_eq!(
