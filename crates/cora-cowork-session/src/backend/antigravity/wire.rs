...
//! `~/cora/protocols/samples/antigravity-cli/1.1.8/`.
//!
...
mod tests {
    ...
    fn parses_mcp_tool_call_parameters() {
        ...
        let line = r#"{"event":"step_update","step_update":{"step_index":6,"state":"ACTIVE","step_type":"tool","tool_name":"call_mcp_tool","tool_info":{"name":"call_mcp_tool","parameters":{"ServerName":"cora-cowork-team","ToolName":"cora_cowork_team_ping","Arguments":{}}}}}"#;
        let AgyEvent::StepUpdate(su) = parse_line(line).expect("parsed") else {
            panic!("expected step_update");
        };
        let params = su.tool_info.expect("tool_info").parameters.expect("parameters");
        assert_eq!(params["ServerName"], "cora-cowork-team");
        assert_eq!(params["ToolName"], "cora_cowork_team_ping");
    }
