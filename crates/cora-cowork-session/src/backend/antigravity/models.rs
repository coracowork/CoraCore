...
async fn read_to_end(stdout: cora_cowork_process::BoxedStdout) -> String {
...
";
    ...
    async fn probing_closes_stdin_so_agy_can_exit() {
        ...
        let spawner: Arc<dyn Spawner> = Arc::new(cora_cowork_process::RealSpawner::new(
            Arc::new(cora_cowork_process::FileRegistryStore::new(tmp.path())),
            uuid::Uuid::now_v7(),
            "test-machine",
        ));
