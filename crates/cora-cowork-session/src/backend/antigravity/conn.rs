...
/// The literal placeholder CoraCowork's team-creation flow persisted as a member's
/// model when an assistant had no concrete model (CoraCowork
/// `teamCreateModelResolver.ts`, fixed in the paired CoraCowork change). agy never
...
pub fn antigravity_modes() -> Vec<ModeInfo> {
    // `yolo` is NOT one of agy's modes — it is CoraCowork's sentinel for "run
...
pub struct AntigravitySessionBackend {
    ...
    current: Arc<Mutex<Option<Arc<cora_cowork_process::ManagedProcess>>>>,
...
impl AntigravitySessionBackend {
    ...
    /// empty — this heals member sessions persisted before the CoraCowork fix.
    ...
    async fn start_turn(&self, content: Vec<ContentBlock>) -> Result<u64, BackendError> {
        ...
        if let Some(cwd) = self.config.cwd.as_deref() {
            ...
            spawn_env.push(cora_cowork_common::EnvVar {
                name: "PWD".to_owned(),
                value: cwd.to_owned(),
            });
        }
    ...
    fn spawn_reader(&self, proc: Arc<cora_cowork_process::ManagedProcess>, turn_gen: u64) {
...
mod tests {
    ...
    fn modes_are_agys_three_plus_the_full_auto_sentinel() {
        // agy's own axis is default / accept-edits / plan. `yolo` is CoraCowork's
    ...
    fn an_empty_model_list_still_drops_the_ui_placeholder() {
        // CoraCowork's team-creation flow used to persist the literal placeholder
