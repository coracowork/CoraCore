...
mod tests {
    ...
    fn mcp_calls_are_displayed_as_server_slash_tool() {
        ...
        let (_, evs) = tr(&[
            r#"{"event":"step_update","step_update":{"step_index":6,"state":"ACTIVE","step_type":"tool","tool_name":"call_mcp_tool","tool_info":{"name":"call_mcp_tool","parameters":{"ServerName":"cora-cowork-team","ToolName":"cora_cowork_team_ping","Arguments":{}}}}}"#,
        ]);
        match &evs[0] {
            SessionEvent::ToolCall { name, .. } => assert_eq!(name, "cora-cowork-team/cora_cowork_team_ping"),
            other => panic!("unexpected {other:?}"),
        }
    }
