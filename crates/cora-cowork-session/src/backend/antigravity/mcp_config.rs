...
//! servers (verified: `crates/cora-cowork-app/tests/live_ws_http_parity_e2e.rs`,
...
mod tests {
    ...
    fn stdio_spec_maps_to_agys_command_shape() {
        ...
        let servers = vec![McpServerSpec {
            name: "cora-cowork-team".into(),
            transport: McpTransport::Stdio {
                command: "/opt/coracowork/backend".into(),
        ...
        let s = &read_config(dir.path())["mcpServers"]["cora-cowork-team"];
        assert_eq!(s["command"], "/opt/coracowork/backend");
