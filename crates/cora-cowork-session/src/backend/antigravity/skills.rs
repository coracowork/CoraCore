...
//! does invoke a skill (verified), and CoraCowork provisions those skills itself
...
/// Where agy looks for workspace skills, and where CoraCowork provisions them.
...
mod tests {
    ...
    fn reads_name_and_description_from_frontmatter() {
        ...
        write_skill(
            dir.path(),
            "cora-cowork-probe",
            "---\nname: cora-cowork-probe\ndescription: Probe skill.\n---\n\n# body\n",
        );

        let cmds = scan_skill_commands(dir.path());
        assert_eq!(cmds.len(), 1);
        assert_eq!(cmds[0].name, "cora-cowork-probe");
