...
/// CoraCowork's sentinel mode id meaning "run without approval prompts".
///
...
/// CoraCowork's mode id meaning "ask before sensitive things" — its counterpart to
...
pub(crate) struct ArgvInput {
    ...
    /// An CoraCowork mode id, not necessarily an agy one. `accept-edits` / `plan` reach agy;
...
pub(crate) fn build_argv(input: &ArgvInput) -> Vec<String> {
    ...
    // "allow" cannot override that (hooks can tighten, not loosen). CoraCowork
    ...
    // agy takes exactly two values here: `accept-edits` and `plan`. CoraCowork's other two
...
mod tests {
    ...
    fn the_full_auto_sentinel_is_never_sent_as_agys_mode() {
        // `yolo` is CoraCowork's marker, not one of agy's modes. It is answered by NOT
    ...
    /// `default` is CoraCowork's "ask before sensitive things" mode, NOT an agy value.
    ///
    ...
    fn permission_gate_is_opened_for_the_hook_bridge() {
        ...
        // "allow" cannot override that. CoraCowork opens the gate here and makes its
