...
//!
//! None of claude / codex / agy ship with CoraCowork: each is whatever the user
...
//! install could differ, so the same prompt behaved differently in CoraCowork and
...
pub fn drift_notice(cli: &str, reported: &str, verified: &str) -> Option<(NoticeLevel, String, LocalizedText)> {
    ...
    match classify(reported, verified) {
        ...
        VersionVerdict::Older => Some((
            NoticeLevel::Info,
            format!(
                "The installed {cli} is older than the version CoraCowork verified; \
                 some features may be missing. Consider upgrading {cli}. \
                 (installed {reported} / verified by CoraCowork: {verified})"
            ),
        ...
        VersionVerdict::Newer => Some((
            NoticeLevel::Info,
            format!(
                "The installed {cli} is newer than the version CoraCowork verified. \
...
async fn read_first_line(stdout: cora_cowork_process::BoxedStdout) -> Option<String> {
