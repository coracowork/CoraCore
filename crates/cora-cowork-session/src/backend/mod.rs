...
/// (cora-cowork-agent-rest/src/protocol/acp.rs:55, an `oneshot` + `tokio::time::timeout`)
...
/// Restore the legacy 30s budget (env-overridable via `CORA_COWORK_HANDSHAKE_TIMEOUT_SECS`
...
pub(crate) fn handshake_budget() -> std::time::Duration {
    let secs = std::env::var("CORA_COWORK_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
pub trait SessionBackend: Send + Sync {
    ...
    /// headless mode, so CoraCowork registers itself as agy's `PreToolUse` hook;
...
/// → `session/new`|`session/load` `mcpServers[]`; codex → `thread/start`; corars →
/// its `McpServerConfig` map). Deliberately crate-local — `cora-cowork-session` stays
/// self-contained (no `cora-cowork-api-types`/ACP-SDK dep, §Cargo.toml). The app
...
pub struct SessionInit {
    ...
    /// Skill ids/names to surface to the agent on the first turn (acp/corars
    ...
    /// The field is carried for the corars/acp first-prompt delivery path only.
    pub skills: Vec<String>,
    /// Composed system prompt / preset context (the `compose_preset_context`
    /// output + corars `preset_rules`-merged prompt). Delivered first-message.
    pub preset_context: Option<String>,
    /// Opaque persisted session snapshot for resume (corars `SessionManager` JSON
    /// blob). Kept as `serde_json::Value` so `cora-cowork-session` need not depend on
    /// the `cora-agent` `Session` type; the corars backend deserializes it.
    pub session_snapshot: Option<serde_json::Value>,
    /// Explicit resume flag. acp/codex resume via `SessionSpec::Resume`; this is the
    /// corars equivalent + the single source of truth for "sanitize on resume".
    pub resume: bool,
}
...
pub struct SessionConfig {
    ...
    pub spawn_env: Vec<cora_cowork_common::EnvVar>,
    /// Antigravity only: the `.agents/hooks.json` body that registers CoraCowork as
    /// agy's approval gate.
    ///
    ...
    /// itself (it knows the workspace, not where CoraCowork's binary lives). Holding
