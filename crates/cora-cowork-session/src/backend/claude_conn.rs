...
/// Pure `serde_json`, no ACP SDK — `cora-cowork-session` stays SDK-free.
...
impl BackendConnection for ClaudeConnection {
    async fn open_session(
        ...
        // Report a claude whose version differs from the release CoraCowork
...
struct ClaudeWakeRecipe {
    ...
    env: Vec<cora_cowork_common::EnvVar>,
...
/// DIAGNOSTIC (env-gated, default OFF): when `CORA_COWORK_CLAUDE_WIRE_DUMP` is set, log
...
fn claude_wire_dump_enabled() -> bool {
    std::env::var("CORA_COWORK_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty())
}
...
fn dump_wire(direction: &str, session_id: &str, turn_gen: u64, bytes: &[u8]) {
    ...
    tracing::info!(
        target: "cora_cowork_session::claude_wire",
        direction,
                        ...
                        match tokio::time::timeout(std::time::Duration::from_millis(200), stdout.read(&mut chunk)).await {
                            ...
                            Ok(Ok(n)) if n > 0 => {
                                ...
                                match parsed {
                                    ...
                                    Err(_) => {
                                        tracing::error!(
                                            target: "cora_cowork_session::backend::claude_conn",
                                            conversation_id = %session_id,
...
mod tests {
    ...
    async fn f4_turn_in_flight_blocks_idle_suspend() {
        ...
        let suspended = backend
            .suspend
            .suspend_if_idle(cora_cowork_common::now_ms() + 10_000, true)
            .await;
