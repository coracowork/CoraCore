...
async fn run_exchange(io: &dyn AgentIo, req: &TitleRequest) -> Option<String> {
    ...
    write_frame(
        ...
        &json!({
            "id": 1,
            "method": "initialize",
            "params": { "clientInfo": { "name": "cora-cowork-title", "version": "1" } },
        }),
    )
    .await?;
...
async fn write_frame(stdin: &mut cora_cowork_process::BoxedStdin, frame: &Value) -> Option<()> {
...
impl TitleIoSource for SpawnedTitleIo {
    async fn open(&self) -> Option<Box<dyn AgentIo>> {
        match self
            .spawner
            .spawn(self.command.clone(), &[], "cora-cowork-session-title")
            .await
        {
...
mod tests {
    ...
    /// `~/cora/protocols/samples/codex-cli/0.144.6/ephemeral_title_thread_roundtrip.jsonl`
