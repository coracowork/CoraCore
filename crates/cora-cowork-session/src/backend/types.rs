...
pub enum Command {
    ...
    /// pure accept/decline approval (codex/corars) ignore it.
    ///
...
impl BackendError {
    ...
    /// before.
    pub(crate) fn from_spawn(context: &'static str, e: cora_cowork_process::ProcessError) -> Self {
        match e {
            cora_cowork_process::ProcessError::WorkspaceUnavailable(path) => Self::WorkspaceUnavailable(path),
...
/// How a session is (re)opened. `session_id` is the STABLE CoraCore-minted
...
mod tests {
    ...
    fn from_spawn_keeps_workspace_unavailable_class() {
        let err = BackendError::from_spawn(
            "codex spawn failed",
            cora_cowork_process::ProcessError::workspace_unavailable("/gone/workspace"),
        );
