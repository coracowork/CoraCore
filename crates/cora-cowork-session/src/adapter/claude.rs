...
/// leak), mirroring the ACP/corars per-kind id convention. `Other` has no
        ...
        for b in &blocks {
            match b.get("type").and_then(Value::as_str).unwrap_or("") {
                ...
                "tool_use" => {
                    ...
                    // #486 (parity with corars output_sink): DROP a malformed empty-name
    ...
    fn parse_control_request(&mut self, v: &Value) -> Vec<SessionEvent> {
        ...
        match (subtype, request_id.is_empty()) {
            ("can_use_tool", false) => {
                ...
                // approves blind (CoraCowork issue #3779). This is display-to-the-approver,
...
async fn write_ndjson_line(stdin: &mut BoxedStdin, value: &Value) -> Result<(), ProcessError> {
    ...
    bytes.push(b'\n');
    // DIAGNOSTIC (env-gated, default OFF via CORA_COWORK_CLAUDE_WIRE_DUMP): log the RAW
    ...
    if std::env::var("CORA_COWORK_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty()) {
...
mod tests {
    ...
    async fn captured_frame_with_id(content: &[crate::ContentBlock], client_msg_id: Option<&str>) -> String {
        ...
        let mut stdin: cora_cowork_process::BoxedStdin = Box::new(CaptureStdin(buf.clone()));
        ClaudeAdapter::new()
    ...
    /// persists as a ghost `tool_step{name:""}` row. Mirrors corars's empty-name
    ...
    fn tool_call_label_prefers_claudes_own_description() {
        ...
        assert_eq!(
            tool_call_display_name(
                "Bash",
                &serde_json::json!({"command": "cargo test -p cora-cowork-session", "description": "Re-run format tests"})
            ),
            "Re-run format tests"
        );
    ...
    fn long_paths_still_yield_the_file_name() {
        for (path, expected) in [
            (
                "/Users/z/Documents/github/Cora-Coworkworktrees/rtl/packages/desktop/src/renderer/components/Markdown/CodeBlock.tsx",
                "Read CodeBlock.tsx",
            ),
            (
                "/Users/z/Documents/github/Cora-Coworkworktrees/rtl/packages/desktop/src/renderer/services/i18n/index.ts",
                "Read index.ts",
            ),
        ] {
    ...
    fn unrecognized_and_mcp_tools_keep_their_own_name() {
        ...
        for name in [
            ...
            "mcp__cora-cowork-team__team_send_message",
            "AskUserQuestion",
            "TodoWrite",
        ] {
            assert_eq!(
    ...
    /// (CoraCowork issue #3779: with input dropped the card read "命令: Bash" and the
