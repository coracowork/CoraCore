...
pub enum SessionEvent {
    ...
    // (No clock/timeout lower: CoraCore does NOT auto-time-out a turn. A user may
    ...
    Detached {
        ...
        /// the 14-keyword allowlist in `cora_cowork_common::error_extract` survive,
    ...
    Permission {
        ...
        /// `{ "server_name": "cora-cowork-team"?, "options": [{"option_id","kind"}] }`.
        ...
        /// AskUserQuestion `{questions:[…]}` — CoraCowork issue #3779: without it the
    ...
    /// fields Option — only claude fills them; codex/ACP/corars never emit this.
    SubagentDetail {
        r#ref: String,
        parent_ref: Option<String>,
        label: Option<String>,
    ...
    /// projects it to a live tool-output pane. Claude/ACP/corars never emit it today.
    ...
    /// the runtime `turn_<shortid>` ids are cora-cowork-minted and mean nothing to
...
mod additive_tests {
    ...
    fn additive_defaults_keep_old_frames_deserializable() {
        ...
        let new_p = r#"{"Permission":{"request_id":"r","kind":"Tool","metadata":{"server_name":"cora-cowork-team"}}}"#;
