//! `cora-cowork-session` — server-authoritative SessionState control plane (feature
//! 002). Domain layer.
//!
//! ## What this crate is
//! It spawns the claude CLI directly (via 001 `cora-cowork-process`, no ACP/SDK),
...
//! `cora-cowork-conversation` + WebSocket — all Non-Goal here, untouched. This crate
...
//!
//! ## Layering
//! Domain layer, parallel to `cora-cowork-ai-agent`. Depends ONLY on
//! `cora-cowork-process` (001) + `cora-cowork-common`. NO production caller this
...
//!   acp/corars impls in `backend/`) — the ONLY backend-aware code: startup+prompt
