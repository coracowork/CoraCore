...
use cora_cowork_session::testing::CountingTerminateIo;
use cora_cowork_session::{AcpSessionBackend, ClaudeSessionBackend, CodexSessionBackend, SessionBackend};
...
/// covered in `cora-cowork-process`; it cannot be unit-constructed here without a
/// real OS process).
#[tokio::test]
async fn fake_agent_io_terminate_is_default_noop() {
    use cora_cowork_session::AgentIo;
    let io = cora_cowork_session::testing::FakeAgentIo::never_exits(Vec::new());
