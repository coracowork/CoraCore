...
//! still can, via the `CORA_COWORK_BUILTIN_ASSISTANTS_PATH` env var → disk
//! fallback path.
...
/// `crates/cora-cowork-app/assets/builtin-assistants/`.
static BUILTIN_ASSETS: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../cora-cowork-app/assets/builtin-assistants");
...
/// `CORA_COWORK_BUILTIN_ASSISTANTS_PATH` at a fixture directory.
enum Source {
    Embedded,
    Disk(PathBuf),
}
...
impl BuiltinAssistantRegistry {
    ...
    ///
    /// If `CORA_COWORK_BUILTIN_ASSISTANTS_PATH` is set and points to a readable
    /// directory, read from disk (test-only override). Otherwise use the
    /// assets embedded at compile time.
    pub fn load() -> Self {
        if let Ok(env) = std::env::var("CORA_COWORK_BUILTIN_ASSISTANTS_PATH") {
            let p = PathBuf::from(env);
            if p.exists() {
                return Self::load_from_dir(p);
            }
            warn!(
                "CORA_COWORK_BUILTIN_ASSISTANTS_PATH points to missing directory; \
    ...
    /// `CORA_COWORK_BUILTIN_ASSISTANTS_PATH` E2E fixtures — the three
...
mod tests {
    ...
    // CORA_COWORK_BUILTIN_ASSISTANTS_PATH). Graceful-degradation semantics must
    // stay intact.
    // -----------------------------------------------------------------------
    ...
    fn load_respects_env_var_disk_override() {
        ...
        let key = "CORA_COWORK_BUILTIN_ASSISTANTS_PATH";
