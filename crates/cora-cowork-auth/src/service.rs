...
use cora_cowork_db::{ExternalUserProjection, IUserRepository, UserStatus, UserType, models::User};
...
pub enum ProvisionError {
    ...
    Db(#[from] cora_cowork_db::DbError),
    #[error("token signing failed: {0}")]
    Token(#[from] crate::AuthError),
}

/// Port for the on-disk side of CoraCowork → CoraPro adoption. When DB adoption
...
/// in the composition layer over the skill filesystem so `cora-cowork-auth` stays
...
impl AuthProvisionService {
    ...
    pub async fn ensure_external_user(
        ...
        // Upgrade path (CoraCowork -> CoraPro over the same database): while this
...
fn map_external_user_type(user_type: ExternalUserType) -> Result<UserType, ProvisionError> {
    match user_type {
        ExternalUserType::Corapro => Ok(UserType::Corapro),
    }
}
