use cora_cowork_common::constants::{COOKIE_MAX_AGE_DAYS, COOKIE_NAME, CSRF_COOKIE_NAME};
...
impl CookieConfig {
    ...
    ///
    /// - `CORA_COWORK_HTTPS=true` → Secure flag, SameSite=Strict
    /// - Otherwise → no Secure flag, SameSite=Lax (for remote HTTP access)
    pub fn from_env() -> Self {
        let https = std::env::var("CORA_COWORK_HTTPS")
...
mod tests {
    ...
    fn session_cookie_http() {
        ...
        assert!(cookie.contains("cora-cowork-session=my_token"));
    ...
    fn clear_session_cookie_sets_max_age_zero() {
        ...
        assert!(cookie.contains("cora-cowork-session="));
    ...
    fn csrf_cookie_not_http_only() {
        ...
        assert!(cookie.contains("cora-cowork-csrf-token=csrf_abc"));
        assert!(!cookie.contains("HttpOnly"));
        assert!(cookie.contains("SameSite=Lax"));
        assert!(cookie.contains("Max-Age="));
    }
