...
pub fn dummy_password_hash() -> &'static str {
    DUMMY_HASH.get_or_init(|| {
        ...
        bcrypt::hash("__cora_cowork_dummy_password__", BCRYPT_COST).expect("bcrypt hash of constant input must succeed")
    })
}
...
/// of `uuid::Uuid::now_v7()` used in `cora-cowork-common`.
...
mod tests {
    ...
    fn dummy_hash_matches_dummy_input() {
        let hash = dummy_password_hash();
        assert!(verify_password("__cora_cowork_dummy_password__", hash).unwrap());
    }
