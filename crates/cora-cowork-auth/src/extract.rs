...
///
/// Priority: `Authorization: Bearer <token>` > `cora-cowork-session` cookie.
...
mod tests {
    ...
    fn token_from_cookie() {
        let headers = headers_with(&[("cookie", "cora-cowork-session=cookie_token; other=val")]);
    ...
    fn token_header_takes_priority_over_cookie() {
        let headers = headers_with(&[
            ...
            ("cookie", "cora-cowork-session=cookie_token"),
        ]);
    ...
    fn ws_token_from_cookie() {
        let headers = headers_with(&[("cookie", "cora-cowork-session=ws_cookie")]);
    ...
    fn token_from_cookie_with_malformed_entries() {
        ...
        let headers = headers_with(&[("cookie", "garbage; cora-cowork-session=abc; nope")]);
