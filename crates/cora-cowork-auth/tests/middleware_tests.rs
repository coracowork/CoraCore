...
use cora_cowork_db::{IUserRepository, SqliteUserRepository, init_database_memory};
    ...
    let resp = app
        .oneshot(
            Request::post("/api/test")
                .header("cookie", format!("cora-cowork-csrf-token={token}"))
...
async fn t12_2_post_with_mismatched_csrf_tokens_rejected() {
    ...
    let resp = app
        .oneshot(
            Request::post("/api/test")
                .header("cookie", "cora-cowork-csrf-token=token_a")
...
async fn t12_2_csrf_cookie_set_on_first_request() {
    ...
    assert!(set_cookie.contains("cora-cowork-csrf-token="));
...
fn t13_1_authorization_header_takes_priority() {
    ...
    headers.insert(header::COOKIE, "cora-cowork-session=cookie_tok".parse().unwrap());
    assert_eq!(
        cora_cowork_auth::extract_token_from_headers(&headers),
...
fn t13_2_cookie_fallback() {
    let mut headers = axum::http::HeaderMap::new();
    headers.insert(header::COOKIE, "cora-cowork-session=fallback_tok".parse().unwrap());
    assert_eq!(
        cora_cowork_auth::extract_token_from_headers(&headers),
...
fn t13_3_no_token_returns_none() {
    ...
    assert_eq!(cora_cowork_auth::extract_token_from_headers(&headers), None);
}
