...
///
/// Priority: `CORA_COWORK_BUN_PATH` env override > embedded + extract >
/// `which("bun")`.
...
///
/// This only reports an explicit `CORA_COWORK_BUN_PATH` override or an extracted
...
fn env_override() -> Option<PathBuf> {
    let raw = std::env::var("CORA_COWORK_BUN_PATH").ok()?;
    ...
    if p.is_file() {
        ...
        tracing::warn!(path = %p.display(), "CORA_COWORK_BUN_PATH does not point to a file; ignoring");
        None
    }
}
...
///
/// For `bun` / `bunx` we go through `cora_cowork_runtime` so the bundled
...
mod tests {
    ...
    fn no_embed_falls_back_to_which() {
        if !crate::test_support::run_in_env_child("resolver::tests::no_embed_falls_back_to_which", |command| {
            command.env_remove("CORA_COWORK_BUN_PATH");
        }) {
            return;
        }
    ...
    fn env_override_wins_over_embed() {
        ...
        if !crate::test_support::run_in_env_child("resolver::tests::env_override_wins_over_embed", |command| {
            command.env("CORA_COWORK_BUN_PATH", &path);
        }) {
            return;
        }
        let path = PathBuf::from(std::env::var_os("CORA_COWORK_BUN_PATH").expect("CORA_COWORK_BUN_PATH"));
    ...
    fn bad_env_override_falls_through_to_embed() {
        if !crate::test_support::run_in_env_child(
            ...
            |command| {
                command.env("CORA_COWORK_BUN_PATH", "/definitely/does/not/exist");
            },
        ) {
            return;
        }
