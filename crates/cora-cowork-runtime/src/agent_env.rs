...
mod tests {
    ...
    const CHILD_MARKER: &str = "CORA_COWORK_RUNTIME_AGENT_ENV_TEST_CHILD";
...
"#,
            ...
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("CORA_COWORK_CURRENT_ONLY", "from-current")
                .env("CORA_COWORK_OVERLAY", "from-current")
        ...
        assert_eq!(value("CORA_COWORK_CURRENT_ONLY").as_deref(), Some("from-current"));
        assert_eq!(value("CORA_COWORK_SHELL_ONLY").as_deref(), Some("from-shell"));
        assert_eq!(value("CORA_COWORK_OVERLAY").as_deref(), Some("from-shell"));
