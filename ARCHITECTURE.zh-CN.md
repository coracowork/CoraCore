...
依赖方向严格向下流动。领域 crate 不可依赖 cora-cowork-app，
cora-cowork-common 没有任何内部依赖。
...
| Crate | 职责 |
|-------|------|
| `cora-cowork-common` | 共享错误类型（ApiError）、枚举、ID 生成、加密工具、时间戳、分页 |
...
| Crate | 职责 |
|-------|------|
| `cora-cowork-auth` | JWT 认证、密码哈希、CSRF 保护、Cookie 管理、认证中间件 |
| `cora-cowork-realtime` | WebSocket 连接管理、事件广播（BroadcastEventBus）、消息路由 |
...
| Crate | 职责 |
|-------|------|
| `cora-cowork-conversation` | 对话管理、消息收发、确认机制、流式响应 |
| `cora-cowork-channel` | 多渠道集成（微信、钉钉、飞书）、插件系统、配对会话 |
| `cora-cowork-team` | 团队协作、任务调度、邮箱系统 |
| `cora-cowork-cron` | 定时任务执行、Cron 表达式、事件触发 |
| `cora-cowork-file` | 文件操作、监听、快照、Git 操作、压缩 |
| `cora-cowork-office` | Office 文档处理（Excel、PPT、Word）、预览、转换 |
| `cora-cowork-system` | 系统设置、提供商管理、版本检查、模型获取 |
| `cora-cowork-mcp` | MCP 协议集成、OAuth、多平台适配器 |
| `cora-cowork-ai-agent` | Agent 生命周期管理、Worker 任务队列、ACP/辅助技能 |
| `cora-cowork-extension` | 扩展注册中心、Hub 管理、技能发现与安装 |
| `cora-cowork-shell` | Shell 命令执行、语音转文字 |
| `cora-cowork-assistant` | Assistant 配置与管理 |

### 组装层（Composition）

| Crate | 职责 |
|-------|------|
| `cora-cowork-app` | 顶层二进制入口，组装所有 crate 为完整的 Axum 服务 |
...
每个领域 crate 遵循统一的内部组织模式，以 cora-cowork-conversation 为参考示例：
...
所有响应类型定义在 `cora-cowork-api-types` 中，这是 API 契约的唯一来源。
...
ACP Agent 的工具调用事件在 `cora-cowork-ai-agent` 翻译层进入统一
...
所有数据库访问通过 trait 抽象，定义在 `cora-cowork-db` 中：
...
| 类型 | 位置 | 用途 | 示例 |
|------|------|------|------|
| Row 模型 | `cora-cowork-db/src/models/` | 数据库行映射 | `ConversationRow` |
| Params 对象 | `cora-cowork-db/src/repository/` | 数据库写入参数 | `UpdateConversationParams` |
| 请求/响应类型 | `cora-cowork-api-types` | API 契约与共享 DTO | `CreateConversationRequest`、`ConversationResponse` |

**Service 层可以直接使用 `cora-cowork-api-types` 中的类型。** 该 crate 是纯数据结构定义，
不依赖任何 HTTP 框架，本质上是共享 DTO 层。

⚠️ **关键约束：`cora-cowork-api-types` 禁止依赖 axum、tower 等 HTTP 框架，
...
- **Repository（cora-cowork-db）**：纯数据库操作，不包含业务逻辑
...
- 迁移文件位于 `crates/cora-cowork-db/migrations/`
...
```
DbError（数据库层）
  ↓ From trait 实现（cora-cowork-db/src/error.rs）
ApiError（统一错误类型）
  ↓ IntoResponse 实现
...
`cora-cowork-app` 中定义 `AppServices`，集中持有所有共享依赖：
...
- **领域 crate 不负责构建自己的依赖**——只定义需要什么（RouterState），由 `cora-cowork-app` 负责组装
...
- Token 提取优先级：`Authorization: Bearer` 头 → `cora-cowork-session` Cookie
- 支持 Token 黑名单（SHA-256 哈希，DashMap 存储）

### CSRF 保护

采用 Double Submit Cookie 模式：
- Cookie 名：`cora-cowork-csrf-token`（非 HttpOnly，JavaScript 需读取）
- 请求头：`x-csrf-token`
...
|--------|----------|--------|----------|---------|
| `cora-cowork-session` | ✅ | HTTPS 时 | Strict(HTTPS) / Lax(HTTP) | 30 天 |
| `cora-cowork-csrf-token` | ❌ | HTTPS 时 | Strict(HTTPS) / Lax(HTTP) | 30 天 |
...
| E2E 测试 | `crates/cora-cowork-app/tests/` | 内存 SQLite | 完整 HTTP 请求链路验证 |
...
`cora-cowork-app/tests/common/mod.rs` 提供共享的测试工具：
...
以添加 `cora-cowork-my-feature` 为例：

**第一步：创建 crate 并注册到 workspace**

1. 创建目录 `crates/cora-cowork-my-feature/`
...
**第三步：如需数据库，在 cora-cowork-db 中添加**

1. 在 `models/` 添加 Row 模型
2. 在 `repository/` 定义 Repository trait（`I` 前缀）和 Sqlite 实现
3. 在 `migrations/` 添加迁移文件（`NNN_descriptive_name.sql`）

**第四步：如需 API 类型，在 cora-cowork-api-types 中添加**

在 `cora-cowork-api-types` 中定义请求/响应类型，保持 API 契约集中管理。

**第五步：接入 cora-cowork-app**

1. 在 `cora-cowork-app/Cargo.toml` 添加依赖：
   ```toml
...
- [ ] Repository trait 定义在 cora-cowork-db，实现使用 Sqlite 前缀
- [ ] API 类型定义在 cora-cowork-api-types
