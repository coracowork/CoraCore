...
4. Never pass, inline, export, echo, or set any `cora-cowork_...` environment variable in any shell command. cora-cowork injects the required runtime environment before the helper runs.
5. Helper commands must directly call `"$cora-cowork_HELPER_BIN" cron-helper ...` without any `cora-cowork_...=...` prefix.
...
1. Run `"$cora-cowork_HELPER_BIN" cron-helper list`.
2. If the returned `data` array is empty, create the task with `"$cora-cowork_HELPER_BIN" cron-helper create <<'JSON'`.
3. If one task exists and the user wants to change it, update that task with `"$cora-cowork_HELPER_BIN" cron-helper update --job-id <job-id> <<'JSON'`.
...
```bash
"$cora-cowork_HELPER_BIN" cron-helper list
```

Create:

```bash
"$cora-cowork_HELPER_BIN" cron-helper create <<'JSON'
{
...
```bash
"$cora-cowork_HELPER_BIN" cron-helper update --job-id cron_123 <<'JSON'
{
  "name": "Daily Summary",
